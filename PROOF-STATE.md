...
| 6 | `opIterD-budget` (was `opIterD≤sizeCount-root-core`) | Op-Dominance.agda | **LANDED + HALVED 2026-08-06 — residual is the climb budget** | The `-core` is a REAL DEFINITION in Caps-Bridge (its seven expression hypotheses kept: `nest≤` applied, six reserved for the residual's structural proof). New module `Verify-Budget-Sufficient/Op-Dominance.agda`: `opIterD-dominated` is real; counting bounds `m≤cSize`/`k≤cSize` proven inline from `nest≤` + `capsAt-base-size`. **The route's named open question is PROVEN**: `fIterD-lvls : fIterD S W d k n J ≤ lvls S W d J n` — ‹bound in n› = n, because `fIterD` IS `iterL` (k unused) and one `dLvl` definitionally CONTAINS a leading `fLvlD` step. Route step (c) ("most uncertain") is closed. **Sole residual: `opIterD-budget`** — the recursion's climb fits the budget, both endpoints in the same lvls-currency (steps (a)+(b), the residual-budget induction on m mutual with sLvlD's k-descent). Probe `Battery-OpIter-Symbolic.agda` DELETED (landed; this commit). |
...
| 12 | `three-size≤capsH-core` | Caps-Bridge.agda:1021 | **DISCHARGEABLE — one line, at symbolic `e`/`ins`** | Not a probe result — a PROOF. `agda/probe/Battery-Caps-Init.agda:194` states the postulate's full type at symbolic `e`/`ins` and closes it with `three-size-le-blowH _ _ (s≤s (s≤s z≤n))`. The reduction `capsH e ins 0 = capsHgo (capsBase e ins) 0 = blowH (capsBase e ins)` is definitional (neither is `abstract`) and the side condition `2 ≤ 3 + X + suc E` is free. **Landing plan:** move `three-size-le-blowH` + its helper chain from `Pool-Lower-Probe` into `Verify-Budget-Sufficient/Caps.agda` (all deps already there), delete the postulate, define `three-size≤capsH` directly. **WIRING CONSEQUENCE — do not miss it:** the postulate's `S≤sizeStep` hypothesis is UNUSED by the proof, and `S≤sizeStep` (Caps-Face:479) has exactly ONE consumer, the assembly at Caps-Bridge:1047. Discharging therefore ORPHANS it — decide delete-or-rewire in the SAME commit, per the wiring law. (That the hypothesis was never needed is itself a signal the `-core` was over-specified.) |

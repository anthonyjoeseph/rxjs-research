...
| 6 | `opIterD-budget-core` (was `opIterD-budget`, was `opIterD≤sizeCount-root-core`) | Op-Dominance.agda | **REFUTED AND REPAIRED 2026-08-06 (`7a82c6c`, `7e077e2`, `df2bb71`)** | **The statement was FALSE.** Machine-refuted in `probe/OpIterD-Budget-Probe.agda` §1 (`opIterD-budget-R0-false`, a proven `→ ⊥`): at `R = 0` the registry walk is empty (`regAt S 0 J = 0 * suc (J * S) = 0`), so `cDel (caps S W 0) d = 0` and the RHS collapses to `lvls S W d 0 0 = 0` — while the LHS is a `dLvl` application and `2≤dLvl` holds unconditionally. The claim read `2 ≤ 0`. The missing hypothesis is `1 ≤ R`, now carried by both `opIterD-budget-core` and `opIterD-dominated`, and **the refutation is recorded in the postulate's own header so the weaker form cannot be re-derived by accident.** Discharge cost nothing: `1≤capsAt-reg` already existed (`Caps.agda:929`). Since converted to a `-core` over the SEVEN expression-level lemmas (`entry-to-index`, `nest≤`, `residAt-connected`, `share-step-resid`, `mu-1≤k`, `mu-step-le`, `k-raise`) — the kit its own header had always named as the consumer, orphaned when the root chain was deleted and rewired here rather than thrown away. **Still open, and still the genuinely new mathematics:** the residual-budget induction on `m` (mutual with `sLvlD`'s k-descent). It cannot be probed into confidence — `widAt 2 1 10` is a ten-story tower, so concrete evaluation is infeasible. The route step it rests on (`fIterD-lvls`, ‹bound in n› = n) IS proven. **THE INVARIANT IS NOW DESIGNED (2026-08-06, `probe/OpIterD-Invariant-Rehearsal.agda` — LANDING-tagged, assembly to the -core's exact conclusion TYPECHECKS).** The design rests on two normal forms: the climb is `TAIL^m ∘ (sLvlD k ∘ J₀)^m` (per-round jump + (k−1)-sub-climb + tail, tails already lvls-dominated by the proven `fIterD-lvls`), and the budget — by the ALREADY-PROVEN `dWalkᶜ-front` — is `regAt`-many iterations of [one dLvl-step, then a full gas-(g−1) budget from the level reached]. **The crux tension and its resolution:** recursive rounds have `m′ = suc (sizeAt S X)` (exponential in the level) against `regAt ≈ R·J·S` positions (linear) — resolved because ONE dLvl-step gains ≥ `sizeAt S J` (it is `1 + sizeAt S J` fLvlD-steps), so a single position's boost puts the sub-walk's `regAt` above `m′`. **Gas accounting closes with ZERO slack at `k = S`** (`suc k ≤ g` descends `(k−1, g−1)` in lockstep against `cDel`'s gas `suc S` — presumably why the recurrence carries `suc cSize`), and `regAt S R 0 = R` may be 1, so at the top the entry and all m rounds must sequence inside ONE position's sub-budget — which is why `climb-paid` (entry+rounds, one budget) is the top statement and is UNSPLITTABLE into entry/rounds conjuncts. **What remains is five postulates in the rehearsal:** `rounds-paid` (the recursive payment invariant), `climb-paid` (its top unrolling), and three estimates — `dLvl-gain-sizeAt` (near-provable with the existing kit), `jump-2step` (needs a 2^-style lower bound on `sizeAt`), `tail-fits` (gas-2 budget length ≥ one tail; gas-1 PROVABLY cannot pay a tail, forcing the tail-headroom conjunct). Corners recorded in the rehearsal header: `k = S` gas exactness, `S = 2` saved by `m ≤ 1`, `d = 0` vs `d ≥ 1` fLvlD gains, and the depth off-by-one (a depth-d L-step contains depth-(d−1) machinery — containment must never pay same-depth climbs; only step COUNTS may). Toolkit already proven: `dWalkᶜ-front`, `dCapᶜ-mono`/`dWalkᶜ-mono` (all arguments incl. gas/level/positions), `dLvl-mono`, `sizeAt-mono`, `widAt-mono`, `lvls-add`, `iterL-infl`, `2≤dLvl`. **UPDATE `ca3f502`: `walk-paid` — the m-induction itself — is now a REAL definition in the rehearsal** (all three estimates proven off the fLvl receipt; the glue kit — walk-spend/walk-spend-many/restart-dominates/G-mono/G-tail/lvls-infl — proven; the walk value at p positions IS the p-th restart level, which makes each round's conclusion the next round's hypothesis verbatim). Open surface: `round-entry-glue` + `round-tail-glue` (one round's jump+sub-climb and one tail — narrow, the entry one carries the (g−1,k−1) recursion), and `climb-paid`'s top unrolling, **blocked on the k = S corner ruling that formalization surfaced**: tails charged at their own walk level cost one gas more than `cDel`'s `suc S` provides at `k = S` (scheme covers `k ≤ S−2` outright). Fix (i): pending-tail accumulator (charge tails two ancestor levels up) — exact but heavy. Fix (ii), repo-preferred: ONE unit of guard slack `nest + 2 ≤ cSize` threaded from the consumers — likely free via the base caps' `2+` floor over `sizeᵉ + slotsSize` and `nest≤`, but it is a statement repair across the -core's guard chain, to be ruled on fresh, not improvised. |
| 7 | `init-capsOK?-base-core` | Caps-Bridge.agda | **COMPLETE PROOF IN HAND 2026-08-07 (`probe/Init-CapsOK-Core-Probe.agda`, green, zero postulates) — LANDING DEFERRED on the 8-orphan ruling** | All five `capsOK?` conjuncts proven, INCLUDING the previously-open non-scripted-live branch: `widLive-mkHot` closes it because `scripted`'s own `{ok : T (isData t)}` index forces `pWᵛ ≡ 0` on every pending value (new `outWᵛ/dWᵛ/pWᵛ-data-zero` structural family), and cold/shared slots contribute no live source at init. The direct proof BYPASSES the -core's 8 scaffold hypotheses — they were kit for a route that was never needed — so landing it orphans all 8, and their homes (Measures, Frame-Width, Caps-Face) are full-rebuild modules. Deferred to the next Measures-dirty window (task #48); the -core postulate stays until then, with the probe as its proof-in-escrow. Old row (probe evidence, structural arguments): | `capsOK?` (Caps-Face:297) is five conjuncts; at `st-init` the registry and nodes are empty, so (2) `regsSz?`, (4) `widNode`, (5) `length ≤ᵇ cReg` pass by `all _ []` — the known-vacuous three. The first probe pass left the two LIVE conjuncts vacuous as well (empty `live`; `pending = []`), which is why it did not count. **The second pass covers them non-vacuously** (`agda/probe/Battery-Caps-Init.agda`, programs C and D: one and three pending scripted values) — green — and, better than rows, gives a REASON refutation is impossible at `baseCaps`: (3) `widLive` cannot fail because `scripted` requires `T (isData t)` and every data type has `pWᵛ ≡ 0` (Frame-Width:294–299), so the check is `0 ≤ᵇ cWid`; (1) `stBounded?` cannot fail because each pending `v` has `sizeᵛ t v < cSize = 2 + sizeᵉ e + slotsSize ins` by construction. **Those two arguments are the proof sketch** — this looks provable, not merely probable. Residual gap: the argument covers `scripted` slots; non-scripted entries in `Sched.live` at init are not analysed. |

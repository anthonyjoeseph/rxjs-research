# fixture — the tier being WORKED has an EMPTY ledger, and the unevidenced row sits in a PARKED tier below it: must PASS

## Tier 0 — the tier whose judge is not a postulate

### Big picture tier roadmap

- **the differential sweep** — re-measure the harness against the reference
  implementation before repairing any shape it reports, since the recorded
  figure predates three rewrites.
- **the two disagreeing shapes** — each one a commit, and each one checked
  against the generator before the implementation, a mis-drawn program being
  the cheaper thing to be wrong about.
- **re-arm the gate** — back into the cheap list and off its CI skip, the two
  moving together so the tier cannot silently rot once it is green.

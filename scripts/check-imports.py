...
CLAIM_ROOT = {
    ...
    os.path.join("agda", "refuted"): os.path.join("Refuted", "Main.agda"),
}
...
def audit(path: str):
    ...
    def used(name: str) -> bool:
        ...
        return any(p in tokens for p in parts) if parts else True
    ...
    return raw, stripped, decls, dead_decls, dead_names, skipped, blanket, reexport
...
def rewrite(raw: str, stripped: str, dead_decls, dead_names) -> str:
    ...
    for d, unused in dead_names:
        ...
        for part in stripped[a + 1: b - 1].split(";"):
            ...
            if (local.group(2) if local else nm) not in unused:
                keep.append(nm)
...
def main() -> int:
    ...
    ap.add_argument("--src", action="append", default=None,
                    help="tree to walk (repeatable); default agda/src + agda/refuted")
    ap.add_argument("--names", action="store_true",
                    help="also report unused NAMES inside a surviving `using` "
                         "clause.  Hygiene only, and not what the gate checks: a "
                         "clause with one live name holds the module edge open, so "
                         "the other thirty cost nothing a build can measure.")
    ...
    wiring, blankets, reexports = [], [], []
    for path in files:
        kind, res = per_file[path]
        raw, stripped, decls, dead_decls, dead_names, skipped, blanket, reexport = res
    ...
    tail = f" and {n_name} dead name(s)" if args.names else ""
    ...
    if n_dead and not args.fix:
        print("imports-check: run `make imports-fix` to delete them")
    if args.fix:
        return 1 if (n_wire or n_blanket or n_reex) else 0
    return 1 if (n_dead or n_name or n_wire or n_blanket or n_reex) else 0

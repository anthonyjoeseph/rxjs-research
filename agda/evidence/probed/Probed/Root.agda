...
-- close their k ≡ 0 case — see Part4.root-mergeAllCache); the pins stay, since
...
-- mergeAll-cert in .Part4 states its own predicate; these rows are evidence
-- about THIS one.
...
-- delivered / cancelled all empty) makes mergeAllCertAt FALSE, so the
...
-- hand-built): the single-inner unbounded-mergeAll shape, mid-cascade
...
-- mergeAll-cert at one node: when mergeAll-st's live count sits at 0, no
-- registry entry has an alive from-inner instance of this node.  A
-- positive count and non-mergeAll nodes are trivially certified.
mergeAllCertAt : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t}
  → NodeId → EvalSt e → Bool
mergeAllCertAt mnid st with lookupNode mnid (EvalSt.nodes st)
... | just (mergeAll-st lim zero q od) =
...
P1 : Closed Γ₀ natᵗ                       -- one mergeAll-st (live count)
P1 = mergeAllᵉ nothing (ofᵉ (strmᵗ (ofᵉ (nat̂ 1 ∷ [])) ∷ []))

P2 : Closed Γ₀ natᵗ                       -- unbounded mergeAll over TWO inners
P2 = mergeAllᵉ nothing (ofᵉ (strmᵗ (ofᵉ (nat̂ 1 ∷ [])) ∷ strmᵗ (ofᵉ (nat̂ 2 ∷ [])) ∷ []))

P3 : Closed Γ₀ natᵗ                       -- nested: two mergeAll nodes
P3 = mergeAllᵉ nothing (ofᵉ (strmᵗ P1 ∷ []))

P4 : Closed Γ₀ natᵗ                       -- mergeAll-st at limit 1: a QUEUE
P4 = mergeAllᵉ (just 1) (ofᵉ (strmᵗ (ofᵉ (nat̂ 1 ∷ [])) ∷ strmᵗ (ofᵉ (nat̂ 2 ∷ [])) ∷ []))
...
S1 : Closed Γ₁ natᵗ
S1 = mergeAllᵉ nothing (ofᵉ (strmᵗ (input zero) ∷ strmᵗ (input zero) ∷ []))
...
-- THE FORMER SHARED HYPOTHESIS.  Both -cores took mergeAll-cert; neither
-- assembly takes it now.  mergeAllCertAt is decidable, so
-- it is pinned rather than assumed — at every node id the merge programs
-- actually mint, plus one that does not exist (where mergeAllCertAt is true by
...
-- keeps mergeAllCertAt honest while it has no consumer in a proof.
_ : mergeAllCertAt 0 (RUN P1) ∷ mergeAllCertAt 0 (RUN P2) ∷ mergeAllCertAt 1 (RUN P3)
  ∷ mergeAllCertAt 0 (RUN P3) ∷ mergeAllCertAt 0 (RUN P7) ∷ mergeAllCertAt 0 (RUN₂ S2)

...
-- taken at a queue holding a program written out, where the summand is
-- a rounding and the queue's own depth is the charge.  The summand was
-- added for the other shape: a queue parking a SLOT REFERENCE, whose
-- layers are zero and whose run is the whole of the slot's definition.
-- There the summand is not a rounding but the entire charge, so it is
-- the one place a green row says something the predecessor reading did
-- not already say.  Both rows stand at the refutation's own states.
--
...
-- each pair IS the refutation's row, so a repair that had reached the
-- schedule without reading the slot table would repeat it in the
-- second.  What the pair decides is REACHABILITY: the drain is handed
-- a `Sched`, and whether the telescope standing behind a parked
...
-- that the summand REACHES the drain, not that its size is right.
--
...
-- decided rather than its reach; a queue holding more than one parked
-- program, where the MAX join is the claim; a queue mixing a reference
-- with a written-out program, where the join is asked to cover two
-- charges of different kinds; a telescope of several slots, since both
-- rows carry one; a slot that is `scripted` rather than `shared`, whose
-- definition the subscription does not run; and the ledger tie, which
...
open import Rx.Evaluator using (root; sched-init; iterSize)
...
open import Refuted.Drain-Queue-Slot
  using (q₂; st₂; out₂; q₃; st₃; out₃)
open import Probed.Apparatus using (Confirms)

----------------------------------------------------------------------
-- THE FOUR CHARGES, READ OFF THE STATES THE ROWS STAND AT: the parked
-- queue's own depth, and that depth plus the telescope the reference
-- names.  The first of each pair is what the refuted reading saw.
----------------------------------------------------------------------
slotCharges : List ℕ
slotCharges = layᵛˢ {Γ = Γ₂} (obs (Pw 12)) q₂
            ∷ layᵛˢ {Γ = Γ₂} (obs (Pw 12)) q₂ + slotsSize sl₂
            ∷ layᵛˢ {Γ = Γ₃} (obs (Pw 13)) q₃
            ∷ layᵛˢ {Γ = Γ₃} (obs (Pw 13)) q₃ + slotsSize sl₃
            ∷ []
...
-- charges that moved with the slot and not with the queue.
slotRows : List Bool
slotRows = valsSz? {Γ = Γ₂} {s = Pw 12}
             (iterSize 51 (layᵛˢ (obs (Pw 12)) q₂) 51) out₂
         ∷ valsSz? {Γ = Γ₂} {s = Pw 12}
             (iterSize 51 (layᵛˢ (obs (Pw 12)) q₂ + slotsSize sl₂) 51) out₂
         ∷ valsSz? {Γ = Γ₃} {s = Pw 13}
             (iterSize 55 (layᵛˢ (obs (Pw 13)) q₃) 55) out₃
         ∷ valsSz? {Γ = Γ₃} {s = Pw 13}
             (iterSize 55 (layᵛˢ (obs (Pw 13)) q₃ + slotsSize sl₃) 55) out₃
         ∷ []
...
-- LOAD-BEARING: the queue, the limit and the active count are the ones
-- an exiting inner hands the drain at this door, so the row stands
-- where the arm actually reaches it.
tieDrainSlot12 : Confirms
  (mergeAllDrain-sz {e = e₂} (gasPad 64 g0) 0 root 0 0 nothing 0 q₂
     (sched-init e₂ sl₂) st₂ 51 51)
tieDrainSlot12 = λ _ _ _ → refl

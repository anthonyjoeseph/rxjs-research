...
-- This file is APPEND-ONLY and grows monotonically: scripts/gen-unit-tests.sh
-- appends each newly-discovered counterexample (deduped by program text) and
-- NEVER deletes or overwrites. A fixed bug simply becomes a passing guard that
-- stays forever. Invariant: this module fully typechecks  ⟺  no known
-- counterexample remains — i.e. green here is the impl≡spec finish line.
...
-- Accumulated guards (fixed bugs — must stay green forever):
...
------------------------------------------------------------------------
-- Appended by scripts/gen-unit-tests.sh (append-only; deduped by program
-- text). Hidden mapE/scanE value functions are replaced by idv/scz — they
-- relabel values, never move batch boundaries, so the batch-STRUCTURE
-- discrepancy under test is preserved. Currently-open bugs are RED here until
-- fixed; a fixed one just goes green and stays.
-- APPEND BELOW
------------------------------------------------------------------------
-- seed 181

...
open import Rx.Exp
  using (Ty; Ctx; Exp; Tm; subΘExp; subΘTm; subΘTms; input; ofᵉ; emptyᵉ; takeᵉ; batchSyncᵉ; mapᵉ;
  scanᵉ; mergeAllᵉ; switchAllᵉ; exhaustAllᵉ; μᵉ; varᵉ; deferᵉ; mintᵉ; uniqᵗ; varᵗ; unit̂;
  bool̂; nat̂; uniq̂; pairᵗ; fstᵗ; sndᵗ; inlᵗ; inrᵗ; caseᵗ; ifᵗ; primᵗ; strmᵗ; _×ᵗ_; nilᵗ;
  consᵗ; foldᵗ)
open import Rx.Subst-Transport
  using ( Cᵉ; Cᵗ; Cˢ; shift; ∈-id; cong₃
        ; pushInput; pushEmpty; pushVarᵉ; pushOf; pushTake; pushMap; pushScan
...
mutual
  ...
  subΘ-idᵉ Θloc (mapᵉ {s = s} f e) =
    trans (cong₂ mapᵉ
            (trans (subΘ-idᵗ (s ∷ Θloc) f) (shift (++-identityʳ Θloc) f))
            (subΘ-idᵉ Θloc e))
          (pushMap (++-identityʳ Θloc) f e)
  subΘ-idᵉ Θloc (scanᵉ {s = s} {t = t} f i e) =
    trans (cong₃ scanᵉ
            (trans (subΘ-idᵗ ((t ×ᵗ s) ∷ Θloc) f) (shift (++-identityʳ Θloc) f))
            (subΘ-idᵗ Θloc i) (subΘ-idᵉ Θloc e))
          (pushScan (++-identityʳ Θloc) f i e)
  ...
  subΘ-idᵉ Θloc (mintᵉ e) =
    trans (cong mintᵉ (trans (subΘ-idᵉ (uniqᵗ ∷ Θloc) e) (shiftᵉ (++-identityʳ Θloc) e)))
          (pushMint (++-identityʳ Θloc) e)

...
-- accumulator at an identifier taken from the run's node counter and
...
-- THE MINT, AND THAT IS WHAT KEEPS THE TWO CONJUNCTS APART.  Stated
-- against the counter directly, every recursive premise would sit at an
-- advanced counter and the induction would have to transport the bound
-- through each one.  Taking the floor as an argument makes each premise
-- an instance of the SAME statement at the SAME floor, so preservation
-- is one family.  Monotonicity is still owed, but only to re-establish
-- the floor hypothesis for a premise running at a mint an earlier
-- premise handed back -- which is a separate induction over the same
-- relations, in its own module, because neither family is a member of
-- the other's cycle.
--
...
-- WHERE THE COUNTER SITS, NAMED ONCE.  Every statement in this tower
-- and the one above it reads the node key out of the run's single
-- ledger, and spelling that projection at each site is what made the
-- old three-field shape unreadable.

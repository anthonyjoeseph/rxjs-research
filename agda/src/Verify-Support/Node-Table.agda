...
--
-- THEY LIVE BELOW THE WELL-FORMEDNESS BRANCH because several of its
-- parts need them and nothing there is the lowest module reaching all
-- of those.  Rather than grow a second copy — which `make dup-check`
-- would fail, and rightly — the fact sits under every consumer.
-- `≡ᵇ-refl` travels with them because `lookupNode-setNode` is stated
-- over the same `_≡ᵇ_` the table is keyed by.
------------------------------------------------------------------
module Verify-Support.Node-Table where

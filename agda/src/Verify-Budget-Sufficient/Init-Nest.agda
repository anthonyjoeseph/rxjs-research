------------------------------------------------------------------
-- INIT-NEST: the initial store's nesting sits under the program's own
-- wrap unit.
--
...
--   (1) slotsNestSum slots  — a summand of the unit itself
...
--   restores the instant-indexed form, `init-nestOK?`, which lifted
--   this to `nestOK?` at every instant through the cap's monotonicity.
--   Nothing asks for it while the entry's only consumer reads instant
--   zero.
------------------------------------------------------------------
...
open import Data.Nat.Properties using (≤-trans; ≤-reflexive; m≤n+m; n≤1+n)

...
open import Data.Nat.Properties using
  (≤-refl; ≤-trans; ≤-reflexive; ≤ᵇ⇒≤; n≤1+n; m≤n+m; +-assoc; +-comm; +-monoˡ-≤; +-monoʳ-≤;
  *-assoc; *-comm; m^n>0; *-identityˡ; *-identityʳ; *-zeroʳ; *-mono-≤; *-monoˡ-≤; *-monoʳ-≤;
  +-mono-≤; *-distribˡ-+; ^-zeroˡ; +-identityʳ; m≤m+n; m≤m⊔n; m≤n⊔m; ⊔-lub; ⊔-assoc; ⊔-mono-≤;
  ^-distribˡ-+-*; ^-monoˡ-≤; ≤⇒≤ᵇ)
...
open import Rx.Evaluator using
  ...
  thru-outer; foldPath; stepFrame; shareAdmit; shareLatch; RegId; NodeId; AllOp; mergeAllᵒ;
  switchᵒ; exhaustᵒ; NodeState; scan-st; take-st; takeVals; mergeAll-st; switch-st; exhaust-st;
  lookupNode; setNode; scanVals; takeDispatch; innerFinish; aliveThroughᶠ; mergeAllDrain;
  subscribeInner; hasRoom; mergeAllBump; switchKill; subscribeE; splitBurst; Stream; mintNode;
  installNode; pushBurst; oneShotBurst; splitEvents; thruConsume; thruWalk; thruWrap;
  retagEvents; subscribeSharedSlot; memberSource; mintSource; sharedConnect; sharedPlumb;
  burstCompleted; register; dropSource; opIterD; sLvlD)
open import Verify-Budget-Sufficient.Keeps-Ring using
  (KeepsC; subscribeE-keeps; subscribeInner-keeps; stepFrame-keeps; thruConsume-keeps;
  switchKill-nodes; switchKill-nextNode)
...
-- THE SUBSCRIBED BURST'S OWN READING, which the caps face does not
-- report: its conclusion carries caps and a count and no floor at all
...
-- the freshness receipt a consume hands back is what both chain-wide
...
open import Verify-Budget-Sufficient.Nest-Ceiling using
  (CeilD; CeilS; ceil-step; ceil-le; ceil-mu; ceil-here; ceilS-head; ceilS-sweep-step)
...
open import Verify-Budget-Sufficient.Fan-Caps using (fanLen; fanSq; delSq; delSq-mono)
open import Verify-Budget-Sufficient.Deliver-Measure using
  (deliverLen; deliverNestF; deliverNestD; admSz?; deliverLen-path; deliverSzSum-path;
  deliverNestD-path; deliverNestF≡; pathSzSum-cap; pathNestD-len)
...
open import Verify-Budget-Sufficient.Nest-Cap using
  (nestB; nestB-mono; arrD≤nestB; nestB-base; nestB-frame; nestB-unit; nestFac; 1≤nestFac;
  nestU; nestU-mono; pow-mono-exp; nestB-at; arrD; arrDW-mono; arrDW-pos; arrDW-key;
  arrDW-flat; arrDW-slot; nestB-monoS; arrDW-frame; nestB-frame-dblW)
...
-- back.  What the flat reading rests on is the missing frame index,
-- and that is a fact about this predicate rather than about the walk:
-- the cone consuming it now REPORTS a level, so a restatement at a
-- stepped cap is what the registry append has been asking for, and the
-- strong face's own proven walk is stated in exactly that shape.
...
-- AND IT TRANSPORTS ALONG THE ORDERING ITSELF, which is what an arm
-- reporting a LEVEL needs and what the frozen-width transport above
-- cannot give: the cap such an arm reports at is the entry's own with
-- both numbers stepped, and each of the four conjuncts reads one field
-- upward.
face⊑ : ∀ {n} {Γ : Ctx n} (c c′ : Caps) (sl : Slots Γ) →
  c ⊑ᶜ c′ → FaceOK c sl → FaceOK c′ sl
face⊑ c c′ sl (hs , hw , hr) f =
  faceOK (≤-trans (FaceOK.fSize f) hs) (≤-trans (FaceOK.fReg f) hr)
    (slotsCaps?-widen (Caps.cSize c) (Caps.cSize c′)
      (Caps.cWid c) (Caps.cWid c′) sl hs hw (FaceOK.fSlC f))
    (≤-trans (FaceOK.fSlSz f) hs)

-- and the ordering from an entry to any level of it, which is the only
-- one the levelled arms below are ever entered at
...
-- and the WRITTEN-size reading widens the same way, which is what lets
-- a levelled arm hand an arrival on at the cap it reports at rather
-- than at the one it was entered with
nestValOK?-widen : ∀ {n} {Γ : Ctx n} {c c′ : Caps} (u : Ty) (v : Val Γ u) →
  c ⊑ᶜ c′ → nestValOK? c u v ≡ true → nestValOK? c′ u v ≡ true
nestValOK?-widen {c = c} u v le h =
  ≤ᵇ-true _ _ (≤-trans (≤ᵇ⇒≤ (syncSizeᵛ u v) (Caps.cSize c) (T-to h)) (proj₁ le))
...
-- and it widens with the cap, which is the half a level-indexed
-- consumer wants: the closure reading is one number against `cSize`
-- and nothing else, so a bigger cap is a weaker demand.
abstract
  nestClosOK?-widen : ∀ {n} {Γ : Ctx n} {u} (c c′ : Caps) (sl : Slots Γ)
    (o : Val Γ (obs u)) →
    c ⊑ᶜ c′ → nestClosOK? c sl o ≡ true → nestClosOK? c′ sl o ≡ true
  nestClosOK?-widen c c′ sl o le h =
    ≤ᵇ-true (closSizeᵉ (slotClos sl) o) (Caps.cSize c′)
      (≤-trans (nestClosOK?-size c sl o h) (proj₁ le))
...
-- AND THE CLOSURE KEY IS READ ONE LEVEL ABOVE THE REST OF THE ENTRY,
-- which is the only conjunct here that is not at the frame's own
-- level.  Nothing records a queued term's closure size: the store
-- predicate's closure half ranges over the live list and never over
-- the node table, and the size the park field DOES record buys the key
-- only through the capped telescope, at the cap times the plain size
-- -- a ratio no single cap absorbs, and the level shift is what pays
-- for it.  One frame step absorbs it exactly, so the entry states the
-- key where it can be minted rather than where it would be convenient.
-- The head's own size conjunct stays at the entry's level, and the
-- successor the ceiling is admitted under is bought where it is spent
-- rather than stored: a level's size cap climbs strictly, so the entry
-- read one level up has room for the successor its consumer charges.
-- REFUTED: `Refuted.Nest-Clos-Stratified`, the key at the entry's own
--   level against a capped telescope, and
--   `Refuted.Step-Frame-Clos-Level` for the same reading one frame in.
...
--
-- AND THE CLIMB IS BOUNDED BY A SWEEP OFF THE ENTRY'S OWN LEVEL, at
-- the depth the frame's room is spent one step BELOW.  The drain runs
-- under a frame whose allowance is the frame level at the count depth,
-- and one frame level is a whole sweep at the depth under it -- so the
-- currency an entry's climb is reported in is a sweep at `pred d`,
-- while the ceiling it buys is still denominated in the count at `d`.
-- A climb bounded at `d` would be asking the frame for room it does
-- not have; a climb bounded at `pred d` is exactly the room one frame
-- step already paid for.
capsDrainAt : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {s}
  (c : Caps) (sl : Slots Γ) (d Lv B̂ : ℕ) (sf : Gas) (allNid : NodeId) (κ : Path Γ s t)
  (id : Id) (now : Tick) (lim : Maybe ℕ) (act : ℕ)
  (q : List (Closed Γ s)) (sched : Sched Γ) (st : EvalSt e) → Set
capsDrainAt c sl d Lv B̂ sf allNid κ id now lim act [] sched st =
  (Sched.slots sched ≡ sl) × (capsOK? (frameStep Lv c) sched st ≡ true)
capsDrainAt {n = n} {s = s} c sl d Lv B̂ sf allNid κ id now lim act (o ∷ q) sched st =
  (Sched.slots sched ≡ sl) × (capsOK? (frameStep Lv c) sched st ≡ true)
  × (nestValOK? (frameStep Lv c) (obs s) o ≡ true)
  × (nestClosOK? (frameStep (suc Lv) c) sl o ≡ true)
  × (sizeᵉ o ≤ Caps.cSize (frameStep Lv c))
  × (dWᵉ n sl o ≤ Caps.cWid (frameStep Lv c))
  × (nest o sl (EvalSt.connectedShares st) ≤ B̂)
  × (Σ ℕ λ r₀ →
      (Lv + r₀ ≤ sLvlD (Caps.cSize c) (Caps.cWid c) (pred d) (suc B̂) (suc Lv)) ×
      capsDrainAt c sl d (Lv + r₀) B̂ sf allNid κ id now lim
        (if proj₁ (proj₂ (proj₂ (proj₂ r))) then act else suc act) q
        (proj₁ (proj₂ (proj₂ (proj₂ (proj₂ r)))))
        (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ r))))))
  where r = subscribeInner sf mergeAllᵒ allNid κ id now o sched st

-- AND THE NESTING BOUND IS THE DRAIN'S OWN, NOT ITS CALLER'S, so it is
...
-- is also admitted by.  The bound is not free -- enlarging it widens
-- the ledger the ceiling has to dominate -- so it is pinned from both
-- sides rather than upward-closed.
--
-- AND THE RECEIPT IS A SWEEP AND NOT A COUNT, which is what lets the
-- frame mint it with nothing but its own allowance.  A count ceiling
-- has to be handed down from somewhere; a sweep ceiling is what one
-- frame level IS, so the fold's single receipt is exactly the room the
-- frame already holds, spent one entry at a time and read off as a
-- count ceiling only where an entry is actually subscribed.
...
capsDrainOK c sl d Lv sf allNid κ id now lim act q sched st =
  Σ ℕ λ B̂ →
    CeilS c (pred d) d Lv (suc B̂) (length q)
    × capsDrainAt c sl d Lv B̂ sf allNid κ id now lim act q sched st
...
-- AND THE ENQUEUE IS THE ONE WRITE ON THIS FACE THAT OVERWRITES PARKED
-- CONTENT, so it is the one that takes the chain.  Every other node
-- write here either mints a cell whose reading is `true` outright or
-- moves a field the reading does not look at; this one grows a queue a
-- registered reader reads at its OWN floor, and node ownership is what
-- ties that floor to the one the arrival is bounded by.
merge-park-caps : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (c : Caps) (L : ℕ) (nid : NodeId) (κ : Path Γ u t) (lim : Maybe ℕ) (act : ℕ)
...
merge-park-caps {n = n} {u = u} c L nid κ lim act q od o sched st hS hsz₀ eq hc hsz hw stO stW =
  ...
  stepL = frameStep-mono-j c hS (n≤1+n L)
  ...
  pk₀ = subst (NodePark (Caps.cSize (frameStep L c))
                        (slotsSize (Sched.slots sched))) eq
          (lookupNode-park (Caps.cSize (frameStep L c))
             (slotsSize (Sched.slots sched)) nid (EvalSt.nodes st)
             (capsOK?-nodePark (frameStep L c) sched st hc))

  pk : parkRoom (Caps.cSize (frameStep (suc L) c))
         (slotsSize (Sched.slots sched)) ns ≡ true
  pk = parkList-push c L (Sched.slots sched) q o hS hsz₀ hsz pk₀

  NC : NodeCaps (frameStep L c) (Sched.slots sched)
         (just (mergeAll-st {t = u} lim act q od))
  NC = subst (NodeCaps (frameStep L c) (Sched.slots sched)) eq
         (lookupNode-caps (frameStep L c) (Sched.slots sched) nid (EvalSt.nodes st)
            (capsOK?-nodeSz (frameStep L c) sched st hc)
            (capsOK?-nodeWid (frameStep L c) sched st hc))

  bn₀ : boundedNode (Caps.cSize (frameStep L c)) (mergeAll-st {t = u} lim act q od) ≡ true
  bn₀ = proj₁ NC

  bn : boundedNode (Caps.cSize (frameStep (suc L) c)) ns ≡ true
  bn = all-++-intro (λ x → sizeᵉ x ≤ᵇ Caps.cSize (frameStep (suc L) c)) q (o ∷ [])
         (all-impl _ _ (λ x hx → ≤ᵇ-true (sizeᵉ x) _
                                   (≤-trans (≤ᵇ⇒≤ (sizeᵉ x) _ (T-to hx)) (proj₁ stepL)))
            q bn₀)
         (∧-intro (≤ᵇ-true (sizeᵉ o) _ (≤-trans hsz (proj₁ stepL))) refl)

  wn₀ : widNode (Caps.cWid (frameStep L c)) (Sched.slots sched)
          (mergeAll-st {t = u} lim act q od) ≡ true
  wn₀ = proj₂ NC

  wn : widNode (Caps.cWid (frameStep (suc L) c)) (Sched.slots sched) ns ≡ true
  wn = widNode-push c L (Sched.slots sched) lim q o act od hS wn₀
         (≤ᵇ-true _ _ (≤-trans hw (iterFold-infl (Caps.cSize c) hS L (Caps.cWid c))))
...
-- asserting them.
--
-- THE PARK ARM CARRIES TWO, not five: the burst is empty there and the
-- schedule is the one that came in, so the conjuncts about them are
-- `z≤n` and a hypothesis -- and the INVARIANT is not this leaf's at
-- all.  The caps walk beside it reports the same write at a level, so
-- carrying a second reading here would be the duplicate that face's
-- own header names.  Stating a leaf over what it actually owes is what
-- keeps the fit CHECKED when it lands.
...
thruStep-merge-park {n = n} {Γ = Γ} {e = e} {u = u}
  c sl B W m m′ nid lim act q od o sched st eq hm hn =
  c1 , c2
  where
...
-- ONE ARRIVAL'S SUBSCRIPTION, READ OFF THE PROVEN FACE.  The descent a
-- `thru-outer` runs is the subscribe machinery's own, and that machinery
-- has a discharged caps result over it, so what this states is the
-- proven face's conclusion with the two things this cone needs kept and
-- the rest dropped: the state pair at some later level, and the slots
-- equation the fold above re-reads its premises against.
--
-- IT IS KEYED ON THE CAPS BOOLEAN AND NOT ON THE NESTING ONE, and that
-- choice is what makes the body exist.  The face is stated over the
-- subscribed source's SYNTACTIC size; the nesting predicates read the
-- source on the sync spine and through the slot closure, both of which
-- are leaves at a defer gate the syntactic size recurses through.  So
-- the two currencies are not orderable on the source side, and the
-- statement takes the one the face takes.
--
-- THE LEVEL IS EXISTENTIAL AND ADDS ON THE LEFT, because the face
-- reports a step relative to the level it was entered at, and the fold
-- above accumulates those steps rather than joining them.
-- DEAD ROUTE: keying this on the nest currency and deriving it from the
--   proven face.  `Refuted.Nest-Size-Currency` builds a source from
--   whatever cap is offered, so the gap is on the SOURCE side and no
--   level, widening or larger cap reaches it -- the currency has to
--   change at the statement, which is what the form above does.
subscribeInner-nestCaps : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (c : Caps) (L : ℕ) (sl : Slots Γ) (fuel : Gas) (op : AllOp) (nid : NodeId)
  (κ : Path Γ u t) (id : Id) (now : Tick)
  (o : Val Γ (obs u)) (sched : Sched Γ) (st : EvalSt e) →
  Sched.slots sched ≡ sl →
  capsOK? (frameStep L c) sched st ≡ true →
  valCaps? (frameStep L c) sl (obs u) o ≡ true →
  pathSz? (Caps.cSize (frameStep L c)) κ ≡ true →
  suc (pathLen κ) ≤ Caps.cSize (frameStep L c) →
  -- THE ENTRY READING, WHICH THIS FACE ONLY FORWARDS.  The nest currency
  -- reads no floor at all, so neither half constrains anything stated
  -- here; they are the caps face's mint side condition arriving through
  -- the one call this body makes
  pathStrat? κ ≡ true →
  inputsBelowᵛ (pathFloor κ) (obs u) o ≡ true →
  -- AND THE THREE THE CAPS FACE ASKS OF AN EDGE THAT MINTS A
  -- REGISTRATION, forwarded exactly as received: the nest currency reads
  -- no floor and no counter, so none of them constrains anything stated
  -- here
...
subscribeInner-nestCaps c L sl fuel op nid κ id now o sched st
  ...
  , trans (KeepsC.slotsEq (subscribeInner-keeps fuel op nid κ id now o sched st)) hsl
  where
  Rr     = subscribeInner fuel op nid κ id now o sched st
  sched₁ = proj₁ (proj₂ (proj₂ (proj₂ (proj₂ Rr))))
  st₁    = proj₂ (proj₂ (proj₂ (proj₂ (proj₂ Rr))))
  R = subscribeInner-caps c (depthInner fuel op nid κ id now o sched st)
        (nest o sl (EvalSt.connectedShares st)) L
...
mergeAllBump-caps : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t}
  (c : Caps) (sl : Slots Γ) (nid : NodeId) (done : Bool)
  (sched : Sched Γ) (st : EvalSt e) →
  capsOK? c sched st ≡ true →
  capsOK? c sched
    (record st { nodes = mergeAllBump nid done (EvalSt.nodes st) }) ≡ true
mergeAllBump-caps c sl nid done sched st h
  with lookupNode nid (EvalSt.nodes st) in eq
... | just (mergeAll-st lim act q od) =
      capsOK?-setNode c nid
        (mergeAll-st lim (if done then act else suc act) q od) sched st
        (proj₁ NC) PK (proj₂ NC) h
        -- the bump moves the SUBSCRIPTION COUNT and leaves the queue
        -- where it was, so the reading is the cell's own
        (λ i h′ → subst (λ m → parkStrat? i m ≡ true) eq h′)
      where
      NC = subst (NodeCaps c (Sched.slots sched)) eq
             (lookupNode-caps c (Sched.slots sched) nid (EvalSt.nodes st)
                (capsOK?-nodeSz c sched st h) (capsOK?-nodeWid c sched st h))
      PK = subst (NodePark (Caps.cSize c) (slotsSize (Sched.slots sched))) eq
             (lookupNode-park (Caps.cSize c) (slotsSize (Sched.slots sched))
                nid (EvalSt.nodes st) (capsOK?-nodePark c sched st h))
... | just (scan-st v)        = h
... | just (take-st k)        = h
...
thruStep-merge-inner-caps : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (c : Caps) (L : ℕ) (sl : Slots Γ) (fuel : Gas) (nid : NodeId)
  (κ : Path Γ u t) (id : Id) (now : Tick)
  (lim : Maybe ℕ) (act : ℕ) (q : List (Val Γ (obs u))) (od : Bool)
  (o : Val Γ (obs u)) (sched : Sched Γ) (st : EvalSt e) →
  lookupNode nid (EvalSt.nodes st) ≡ just (mergeAll-st {t = u} lim act q od) →
...
thruStep-merge-inner : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  Sched.slots sched ≡ sl →
  suc m ≤ m′ →
  nestCapsOK? (frameStep L c) sched st ≡ true →
  ...
  × (nodesMax st′ ≤ nodesMax st ⊔ G′)
  × ((j : NodeId) → nodeNestAt j st′ ≤ nodeNestAt j st ⊔ G′)
thruStep-merge-inner {e = e} c L sl B W m m′ fuel nid κ id now lim act q od o sched st
  ...
  hstep : c ⊑ᶜ frameStep L c
  hstep = c⊑step c L (FaceOK.fSize faceHere)

  instance
    faceL : FaceOK (frameStep L c) sl
    faceL = face⊑ c (frameStep L c) sl hstep faceHere

  ARR = subscribeInner-nest-arr (frameStep L c) sl N W fuel mergeAllᵒ nid κ id now o sched st
          hsl hc (nestValOK?-widen _ o hstep hv)
          (≤ᵇ-true (closSizeᵉ (slotClos sl) o) (Caps.cSize (frameStep L c))
             (≤-trans hcl (proj₁ hstep))) hn hwi
...
abstract
  -- and neither the slots nor the width fold over them, so the whole
  -- invariant rides the kill at ANY cap -- which is what lets the
  -- levelled arm below read it at the level it was entered with
  switchKill-slots : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t}
    (cur : Maybe Id) (sched : Sched Γ) (st : EvalSt e) →
    Sched.slots (proj₁ (proj₂ (switchKill cur sched st))) ≡ Sched.slots sched
  switchKill-slots nothing  sched st = refl
  switchKill-slots (just v) sched st = refl

  switchKill-nestCaps : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t}
    (c : Caps) (cur : Maybe Id) (sched : Sched Γ) (st : EvalSt e) →
    nestCapsOK? c sched st ≡ true →
    nestCapsOK? c (proj₁ (proj₂ (switchKill cur sched st)))
                  (proj₂ (proj₂ (switchKill cur sched st))) ≡ true
  switchKill-nestCaps c nothing  sched st h = h
  switchKill-nestCaps c (just v) sched st h = h
...
thruStep-switch-inner-caps : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  pathStrat? κ ≡ true →
  inputsBelowᵛ (pathFloor κ) (obs u) o ≡ true →
  -- THE THREE READINGS ARE ASKED AT THE PRE-KILL STATE, which is where
  -- the caller stands; the kill rewrites the registry, the cancelled
  -- list and the live set and touches neither the node table nor the
  -- counter, so both halves transport across it by `refl` per arm
...
thruStep-switch-inner-caps c L sl fuel nid κ id now nothing od o sched st
  ...
  (proj₁ (proj₁ SUB)
   , capsOK?-setNode (frameStep (proj₁ (proj₁ SUB) + L) c) nid
       ...
       refl refl refl (proj₂ (proj₁ SUB)) (λ i h → refl))
  , proj₂ SUB
  where
  R      = subscribeInner fuel switchᵒ nid κ id now o sched st
  inst   = proj₁ R
...
thruStep-switch-inner-caps c L sl fuel nid κ id now (just v) od o sched st
  ...
  st₂    = proj₂ (proj₂ (proj₂ (proj₂ (proj₂ R))))
  SUB = subscribeInner-nestCaps c L sl fuel switchᵒ nid κ id now o sched₁ st₁
          hsl (switchKill-caps (frameStep L c) (just v) sched st hc) hv hpk hpl
          stP stO
...
thruStep-switch-inner : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  × (nodesMax st′ ≤ nodesMax st ⊔ G′)
  × ((j : NodeId) → nodeNestAt j st′ ≤ nodeNestAt j st ⊔ G′)
thruStep-switch-inner {e = e} c L sl B W m m′ fuel nid κ id now cur od o sched st
                      eq hsl hm hc hv hcl hw hn hwi =
    ≤-trans (proj₁ ARR) frame
  ...
  ARR = subscribeInner-nest-arr (frameStep L c) sl N W fuel switchᵒ nid κ id now o sched₁ st₁
          (trans (switchKill-slots cur sched st) hsl)
          (switchKill-nestCaps (frameStep L c) cur sched st hc)
          (nestValOK?-widen _ o hstep hv)
          (≤ᵇ-true (closSizeᵉ (slotClos sl) o) (Caps.cSize (frameStep L c))
             (≤-trans hcl (proj₁ hstep))) hn hwi
...
thruStep-exhaust-inner : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  × (nodesMax st′ ≤ nodesMax st ⊔ G′)
  × ((j : NodeId) → nodeNestAt j st′ ≤ nodeNestAt j st ⊔ G′)
thruStep-exhaust-inner {e = e} c L sl B W m m′ fuel nid κ id now od o sched st
                       eq hsl hm hc hv hcl hw hn hwi =
    ≤-trans (proj₁ ARR) frame
  ...
  ARR = subscribeInner-nest-arr (frameStep L c) sl N W fuel exhaustᵒ nid κ id now o sched st
          hsl hc (nestValOK?-widen _ o hstep hv)
          (≤ᵇ-true (closSizeᵉ (slotClos sl) o) (Caps.cSize (frameStep L c))
             (≤-trans hcl (proj₁ hstep))) hn hwi
...
thruConsume-caps : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  suc (pathLen κ) ≤ Caps.cSize (frameStep L c) →
  thruRoom c W fuel op nid κ id now o sched st →
  -- THE ENTRY READING, FORWARDED TO THE THREE *All ARMS.  Only they
  -- subscribe, and a mint's side condition is stated against the
  -- payload's own path and reading; the other twenty-two clauses hand
  -- back the state they were given and consume neither
  pathStrat? κ ≡ true →
  inputsBelowᵛ (pathFloor κ) (obs u) o ≡ true →
  -- AND THE TWO CHAIN READINGS PLUS THE OUTER'S OWN CELL, forwarded to
  -- the same three arms for the same reason: a mint registers the
  -- payload under this chain, so the order it is registered at and the
  -- stratification of everything already parked below it are what the
  -- registry-level conjuncts will ask of the state that comes back
  pathOrd? (Sched.nextNode sched) (thru-outer op nid ↠ κ) ≡ true →
  pathPark? κ st ≡ true →
  parkStrat? (pathFloor κ) (lookupNode nid (EvalSt.nodes st)) ≡ true →
...
...     | true =
          let C = thruStep-merge-inner-caps c L sl fuel nid κ id now
...
...     | false =
          1 , (merge-park-caps c L nid κ lim act q od o sched st
                   ...
                   (subst (λ y → slotsSize y ≤ Caps.cSize c) (sym hsl)
                      (FaceOK.fSlSz faceHere))
                   eq hc
...
thruWalk-caps : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  Sched.slots sched ≡ sl →
  capsOK? (frameStep L c) sched st ≡ true →
  all (valCaps? c sl (obs u)) os ≡ true →
  pathSz? (Caps.cSize c) κ ≡ true →
  suc (pathLen κ) ≤ Caps.cSize c →
  thruRoomOK c W fuel op nid κ id now os sched st →
  -- THE ENTRY READING, PROJECTED PER PAYLOAD.  The path half is constant
  -- along the walk -- nothing here extends κ -- and the payload half
  -- splits at each cons exactly as the caps reading beside it does
  pathStrat? κ ≡ true →
  valsStrat? (pathFloor κ) os ≡ true →
  -- AND THE TWO CHAIN READINGS PLUS THE OUTER'S OWN CELL.  All three are
  -- constant in the payload -- nothing here extends the chain -- so the
  -- walk carries them along its own state thread rather than splitting
  -- them at each cons the way the caps reading beside them is split
  pathOrd? (Sched.nextNode sched) (thru-outer op nid ↠ κ) ≡ true →
  pathPark? κ st ≡ true →
  parkStrat? (pathFloor κ) (lookupNode nid (EvalSt.nodes st)) ≡ true →
...
thruWalk-caps {u = u} c L sl W fuel op nid κ id now (o ∷ os) sched st hsl hc hv hpk hpl (hro , hros) stP stV stR stK stQ stW =
  ...
  , subst (λ z → capsOK? (frameStep z c)
                   (proj₁ (proj₂ (proj₂ rw))) (proj₂ (proj₂ (proj₂ rw))) ≡ true)
          (sym (+-assoc (proj₁ IH) (proj₁ C) L)) (proj₂ IH)
  where
  rc = thruConsume fuel op nid κ id now o sched st
  rw = thruWalk fuel op nid κ id now os
         (proj₁ (proj₂ (proj₂ rc))) (proj₂ (proj₂ (proj₂ rc)))
  2≤S = FaceOK.fSize faceHere
  stepL = c⊑step c L 2≤S
  stO = proj₁ (∧-true (inputsBelowᵛ (pathFloor κ) (obs u) o)
                      (all (inputsBelowᵛ (pathFloor κ) (obs u)) os) stV)
  stT = proj₂ (∧-true (inputsBelowᵛ (pathFloor κ) (obs u) o)
                      (all (inputsBelowᵛ (pathFloor κ) (obs u)) os) stV)
  ...
  FRκ = thruConsume-fresh (pathRead κ) fuel op nid κ id now o sched st
          (≤-trans (m≤n⊔m (suc nid) (pathRead κ))
                   (pathOrd?-read (Sched.nextNode sched)
                      (thru-outer op nid ↠ κ) stR))
          (pathOrd?-hop (Sched.nextNode sched) nid op κ stR)
  IH = thruWalk-caps c (proj₁ C + L) sl W fuel op nid κ id now os
         (proj₁ (proj₂ (proj₂ rc))) (proj₂ (proj₂ (proj₂ rc)))
         (trans (KeepsC.slotsEq (thruConsume-keeps fuel op nid κ id now o sched st)) hsl)
         (proj₂ C) (proj₂ (∧-true _ _ hv)) hpk hpl hros stP stT
         -- ONE PAYLOAD ONLY RAISES THE COUNTER, and the chain is fixed
...
thruWrap-caps : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  capsOK? c (proj₁ (proj₂ (proj₂ r))) (proj₂ (proj₂ (proj₂ r))) ≡ true →
  let w = thruWrap op nid fin r in
  capsOK? c (proj₁ (proj₂ (proj₂ (proj₂ w))))
...
thruWrap-caps c sl mergeAllᵒ nid true (vs , bs , sched′ , st′) hc
  with lookupNode nid (EvalSt.nodes st′) in eq
... | just (mergeAll-st lim act q od) =
      capsOK?-setNode c nid (mergeAll-st lim act q true) sched′ st′
        (proj₁ NC) PK (proj₂ NC) hc
        -- the wrap flips the DONE flag and nothing else, and the park
        -- reading does not look at it: the queue it reports on is the
        -- one already in the cell
        (λ i h → subst (λ m → parkStrat? i m ≡ true) eq h)
      where
      NC = subst (NodeCaps c (Sched.slots sched′)) eq
             (lookupNode-caps c (Sched.slots sched′) nid (EvalSt.nodes st′)
                (capsOK?-nodeSz c sched′ st′ hc) (capsOK?-nodeWid c sched′ st′ hc))
      PK = subst (NodePark (Caps.cSize c) (slotsSize (Sched.slots sched′))) eq
             (lookupNode-park (Caps.cSize c) (slotsSize (Sched.slots sched′))
                nid (EvalSt.nodes st′) (capsOK?-nodePark c sched′ st′ hc))
... | nothing = hc
... | just (scan-st _) = hc
... | just (take-st _) = hc
...
thruWrap-caps c sl switchᵒ nid true (vs , bs , sched′ , st′) hc
  with lookupNode nid (EvalSt.nodes st′)
... | just (switch-st cur od) =
      capsOK?-setNode c nid (switch-st cur true) sched′ st′ refl refl refl hc
        (λ i h → refl)
... | nothing = hc
... | just (scan-st _) = hc
... | just (take-st _) = hc
...
thruWrap-caps c sl exhaustᵒ nid true (vs , bs , sched′ , st′) hc
  with lookupNode nid (EvalSt.nodes st′)
... | just (exhaust-st act od) =
      capsOK?-setNode c nid (exhaust-st act true) sched′ st′ refl refl refl hc
        (λ i h → refl)
... | nothing = hc
... | just (scan-st _) = hc
... | just (take-st _) = hc
...
stepThru-caps : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  -- the wrap flips a done flag the readings never look at, so the three
  -- the walk under it asks for are the three this clause is handed
  pathOrd? (Sched.nextNode sched) (thru-outer op nid ↠ κ) ≡ true →
  pathPark? κ st ≡ true →
  parkStrat? (pathFloor κ) (lookupNode nid (EvalSt.nodes st)) ≡ true →
  regOwn? nid (pathFloor κ) (EvalSt.registry st) ≡ true →
...
thruStep-merge {u = u} c L sl B W m m′ fuel nid κ id now o sched st hsl hm hc hv hcl hr hn
  with lookupNode nid (EvalSt.nodes st) in eq
... | just (mergeAll-st {w} lim act q od) with w ≟ᵗ u
...   | no _ = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
...   | yes refl with hasRoom lim act
...     | true =
          thruStep-merge-inner c L sl B W m m′ fuel nid κ id now
            lim act q od o sched st eq hsl hm hc hv hcl
            (proj₁ hr) hn (proj₁ (proj₂ hr))
...     | false =
          let P = thruStep-merge-park c sl B W m m′ nid
                    lim act q od o sched st eq hm hn
          in z≤n , proj₁ P , proj₂ P
thruStep-merge c L sl B W m m′ fuel nid κ id now o sched st hsl hm hc hv hcl hr hn
    | nothing               = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
thruStep-merge c L sl B W m m′ fuel nid κ id now o sched st hsl hm hc hv hcl hr hn
    | just (scan-st _)      = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
thruStep-merge c L sl B W m m′ fuel nid κ id now o sched st hsl hm hc hv hcl hr hn
    | just (take-st _)      = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
thruStep-merge c L sl B W m m′ fuel nid κ id now o sched st hsl hm hc hv hcl hr hn
    | just (switch-st _ _)  = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
thruStep-merge c L sl B W m m′ fuel nid κ id now o sched st hsl hm hc hv hcl hr hn
    | just (exhaust-st _ _) = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
...
thruStep-switch : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  × ((j : NodeId) →
       nodeNestAt j (proj₂ (proj₂ (proj₂ rc))) ≤ nodeNestAt j st ⊔ G′)
thruStep-switch {u = u} c L sl B W m m′ fuel nid κ id now o sched st hsl hm hc hv hcl hr hn
  with lookupNode nid (EvalSt.nodes st) in eq
... | just (switch-st cur od) =
      thruStep-switch-inner c L sl B W m m′ fuel nid κ id now cur od
        o sched st eq hsl hm hc hv hcl (proj₁ hr) hn
        (proj₂ (proj₂ hr) cur od refl)
... | nothing               = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
... | just (mergeAll-st _ _ _ _) = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
... | just (scan-st _)      = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
... | just (take-st _)      = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
... | just (exhaust-st _ _) = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
...
thruStep-exhaust : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  × ((j : NodeId) →
       nodeNestAt j (proj₂ (proj₂ (proj₂ rc))) ≤ nodeNestAt j st ⊔ G′)
thruStep-exhaust {u = u} c L sl B W m m′ fuel nid κ id now o sched st hsl hm hc hv hcl hr hn
  with lookupNode nid (EvalSt.nodes st) in eq
... | just (exhaust-st false od) =
      thruStep-exhaust-inner c L sl B W m m′ fuel nid κ id now od
        o sched st eq hsl hm hc hv hcl (proj₁ hr) hn
        (proj₁ (proj₂ hr))
... | just (exhaust-st true od)  = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
... | nothing               = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
... | just (mergeAll-st _ _ _ _) = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
... | just (scan-st _)      = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
... | just (take-st _)      = z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
...
thruFit-vals : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (c : Caps) (L : ℕ) (sl : Slots Γ) (B W m m′ : ℕ) (fuel : Gas) (op : AllOp) (nid : NodeId)
  (κ : Path Γ u t) (id : Id) (now : Tick) (os : List (Val Γ (obs u)))
  (sched : Sched Γ) (st : EvalSt e) →
  Sched.slots sched ≡ sl →
  suc m ≤ m′ →
  ...
  pathStrat? κ ≡ true →
  valsStrat? (pathFloor κ) os ≡ true →
  -- AND THE TWO CHAIN READINGS PLUS THE OUTER'S OWN CELL, carried along
  -- the walk's own state thread.  All three are constant in the payload,
  -- so unlike the caps reading beside them they are not split at each
  -- cons -- what they need instead is a transport across the ONE consume
  -- the cons performs, which is where the counter rises and where the
  -- outer's cell may be written
  pathOrd? (Sched.nextNode sched) (thru-outer op nid ↠ κ) ≡ true →
  pathPark? κ st ≡ true →
  parkStrat? (pathFloor κ) (lookupNode nid (EvalSt.nodes st)) ≡ true →
...
thruFit-vals {u = u} c L sl B W m m′ fuel mergeAllᵒ nid κ id now (o ∷ os) sched st hsl hm hc hv hcl hva hpk hpl hr hn stP stV stR stK stQ stW =
  ...
  , thruFit-vals c (proj₁ K + L) sl B W m m′ fuel mergeAllᵒ nid κ id now os
      ...
      (proj₂ K) (proj₂ (∧-true _ _ hv))
      (proj₂ (∧-true _ _ hcl)) (proj₂ (∧-true _ _ hva)) hpk hpl (proj₂ hr)
      (≤-trans (m≤n⊔m (nestDᵛ (obs u) o) (nestDᵛˢ os)) hn) stP stT
      -- ONE PAYLOAD ONLY RAISES THE COUNTER, and the chain is fixed
      (pathOrd?-mono (Sched.nextNode sched) (Sched.nextNode sd₁)
  ...
  sd₁ = proj₁ (proj₂ (proj₂ rc))
  st₁ = proj₂ (proj₂ (proj₂ rc))
  FR0 = thruConsume-fresh 0 fuel mergeAllᵒ nid κ id now o sched st z≤n z≤n
  FRκ = thruConsume-fresh (pathRead κ) fuel mergeAllᵒ nid κ id now o sched st
          (≤-trans (m≤n⊔m (suc nid) (pathRead κ))
                   (pathOrd?-read (Sched.nextNode sched)
                      (thru-outer mergeAllᵒ nid ↠ κ) stR))
          (pathOrd?-hop (Sched.nextNode sched) nid mergeAllᵒ κ stR)
  2≤S = FaceOK.fSize faceHere
  stepL = c⊑step c L 2≤S
  stO = proj₁ (∧-true (inputsBelowᵛ (pathFloor κ) (obs u) o)
                      (all (inputsBelowᵛ (pathFloor κ) (obs u)) os) stV)
...
thruFit-vals {u = u} c L sl B W m m′ fuel switchᵒ nid κ id now (o ∷ os) sched st hsl hm hc hv hcl hva hpk hpl hr hn stP stV stR stK stQ stW =
  ...
  sd₁ = proj₁ (proj₂ (proj₂ rc))
  st₁ = proj₂ (proj₂ (proj₂ rc))
  FR0 = thruConsume-fresh 0 fuel switchᵒ nid κ id now o sched st z≤n z≤n
  FRκ = thruConsume-fresh (pathRead κ) fuel switchᵒ nid κ id now o sched st
          (≤-trans (m≤n⊔m (suc nid) (pathRead κ))
                   (pathOrd?-read (Sched.nextNode sched)
                      (thru-outer switchᵒ nid ↠ κ) stR))
          (pathOrd?-hop (Sched.nextNode sched) nid switchᵒ κ stR)
  2≤S = FaceOK.fSize faceHere
  stepL = c⊑step c L 2≤S
  stO = proj₁ (∧-true (inputsBelowᵛ (pathFloor κ) (obs u) o)
                      (all (inputsBelowᵛ (pathFloor κ) (obs u)) os) stV)
...
thruFit-vals {u = u} c L sl B W m m′ fuel exhaustᵒ nid κ id now (o ∷ os) sched st hsl hm hc hv hcl hva hpk hpl hr hn stP stV stR stK stQ stW =
  ...
  sd₁ = proj₁ (proj₂ (proj₂ rc))
  st₁ = proj₂ (proj₂ (proj₂ rc))
  FR0 = thruConsume-fresh 0 fuel exhaustᵒ nid κ id now o sched st z≤n z≤n
  FRκ = thruConsume-fresh (pathRead κ) fuel exhaustᵒ nid κ id now o sched st
          (≤-trans (m≤n⊔m (suc nid) (pathRead κ))
                   (pathOrd?-read (Sched.nextNode sched)
                      (thru-outer exhaustᵒ nid ↠ κ) stR))
          (pathOrd?-hop (Sched.nextNode sched) nid exhaustᵒ κ stR)
  2≤S = FaceOK.fSize faceHere
  stepL = c⊑step c L 2≤S
  stO = proj₁ (∧-true (inputsBelowᵛ (pathFloor κ) (obs u) o)
                      (all (inputsBelowᵛ (pathFloor κ) (obs u)) os) stV)
...
-- ALL OF IT.  Two of the record's three conjuncts are the burst's and
-- the last is the arriving value's own frame width -- which the caps
-- premise already carries, `pWᵛ` at an observable type BEING `pWᵉ`.  So
-- the frame's job is not to derive the record but to carry it along the
-- walk: each consume runs at the state the previous one left, and the
...
thruRoom-frame : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  capsOK? (frameStep L c) sched st ≡ true →
  all (valCaps? c sl (obs u)) vals ≡ true →
  pathSz? (Caps.cSize c) p ≡ true →
  suc (pathLen p) ≤ Caps.cSize c →
...
thruRoom-frame {n = n} {u = u} c L W sl sf id now op nid p (o ∷ os) sched st
  ...
  , thruRoom-frame c (proj₁ K + L) W sl sf id now op nid p os
      (proj₁ (proj₂ (proj₂ rc))) (proj₂ (proj₂ (proj₂ rc)))
      (trans (KeepsC.slotsEq
                (thruConsume-keeps sf op nid p id now o sched st)) hsl) (proj₂ K)
  ...
  sd₁ = proj₁ (proj₂ (proj₂ rc))
  st₁ = proj₂ (proj₂ (proj₂ rc))
  FR0 = thruConsume-fresh 0 sf op nid p id now o sched st z≤n z≤n
  FRκ = thruConsume-fresh (pathRead p) sf op nid p id now o sched st
          (≤-trans (m≤n⊔m (suc nid) (pathRead p))
                   (pathOrd?-read (Sched.nextNode sched)
                      (thru-outer op nid ↠ p) stR))
          (pathOrd?-hop (Sched.nextNode sched) nid op p stR)
  2≤S = FaceOK.fSize faceHere
  stepL = c⊑step c L 2≤S
  stO = proj₁ (∧-true (inputsBelowᵛ (pathFloor p) (obs u) o)
                      (all (inputsBelowᵛ (pathFloor p) (obs u)) os) stV)
  stT = proj₂ (∧-true (inputsBelowᵛ (pathFloor p) (obs u) o)
                      (all (inputsBelowᵛ (pathFloor p) (obs u)) os) stV)
...
thruFit-frame : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  pathStrat? p ≡ true →
  valsStrat? (pathFloor p) vals ≡ true →
  -- AND THE THREE THE WALK UNDER THIS FRAME CARRIES, forwarded twice:
  -- the room walk and the fit walk make the same consumes in the same
  -- order, so each is entered at the state this frame was handed
  pathOrd? (Sched.nextNode sched) (thru-outer op nid ↠ p) ≡ true →
  pathPark? p st ≡ true →
  parkStrat? (pathFloor p) (lookupNode nid (EvalSt.nodes st)) ≡ true →
...
thruFit-frame {e = e} c W sl sf id now op nid p vals sched st
  ...
  thruFitOK-mono
    ...
    (thruFit-vals c 0 sl (nestDᵛˢ vals) W 0 (Caps.cSize c) sf op nid p id now
       vals sched st hsl h1S cap0
...
stepFrame-nodes-thru : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  pathStrat? p ≡ true →
  valsStrat? (pathFloor p) vals ≡ true →
  -- AND THE THREE THE FIT UNDER THIS HEAD CARRIES.  The wrap that
  -- follows the walk reads none of them -- it flips a done flag -- so
  -- they are forwarded to the fit and nowhere else
  pathOrd? (Sched.nextNode sched) (thru-outer op nid ↠ p) ≡ true →
  pathPark? p st ≡ true →
  parkStrat? (pathFloor p) (lookupNode nid (EvalSt.nodes st)) ≡ true →
...
pushValsOK : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (c : Caps) (L : ℕ) (sl : Slots Γ) (B W m : ℕ) (fuel : Gas) (op : AllOp) (nid : NodeId)
  (κ : Path Γ u t) (id : Id) (now : Tick) (str : Stream Γ (obs u))
  (sched : Sched Γ) (st : EvalSt e) → Set
pushValsOK c L sl B W m fuel op nid κ id now [] sched st = ⊤
pushValsOK {Γ = Γ} {e = e} {u = u} c L sl B W m fuel op nid κ id now (em ∷ ems) sched st =
  (Sched.slots sched ≡ sl)
  × (capsOK? (frameStep L c) sched st ≡ true)
  ...
  × (Σ ℕ λ L′ → pushValsOK c (L′ + L) sl B W m fuel op nid κ id now ems
      (proj₁ (proj₂ (proj₂ (proj₂ sf)))) (proj₂ (proj₂ (proj₂ (proj₂ sf)))))
  where
  sp = splitEvents {A = Val Γ u} (InstEmit.events em)
  sf = stepFrame fuel id now (thru-outer op nid) κ (proj₁ sp)
...
pushValsCapsOK : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  (κ : Path Γ u t) (id : Id) (now : Tick) (str : Stream Γ (obs u))
  (sched : Sched Γ) (st : EvalSt e) → Set
pushValsCapsOK c L sl W fuel op nid κ id now [] sched st = ⊤
pushValsCapsOK {Γ = Γ} {u = u} c L sl W fuel op nid κ id now (em ∷ ems) sched st =
  (Sched.slots sched ≡ sl)
  × (capsOK? (frameStep L c) sched st ≡ true)
  × (all (nestValOK? c (obs u)) (proj₁ sp) ≡ true)
  × (all (nestClosOK? c sl) (proj₁ sp) ≡ true)
  × (all (valCaps? c sl (obs u)) (proj₁ sp) ≡ true)
  × thruRoomOK c W fuel op nid κ id now (proj₁ sp) sched st
  × (Σ ℕ λ L′ → pushValsCapsOK c (L′ + L) sl W fuel op nid κ id now ems
      (proj₁ (proj₂ (proj₂ (proj₂ sf)))) (proj₂ (proj₂ (proj₂ (proj₂ sf)))))
  where
  sp = splitEvents {A = Val Γ u} (InstEmit.events em)
  sf = stepFrame fuel id now (thru-outer op nid) κ (proj₁ sp)
...
pushVals-both : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  (κ : Path Γ u t) (id : Id) (now : Tick) (str : Stream Γ (obs u))
  (sched : Sched Γ) (st : EvalSt e) →
  pushValsCapsOK c L sl W fuel op nid κ id now str sched st →
  pushValsNestOK c sl B W m fuel op nid κ id now str sched st →
  pushValsOK c L sl B W m fuel op nid κ id now str sched st
pushVals-both c L sl B W m fuel op nid κ id now [] sched st hcap hnest = tt
pushVals-both {Γ = Γ} {u = u} c L sl B W m fuel op nid κ id now (em ∷ ems) sched st
              (hsl , hc , hv , hcl , hva , hr , restC) (hn , restN) =
  hsl , hc , hv , hcl , hva , hr , hn
  , (proj₁ restC
     , pushVals-both c (proj₁ restC + L) sl B W m fuel op nid κ id now ems
         (proj₁ (proj₂ (proj₂ (proj₂ sf)))) (proj₂ (proj₂ (proj₂ (proj₂ sf))))
         (proj₂ restC) restN)
  where
  sp = splitEvents {A = Val Γ u} (InstEmit.events em)
...
pushValsStOK : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  (κ : Path Γ u t) (id : Id) (now : Tick) (str : Stream Γ (obs u))
  (sched : Sched Γ) (st : EvalSt e) → Set
pushValsStOK c L sl fuel op nid κ id now [] sched st = ⊤
pushValsStOK {Γ = Γ} {u = u} c L sl fuel op nid κ id now (em ∷ ems) sched st =
  (Sched.slots sched ≡ sl)
  × (capsOK? (frameStep L c) sched st ≡ true)
  × (Σ ℕ λ L′ → pushValsStOK c (L′ + L) sl fuel op nid κ id now ems
      (proj₁ (proj₂ (proj₂ (proj₂ sf)))) (proj₂ (proj₂ (proj₂ (proj₂ sf)))))
  where
  sp = splitEvents {A = Val Γ u} (InstEmit.events em)
  sf = stepFrame fuel id now (thru-outer op nid) κ (proj₁ sp)
...
pushVals-caps-join : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  pushValsCapsOK c L sl W fuel op nid κ id now str sched st
pushVals-caps-join c L sl W fuel op nid κ id now [] sched st hst hadm hbc hroom = tt
pushVals-caps-join {Γ = Γ} {u = u} c L sl W fuel op nid κ id now (em ∷ ems) sched st
    (hsl , hc , restSt) (hv , hcl , restAdm) hbc (hr , restRoom) =
  hsl , hc , hv , hcl
  , splitEvents-vals-caps {u = u} c sl (InstEmit.events em) (proj₁ (∧-true _ _ hbc))
  , hr
  , (proj₁ restSt
     , pushVals-caps-join c (proj₁ restSt + L) sl W fuel op nid κ id now ems _ _
         (proj₂ restSt) restAdm (proj₂ (∧-true _ _ hbc)) restRoom)
...
pushValsSt-walk : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  suc (pathLen κ) ≤ Caps.cSize c →
  pushValsRoomOK c sl W fuel op nid κ id now str sched st →
  -- THE ENTRY READING OVER THE WHOLE BURST, which is the shape the
  -- split needs: an instant's payload is the values its EVENTS carry,
  -- so the per-instant reading is `splitEvents-valsStrat` of this one
  pathStrat? κ ≡ true →
  burstStrat? (pathFloor κ) str ≡ true →
  -- AND THE THREE THE PER-INSTANT STEP CONSUMES, carried along the state
  -- thread rather than split with the burst: the frame is the SAME one
  -- at every emit, so the chain never changes and only the state under
  -- it does
  pathOrd? (Sched.nextNode sched) (thru-outer op nid ↠ κ) ≡ true →
  pathPark? κ st ≡ true →
  parkStrat? (pathFloor κ) (lookupNode nid (EvalSt.nodes st)) ≡ true →
  ...
  , (proj₁ S
     , pushValsSt-walk c (proj₁ S + L) sl W fuel op nid κ id now ems
         ...
         (trans (KeepsC.slotsEq (stepFrame-keeps fuel id now (thru-outer op nid) κ (proj₁ sp)
                                (proj₂ (proj₂ sp)) sched st)) hsl)
         (proj₂ S) restAdm (proj₂ (∧-true _ _ hbc)) hpk hpl restRoom
         stP (proj₂ (∧-true _ _ stB))
         (pathOrd?-mono (Sched.nextNode sched)
            (Sched.nextNode (proj₁ (proj₂ (proj₂ (proj₂ sf)))))
            (thru-outer op nid ↠ κ)
            (stepFrame-nextNode fuel id now (thru-outer op nid) κ (proj₁ sp)
               (proj₂ (proj₂ sp)) sched st)
            stR)
...
pushVals-room-join {Γ = Γ} {u = u} c L sl W fuel op nid κ id now (em ∷ ems) sched st
  ...
  , pushVals-room-join c (proj₁ S + L) sl W fuel op nid κ id now ems
      ...
      (trans (KeepsC.slotsEq (stepFrame-keeps fuel id now (thru-outer op nid) κ (proj₁ sp)
                             (proj₂ (proj₂ sp)) sched st)) hsl)
      (proj₂ S) restAdm (proj₂ (∧-true _ _ hbc)) hpk hpl restWid restW
      stP (proj₂ (∧-true _ _ stB))
      (pathOrd?-mono (Sched.nextNode sched)
         (Sched.nextNode (proj₁ (proj₂ (proj₂ (proj₂ sf)))))
         (thru-outer op nid ↠ κ)
  ...
  sf = stepFrame fuel id now (thru-outer op nid) κ (proj₁ sp)
         (proj₂ (proj₂ sp)) sched st
  hva = splitEvents-vals-caps {u = u} c sl (InstEmit.events em)
          (proj₁ (∧-true _ _ hbc))
  stV = splitEvents-valsStrat {u = u} (pathFloor κ) (InstEmit.events em)
          (proj₁ (∧-true _ _ stB))
  FP = framePark-step fuel id now (thru-outer op nid) κ (proj₁ sp)
...
pushFit-ems : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  pathStrat? κ ≡ true →
  burstStrat? (pathFloor κ) str ≡ true →
  -- AND THE THREE THE PER-INSTANT FIT CONSUMES, on the same state thread
  -- the state walk beside this one advances: the chain is fixed at every
  -- emit and only the state under it moves
  pathOrd? (Sched.nextNode sched) (thru-outer op nid ↠ κ) ≡ true →
  pathPark? κ st ≡ true →
  parkStrat? (pathFloor κ) (lookupNode nid (EvalSt.nodes st)) ≡ true →
...
NestAt {n = n} {Γ = Γ} {t = t} {e = e} c d sl B W Lv g o κ id now sched st =
  Sched.slots sched ≡ sl → capsOK? (frameStep Lv c) sched st ≡ true →
  nestValOK? (frameStep Lv c) (obs _) o ≡ true →
  -- THE CAP DOMINATES THE ARRIVAL'S CLOSURE, not merely its written
  ...
  pathSz? (Caps.cSize (frameStep Lv c)) κ ≡ true →
  suc (pathLen κ) ≤ Caps.cSize (frameStep Lv c) →
  -- AND THE ENTRY READING TRAVELS WITH THEM, FOR THE SAME REASON.  The
  -- nest currency reads no floor, so neither half constrains the grant
  -- below; what asks for them is the *All arms' descent into the caps
  -- face, whose mint carries a side condition on the payload's own path.
  -- A walk cannot produce them either -- the floor is the arrival's, not
  -- the walk's -- so they are carried clause for clause exactly as the
  -- four keys above are, and a frame the descent adds passes both
  -- unchanged: `frameStrat?` admits a `thru-outer` and `pathFloor`
  -- ignores every frame.
  pathStrat? κ ≡ true →
  inputsBelowᵉ (pathFloor κ) o ≡ true →
  -- AND THE TWO REGISTRY-LEVEL READINGS BESIDE THEM, FOR EXACTLY THE
  -- REASON THE BLOCK ABOVE GIVES ABOUT THE OTHER TWO.  Both are premises
  -- of the caps face this walk descends into at its `*All` arms, both
  -- are refuted as derivable from the receipt the call site holds --
  -- the two hand-built states satisfy `capsOK?` outright while failing
  -- the reading -- and neither is producible here, since a walk sees
  -- neither the registry's growth site nor the cell a mint will write.
  -- So they are carried clause for clause, and a frame the descent adds
  -- passes both by the same two facts: `pathOrd?` grows only at a cell
  -- the mint has just allocated, and `pathPark?` reads the table the
  -- mint installs into.
  ...
  -- AND THE LEVEL REPORTED DOMINATES THE ONE THE PREMISES WERE READ
  -- AT, which is what makes the report readable at all.  `G` is keyed
  -- on the reported level, and the arrival premises are keyed on the
  -- entry one; without this the two are incomparable and a caller
  -- holding a receipt at the entry level cannot spend it against the
  -- grant.  Every arm supplies it for free -- it reports the entry
  -- level itself or a join containing it.
  × (Lv ≤ j)
  × (nestDᵛˢ (proj₁ (splitBurst {A = Val Γ t} (proj₁ r))) ≤ G)
  × (nodesMax (proj₂ (proj₂ r)) ≤ nodesMax st ⊔ G)
...
-- admissibility and closure premises narrow to the body's -- each wrap
-- costs one `suc` in both measures -- and the body's descent under the
-- installed frame is inside the wrap's.  The two node keys are what a
-- burst leaf keyed on that invariant asks for at the install: the
-- fresh queue is empty and the other two heads carry no payload, so
-- both are `refl` per op
allFresh-widN : ∀ {n} {Γ : Ctx n} (Wd : ℕ) (sl : Slots Γ)
  (u : Ty) (op : AllOp) (lim : Maybe ℕ) →
  widNode Wd sl (allFresh u op lim) ≡ true
allFresh-widN Wd sl u mergeAllᵒ lim = refl
allFresh-widN Wd sl u switchᵒ   _   = refl
allFresh-widN Wd sl u exhaustᵒ  _   = refl

allFresh-bnd : ∀ {n} {Γ : Ctx n} (B : ℕ)
  (u : Ty) (op : AllOp) (lim : Maybe ℕ) →
  boundedNode {Γ = Γ} B (allFresh u op lim) ≡ true
allFresh-bnd B u mergeAllᵒ lim = refl
allFresh-bnd B u switchᵒ   _   = refl
allFresh-bnd B u exhaustᵒ  _   = refl

allFresh-park : ∀ {n} {Γ : Ctx n} (B Z : ℕ)
  (u : Ty) (op : AllOp) (lim : Maybe ℕ) →
  parkRoom {Γ = Γ} B Z (allFresh u op lim) ≡ true
allFresh-park B Z u mergeAllᵒ lim = refl
allFresh-park B Z u switchᵒ   _   = refl
allFresh-park B Z u exhaustᵒ  _   = refl

-- AND THE SAME CELL AT EVERY FLOOR.  The three shapes a mint installs
-- hold no payload at all -- an empty queue and two flags -- so the park
-- reading passes them without looking at the floor, which is what lets
-- a mint discharge its own cell obligation with no fact about the
-- chain it is being installed under
allFresh-parkStrat : ∀ {n} {Γ : Ctx n} (k : ℕ)
  (u : Ty) (op : AllOp) (lim : Maybe ℕ) →
  parkStrat? {Γ = Γ} k (just (allFresh u op lim)) ≡ true
allFresh-parkStrat k u mergeAllᵒ lim = refl
allFresh-parkStrat k u switchᵒ   _   = refl
allFresh-parkStrat k u exhaustᵒ  _   = refl

-- AND THE THREE READINGS A MINT-AND-SUBSCRIBE HANDS BACK, which is the
-- one composite every `*All` head performs and which the install and
-- subscribe transports cannot be applied to directly: the chain the
-- walk below reads names the cell the mint has only now allocated, so
-- neither statement is at the counter the caller holds.  Stating the
-- composite once is what keeps each head's call a line -- the head
-- chooses the op and the payload and nothing else varies -- and it is
-- what makes the ORDER reading a construction rather than a premise,
-- since the frame being pushed sits at exactly the cell just handed out.
mintSub-ord : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  pathOrd?
    ...
    (thru-outer op (proj₁ (mintNode sched)) ↠ κ) ≡ true
mintSub-ord {u = u} g op lim b κ id now sched st stO =
  subscribeE-ord g b (thru-outer op (Sched.nextNode sched) ↠ κ)
    (thru-outer op (Sched.nextNode sched) ↠ κ) id now
    (proj₂ (mintNode sched))
    (installNode (Sched.nextNode sched) (allFresh u op lim) st)
    (pathOrd?-push (suc (Sched.nextNode sched))
       (thru-outer op (Sched.nextNode sched)) κ ≤-refl
       (T⇒≡true _ (≤⇒≤ᵇ (pathOrd?-read (Sched.nextNode sched) κ stO)))
       (pathOrd?-mono (Sched.nextNode sched) (suc (Sched.nextNode sched)) κ
          (n≤1+n (Sched.nextNode sched)) stO))

mintSub-pathPark : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (g : Gas) (op : AllOp) (lim : Maybe ℕ) (b : Closed Γ (obs u))
  (κ : Path Γ u t) (id : Id) (now : Tick) (sched : Sched Γ) (st : EvalSt e) →
  pathOrd? (Sched.nextNode sched) κ ≡ true →
  pathPark? κ st ≡ true →
  pathPark? κ
    (proj₂ (proj₂ (subscribeE g b (thru-outer op (proj₁ (mintNode sched)) ↠ κ)
                     ...
                     (installNode (proj₁ (mintNode sched)) (allFresh u op lim) st))))
    ≡ true
mintSub-pathPark {u = u} g op lim b κ id now sched st stO stK =
  subscribeE-pathPark g b (thru-outer op (Sched.nextNode sched) ↠ κ) κ id now
    (proj₂ (mintNode sched))
    (installNode (Sched.nextNode sched) (allFresh u op lim) st)
...
allWrap-size : ∀ {n} {Γ : Ctx n} {u} (op : AllOp) (lim : Maybe ℕ)
  (b : Closed Γ (obs u)) → sizeᵉ b ≤ sizeᵉ (allWrap op lim b)
allWrap-size mergeAllᵒ lim b = n≤1+n _
allWrap-size switchᵒ   _   b = n≤1+n _
allWrap-size exhaustᵒ  _   b = n≤1+n _
...
subscribeE-burst-capsL c L sl g o κ id now sched st ⦃ f ⦄ hsl hc hsz hdw hpk hpl stP stB stR stK =
  ...
  R = subscribeE-caps c (depthE g o κ id now sched st)
        ...
        hc hsz hdw hpk hpl ≤-refl ≤-refl ≤-refl stP stB stR stK
...
postulate
  ...
  --   answers at, against an index the program's count fixes.  That is
  --   what this statement's INCREMENT form exists to avoid: nothing
  --   here is asked at a stepped cap's own size, so the level only
  --   ever adds off the one entry base and the arrival family is
  --   never re-entered a level up.
  -- DEAD ROUTE: threading the level down through the `pushVals-*` and
  --   `thruFit-*` shelf, so the drain's queue conjuncts could be read
  --   at a stepped cap.  A burst reading is priced as ONE step of
  --   whatever cap its inputs were read at, so levelling the inputs
  --   makes that side a step OF A STEP while every ceiling reported
  --   here is a step of the FIXED entry cap;
  --   `Refuted.Frame-Step-Compose.frameStep-compose-absurd` is the
  --   witness that no flat step bounds it.  The arms escape this only
  --   by raising the level of the SAME base, which is the form this
  --   statement now reports in.
  -- DEAD ROUTE: freezing the arrivals' WIDTH at the entry field, and
  --   stepping that frozen width by one level or by any constant.
  --   `Refuted.Subscribe-Burst-Width` kills the freeze -- a head that
  --   parks its inners hands back a payload the field cannot admit --
  --   and a walk that steps the cap steps it again at every arrival,
  --   since the thru chain subscribes once per arrival at a cap it
  --   threads unchanged across the fold, leaving a per-arrival
  --   RECURRENCE no constant is chosen around.
  ...
  -- PROBED: `Probed.Burst-Nest-Ladder` measures the increment this
  --   statement reports, at the substitution depth that is where a
  --   level's growth rate is tested at all: a ladder whose every rung
  --   substitutes the rung below it whole.  The increment is ONE at
  --   every depth -- entry size 28, 39, 50 against largest emitted 39,
  --   61, 83 -- and the control, the same shape with a step function
  --   naming its payload once, is admitted at the entry cap and needs
  --   ZERO.  So the increment is FLAT in the depth, which is what says
  --   a report off the one entry base is the right denomination.  NOT
  --   covered: the sweep
  ...
  subscribeE-burst-nestL : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
    (c : Caps) (L : ℕ) (sl : Slots Γ) (W : ℕ) (g : Gas) (o : Closed Γ u)
    (κ : Path Γ u t) (id : Id) (now : Tick) (sched : Sched Γ) (st : EvalSt e) →
    ⦃ _ : FaceOK c sl ⦄ →
    Sched.slots sched ≡ sl →
    capsOK? (frameStep L c) sched st ≡ true →
    nestValOK? (frameStep L c) (obs u) o ≡ true →
    nestClosOK? (frameStep L c) sl o ≡ true →
    sizeᵉ o ≤ Caps.cSize (frameStep L c) →
    dWᵉ n sl o ≤ Caps.cWid (frameStep L c) →
    pathSz? (Caps.cSize (frameStep L c)) κ ≡ true →
    suc (pathLen κ) ≤ Caps.cSize (frameStep L c) →
  ...
  -- PROBED: `Probed.PushVals-Caps` inhabits this leaf and the exit
  --   pair, and the admissibility beside them, at every constructor of
  --   the op axis -- which the rows therefore cover in FULL -- at the
  --   cap each value's own sync size gives, at two nesting depths and
  --   at limits 0 and 1, with each head's written and CLOSURE premises
  --   pinned by `refl` rather than assumed.  The RECURSION is covered
  --   at all three heads: the one route to a subscribe-frame burst
  --   longer than one instant is a share CONNECT, whose head instant
  --   rides in front of the def's plumbed burst, and the share-headed
  --   rows pin that burst at length TWO and inhabit the second instant
  --   at the state the first instant's step leaves.  NOT covered, a
  --   reading the rows make rather than a gap they leave: the invariant
  --   conjunct's discrimination, since the state the descent leaves
  --   carries one node and an empty registry and live set, so it reads
  --   true even at a cap granting nothing.
...
subscribeE-burst-capsAt {n = n} c d sl Lv g o κ id now sched st ⦃ f ⦄
                        hsl hc hsz hdw hpk hpl stP stB stR stK hd hceil =
  Lv + proj₁ R
  , hceil (proj₁ R)
      (≤-trans (proj₁ (proj₂ R))
...
-- AND THE NESTING HALF CONVERTS THE SAME WAY, off the same carried
-- ceiling.  The arrivals used to answer at the entry cap stepped that
-- cap's own SIZE many times, which is a denomination no level can be
-- entered at; reporting an increment instead puts every level on the
-- one entry base, where the ceiling already knows how to spend them.
subscribeE-burst-nestAt : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (c : Caps) (d : ℕ) (sl : Slots Γ) (Lv W : ℕ) (g : Gas) (o : Closed Γ u)
  (κ : Path Γ u t) (id : Id) (now : Tick) (sched : Sched Γ) (st : EvalSt e) →
  ⦃ _ : FaceOK c sl ⦄ →
  Sched.slots sched ≡ sl →
  capsOK? (frameStep Lv c) sched st ≡ true →
  nestValOK? (frameStep Lv c) (obs u) o ≡ true →
  nestClosOK? (frameStep Lv c) sl o ≡ true →
  sizeᵉ o ≤ Caps.cSize (frameStep Lv c) →
  dWᵉ n sl o ≤ Caps.cWid (frameStep Lv c) →
  pathSz? (Caps.cSize (frameStep Lv c)) κ ≡ true →
  ...
  Σ ℕ λ L →
    (L ≤ sizeCount c d ⊔ Caps.cSize c)
    × (burstNest? (frameStep L c) sl
         (proj₁ (subscribeE g o κ id now sched st)) ≡ true)
subscribeE-burst-nestAt c d sl Lv W g o κ id now sched st ⦃ f ⦄
                        hsl hc hv hcl hsz hdw hpk hpl hw hd hceil =
  Lv + proj₁ R
  , hceil (proj₁ R)
      (≤-trans (proj₁ (proj₂ R))
        (opIterD-mono (suc (sizeᵉ o)) (suc (sizeᵉ o))
  ...
  , proj₂ (proj₂ R)
  where
  R = subscribeE-burst-nestL c Lv sl W g o κ id now sched st
        hsl hc hv hcl hsz hdw hpk hpl hw
...
pushVals-caps-wid : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (c : Caps) (j Lv : ℕ) (sl : Slots Γ) (W : ℕ) (g : Gas)
  (op : AllOp) (lim : Maybe ℕ) (b : Closed Γ (obs u))
  (κ : Path Γ u t) (id : Id) (now : Tick) (sched : Sched Γ) (st : EvalSt e) →
  Caps.cSize c ≤ j → Lv ≤ j →
  ⦃ _ : FaceOK c sl ⦄ →
  Sched.slots sched ≡ sl → capsOK? (frameStep Lv c) sched st ≡ true →
  nestValOK? (frameStep Lv c) (obs u) (allWrap op lim b) ≡ true →
  nestClosOK? (frameStep Lv c) sl (allWrap op lim b) ≡ true →
  descW g (allWrap op lim b) κ id now sched st ≤ W →
...
pushVals-caps-wid {u = u} c j Lv sl W g op lim b κ id now sched st hj hLv hsl hc hv hcl hw hbc =
...
-- DEAD ROUTE: DERIVING the face's size premise from the other premises
--   here, which is why that premise is threaded rather than produced.
--   The face takes `sizeᵉ` of the subscribed source against the cap;
...
pushVals-caps-exit : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  nestClosOK? c sl (allWrap op lim b) ≡ true →
  descW g (allWrap op lim b) κ id now sched st ≤ W →
  sizeᵉ (allWrap op lim b) ≤ Caps.cSize c →
  pathSz? (Caps.cSize c) κ ≡ true →
  suc (pathLen κ) ≤ Caps.cSize c →
  -- THE MINT'S SIDE CONDITION, STATED AT THE PARENT'S PATH.  The frame
  -- this body pushes is a `thru-outer`, which `frameStrat?` passes and
  -- `pathFloor` ignores, so the extended path's two readings ARE these
  -- two and the descent takes them unchanged
  pathStrat? κ ≡ true →
  inputsBelowᵉ (pathFloor κ) b ≡ true →
  -- AND THE TWO REGISTRY READINGS, WHICH THE MINT SPENDS RATHER THAN
  -- FORWARDS.  The frame this body pushes names the cell the mint just
  -- allocated, so the extended chain's order reading is the parent's
  -- pushed at a cell the counter has only now handed out, and its park
  -- reading is the parent's beside the cell the install writes -- both
  -- built here, neither asked of the caller at the extended chain
  pathOrd? (Sched.nextNode sched) κ ≡ true →
  pathPark? κ st ≡ true →
  ⦃ _ : FaceOK c sl ⦄ →
  let res = subscribeE g b (thru-outer op (proj₁ (mintNode sched)) ↠ κ)
          ...
          (installNode (proj₁ (mintNode sched)) (allFresh u op lim) st)
  in Σ ℕ λ L₀ →
     capsOK? (frameStep L₀ c) (proj₁ (proj₂ res)) (proj₂ (proj₂ res)) ≡ true
pushVals-caps-exit {n = n} {u = u} c sl W g op lim b κ id now sched st
                   hsl hc hv hcl hw hsz hpk hpl stP stB stO stK =
  suc k + proj₁ R , proj₁ (proj₂ R)
  where
  nid    = proj₁ (mintNode sched)
  sched′ = proj₂ (mintNode sched)
  st′    = installNode nid (allFresh u op lim) st
  κ′     = thru-outer op nid ↠ κ
  resE   = subscribeE g b κ′ id now sched′ st′
  k      = sizeᵉ b
  2≤S    = FaceOK.fSize faceHere
  szb : sizeᵉ b ≤ Caps.cSize c
  szb = ≤-trans (allWrap-size op lim b) hsz
  capsE : capsOK? c sched′ st′ ≡ true
  capsE = capsOK?-setNode c nid (allFresh u op lim) sched′ st
            (allFresh-bnd (Caps.cSize c) u op lim)
            (allFresh-park (Caps.cSize c) (slotsSize (Sched.slots sched′)) u op lim)
            (allFresh-widN (Caps.cWid c) (Sched.slots sched′) u op lim)
            (capsOK?-nextNode c (suc (Sched.nextNode sched)) sched st
               (n≤1+n (Sched.nextNode sched)) hc)
            (λ i h → allFresh-parkStrat i u op lim)
  PL : suc (pathLen κ) ≤ Caps.cSize (frameStep k c)
  PL = ≤-trans hpl (proj₁ (c⊑step c k 2≤S))
  PS = path-step c k (thru-outer op nid) κ 2≤S refl
         (pathSz?-lvl c k κ 2≤S hpk) PL
  read≤ : pathRead κ ≤ nid
  read≤ = pathOrd?-read (Sched.nextNode sched) κ stO
  ordPUSH : pathOrd? (Sched.nextNode sched′) κ′ ≡ true
  ordPUSH = pathOrd?-push (suc nid) (thru-outer op nid) κ ≤-refl
              (T⇒≡true _ (≤⇒≤ᵇ read≤))
              (pathOrd?-mono nid (suc nid) κ (n≤1+n nid) stO)
  ...
  R = subscribeE-caps c (depthE g b κ′ id now sched′ st′)
        ...
        2≤S (FaceOK.fReg faceHere) hsl (FaceOK.fSlC faceHere) (FaceOK.fSlSz faceHere)
        (capsOK?-lvl c (suc k) sched′ st′ 2≤S capsE)
        (≤-trans szb (proj₁ (c⊑step c (suc k) 2≤S)))
        (expWid-fromSize c 0 0 k sl 2≤S (FaceOK.fSlC faceHere) b ≤-refl)
        (proj₁ PS) (proj₂ PS) ≤-refl ≤-refl ≤-refl stP stB ordPUSH parkκ′
...
pushVals-caps-room : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  nestClosOK? (frameStep Lv c) sl (allWrap op lim b) ≡ true →
  descW g (allWrap op lim b) κ id now sched st ≤ W →
  sizeᵉ (allWrap op lim b) ≤ Caps.cSize (frameStep j c) →
  ...
  burstCaps? (frameStep j c) sl
    (proj₁ (subscribeE g b (thru-outer op (proj₁ (mintNode sched)) ↠ κ)
              id now (proj₂ (mintNode sched))
              (installNode (proj₁ (mintNode sched)) (allFresh u op lim) st))) ≡ true →
  burstNest? (frameStep j c) sl
    (proj₁ (subscribeE g b (thru-outer op (proj₁ (mintNode sched)) ↠ κ)
              id now (proj₂ (mintNode sched))
              (installNode (proj₁ (mintNode sched)) (allFresh u op lim) st))) ≡ true →
  -- THE MINT'S SIDE CONDITION, AND THE BURST'S READING COMES OUT OF IT
  -- RATHER THAN FROM THE CAPS FACE.  The subscribe reports caps and a
  -- count, never a floor, so the burst the walk below is handed reads
  -- below `pathFloor κ` by `subscribeE-burstStrat` and by nothing here
  pathStrat? κ ≡ true →
  inputsBelowᵉ (pathFloor κ) b ≡ true →
  -- AND THE TWO REGISTRY READINGS AT THE UNEXTENDED CHAIN, which the
  -- mint below spends rather than forwards: the frame this head pushes
  -- names the cell the counter has only now handed out, so the extended
  -- chain's order reading is built here and the caller is asked for the
  -- parent's
  pathOrd? (Sched.nextNode sched) κ ≡ true →
  pathPark? κ st ≡ true →
  let res = subscribeE g b (thru-outer op (proj₁ (mintNode sched)) ↠ κ)
  ...
  in pushValsRoomOK (frameStep j c) sl W g op (proj₁ (mintNode sched)) κ id now
       (proj₁ res) (proj₁ (proj₂ res)) (proj₂ (proj₂ res))
pushVals-caps-room {u = u} c j Lv sl W g op lim b κ id now sched st hj hLv hsl hc hv hcl hw hsz hpk hpl hbc hbn stP stB stO stK =
  pushVals-room-join (frameStep j c) (proj₁ EX) sl W g op (proj₁ (mintNode sched)) κ id now
    (proj₁ res) (proj₁ (proj₂ res)) (proj₂ (proj₂ res))
    ...
    hbc hpk hpl
    (pushVals-caps-wid c j Lv sl W g op lim b κ id now sched st hj hLv hsl hc hv hcl hw hbc)
    (pushVals-caps-burstW (frameStep Lv c) sl W g op lim b κ id now sched st
       ⦃ face⊑ c (frameStep Lv c) sl (c⊑step c Lv 2≤S) faceHere ⦄ hsl hc hv hcl hw)
    stP bStr
  ...
  res = subscribeE g b (thru-outer op (proj₁ (mintNode sched)) ↠ κ)
          ...
          (installNode (proj₁ (mintNode sched)) (allFresh u op lim) st)
  2≤S  = FaceOK.fSize faceHere
  step = c⊑step c j (FaceOK.fSize faceHere)
  facej = face⊑ c (frameStep j c) sl step faceHere
  stepL = frameStep-mono-j c 2≤S hLv

  bStr = subscribeE-burstStrat g b (thru-outer op (proj₁ (mintNode sched)) ↠ κ)
           id now (proj₂ (mintNode sched))
           (installNode (proj₁ (mintNode sched)) (allFresh u op lim) st) stP stB

  EX = pushVals-caps-exit (frameStep j c) sl W g op lim b κ id now sched st
         hsl (capsOK?-mono (frameStep Lv c) (frameStep j c) sched st stepL hc)
         (nestValOK?-widen (obs u) (allWrap op lim b) stepL hv)
         (nestClosOK?-widen (frameStep Lv c) (frameStep j c) sl (allWrap op lim b) stepL hcl)
         hw hsz hpk hpl stP stB stO stK ⦃ facej ⦄
...
pushVals-caps-st : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  let res = subscribeE g b (thru-outer op (proj₁ (mintNode sched)) ↠ κ)
          ...
          (installNode (proj₁ (mintNode sched)) (allFresh u op lim) st)
  in Σ ℕ λ L₀ →
     pushValsStOK (frameStep j c) L₀ sl g op (proj₁ (mintNode sched)) κ id now
       (proj₁ res) (proj₁ (proj₂ res)) (proj₂ (proj₂ res))
pushVals-caps-st {u = u} c j Lv sl W g op lim b κ id now sched st hj hLv hsl hc hv hcl hw hsz hpk hpl hbc hbn stP stB stO stK =
  proj₁ EX ,
  pushValsSt-walk (frameStep j c) (proj₁ EX) sl W g op (proj₁ (mintNode sched)) κ id now (proj₁ res)
    (proj₁ (proj₂ res)) (proj₂ (proj₂ res))
    ⦃ face⊑ c (frameStep j c) sl (c⊑step c j (FaceOK.fSize faceHere)) faceHere ⦄
    (trans (KeepsC.slotsEq
              (subscribeE-keeps g b (thru-outer op (proj₁ (mintNode sched)) ↠ κ)
    ...
    (pushVals-caps-room c j Lv sl W g op lim b κ id now sched st hj hLv hsl hc hv hcl hw
       hsz hpk hpl hbc hbn stP stB stO stK)
    stP bStr
    (mintSub-ord g op lim b κ id now sched st stO)
    (mintSub-pathPark g op lim b κ id now sched st stO stK)
    (∧-trueˡ (mintSub-framePark g op lim b κ id now sched st (capsOK?-regOrd _ sched st hc)))
...
pushVals-caps : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  in Σ ℕ λ L₀ →
     pushValsCapsOK (frameStep j c) L₀ sl W g op (proj₁ (mintNode sched)) κ id now
       (proj₁ res) (proj₁ (proj₂ res)) (proj₂ (proj₂ res))
pushVals-caps {u = u} c j Lv sl W g op lim b κ id now sched st hj hLv hsl hc hv hcl hw hsz hpk hpl hbc hbn stP stB stO stK =
  proj₁ ST ,
...
pushVals-merge : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (c : Caps) (j Lv : ℕ) (sl : Slots Γ) (B W : ℕ) (g : Gas)
  (lim : Maybe ℕ) (b : Closed Γ (obs u))
  (κ : Path Γ u t) (id : Id) (now : Tick) (sched : Sched Γ) (st : EvalSt e) →
  ⦃ _ : FaceOK c sl ⦄ →
  Caps.cSize c ≤ j → Lv ≤ j →
  Sched.slots sched ≡ sl → capsOK? (frameStep Lv c) sched st ≡ true →
  nestValOK? (frameStep Lv c) (obs u) (mergeAllᵉ lim b) ≡ true →
  nestClosOK? (frameStep Lv c) sl (mergeAllᵉ lim b) ≡ true →
  nestDᵉ (mergeAllᵉ lim b) ≤ B →
  descW g (mergeAllᵉ lim b) κ id now sched st ≤ W →
  sizeᵉ (mergeAllᵉ lim b) ≤ Caps.cSize (frameStep j c) →
  pathSz? (Caps.cSize (frameStep j c)) κ ≡ true →
  suc (pathLen κ) ≤ Caps.cSize (frameStep j c) →
  burstCaps? (frameStep j c) sl
  ...
  pathStrat? κ ≡ true →
  inputsBelowᵉ (pathFloor κ) b ≡ true →
  -- AND THE TWO REGISTRY READINGS AT THE UNEXTENDED CHAIN, forwarded to
  -- the caps master, which is the only thing under this head that mints
  pathOrd? (Sched.nextNode sched) κ ≡ true →
  pathPark? κ st ≡ true →
  let res = subscribeE g b (thru-outer mergeAllᵒ (proj₁ (mintNode sched)) ↠ κ)
          id now (proj₂ (mintNode sched))
          (installNode (proj₁ (mintNode sched)) (mergeAll-st {t = u} lim 0 [] false) st)
  in Σ ℕ λ L₀ →
     pushValsOK (frameStep j c) L₀ sl B W (syncSizeᵉ b)
       g mergeAllᵒ (proj₁ (mintNode sched)) κ id now
       (proj₁ res) (proj₁ (proj₂ res)) (proj₂ (proj₂ res))
pushVals-merge {Γ = Γ} {t = t} {e = e} {u = u} c j Lv sl B W g lim b κ id now sched st
               hj hLv hsl hc hv hcl hn hw hsz hpk hpl hbc hbn hbu stP stB stO stK =
  proj₁ CAPS ,
...
pushVals-switch : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  Caps.cSize c ≤ j → Lv ≤ j →
  Sched.slots sched ≡ sl → capsOK? (frameStep Lv c) sched st ≡ true →
  nestValOK? (frameStep Lv c) (obs u) (switchAllᵉ b) ≡ true →
  nestClosOK? (frameStep Lv c) sl (switchAllᵉ b) ≡ true →
  nestDᵉ (switchAllᵉ b) ≤ B →
  descW g (switchAllᵉ b) κ id now sched st ≤ W →
  sizeᵉ (switchAllᵉ b) ≤ Caps.cSize (frameStep j c) →
  pathSz? (Caps.cSize (frameStep j c)) κ ≡ true →
  suc (pathLen κ) ≤ Caps.cSize (frameStep j c) →
  burstCaps? (frameStep j c) sl
...
pushVals-exhaust : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  Caps.cSize c ≤ j → Lv ≤ j →
  Sched.slots sched ≡ sl → capsOK? (frameStep Lv c) sched st ≡ true →
  nestValOK? (frameStep Lv c) (obs u) (exhaustAllᵉ b) ≡ true →
  nestClosOK? (frameStep Lv c) sl (exhaustAllᵉ b) ≡ true →
  nestDᵉ (exhaustAllᵉ b) ≤ B →
  descW g (exhaustAllᵉ b) κ id now sched st ≤ W →
  sizeᵉ (exhaustAllᵉ b) ≤ Caps.cSize (frameStep j c) →
  pathSz? (Caps.cSize (frameStep j c)) κ ≡ true →
  suc (pathLen κ) ≤ Caps.cSize (frameStep j c) →
  burstCaps? (frameStep j c) sl
...
thruFit-merge : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  pathStrat? κ ≡ true →
  inputsBelowᵉ (pathFloor κ) b ≡ true →
  -- AND THE TWO READINGS THE MINT UNDER THIS HEAD SPENDS, forwarded to
  -- the value walk and, at the counter the subscribe leaves, to the fit
  pathOrd? (Sched.nextNode sched) κ ≡ true →
  pathPark? κ st ≡ true →
  let res = subscribeE g b (thru-outer mergeAllᵒ (proj₁ (mintNode sched)) ↠ κ)
              id now (proj₂ (mintNode sched))
  ...
  in pushFitOK (nestB (Caps.cSize (frameStep j c)) W (nestUnit e sl) B
       ...
       g mergeAllᵒ (proj₁ (mintNode sched)) κ id now
       (proj₁ res) (proj₁ (proj₂ res)) (proj₂ (proj₂ res))
thruFit-merge {e = e} {u = u} c j Lv sl B W g lim b κ id now sched st hj hLv hsl hc hv hcl hn hw hsz hpk hpl hbc hbn hbu stP stB stO stK =
  ...
  PV = pushVals-merge c j Lv sl B W g lim b κ id now sched st hj hLv hsl hc hv hcl hn hw
         hsz hpk hpl hbc hbn hbu stP stB stO stK
...
thruFit-switch : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (c : Caps) (j Lv : ℕ) (sl : Slots Γ) (B W : ℕ) (g : Gas) (b : Closed Γ (obs u))
  (κ : Path Γ u t) (id : Id) (now : Tick) (sched : Sched Γ) (st : EvalSt e) →
  ⦃ _ : FaceOK c sl ⦄ →
  Caps.cSize c ≤ j → Lv ≤ j →
  Sched.slots sched ≡ sl → capsOK? (frameStep Lv c) sched st ≡ true →
...
subscribeE-nest-slot {e = e} c d sl B W Lv g i κ id now sched st hsl hc hv hcl hn hw hd hsz hdw hpk hpl _ _ _ _ hceil =
  Lv , ceil-here c (pred d) d Lv _ _ hceil , ≤-refl
  , nestTriple-widen
        (arrD≤nestB (Caps.cSize (frameStep Lv c)) W (nestUnit e sl) B
           (slotClos sl i)
           (nestClosOK?-size (frameStep Lv c) sl (input i) hcl))
        (subscribeE-nest-arr (frameStep Lv c) sl B W g (input i) κ id now sched st
           hsl (capsOK?⇒nest (frameStep Lv c) sched st hc) hv hcl hn hw)
  where

  instance
    faceLv : FaceOK (frameStep Lv c) sl
    faceLv = face⊑ c (frameStep Lv c) sl (c⊑step c Lv (FaceOK.fSize faceHere)) faceHere
...
subscribeE-nest {Γ = Γ} {t = t} {e = e} {u = u} c d sl B W Lv g (ofᵉ ts) κ id now sched st
  hsl hc hv hcl hn hw hd hsz hdw hpk hpl _ _ _ _ hceil =
  Lv , ceil-here c (pred d) d Lv _ _ hceil , ≤-refl ,
  ≤-trans (≤-reflexive (cong (nestDᵛˢ {u = u})
             (oneShot-vals {A = Val Γ t} (map (λ tm → evalTm tm) ts) id sched)))
    (≤-trans (≤-trans (ofVals-nest-sync ts) (*-monoʳ-≤ (2 ^ syncSizeᵗˢ ts) hn))
      (≤-trans (*-monoʳ-≤ (2 ^ syncSizeᵗˢ ts)
                  (m≤m+n B (nestB (Caps.cSize (frameStep Lv c)) W (nestUnit e sl) B 0)))
               (nestB-frame (Caps.cSize (frameStep Lv c)) W (nestUnit e sl) B
                  0 (syncSizeᵗˢ ts) (syncSizeᵉ (ofᵉ ts))
                  (nestValOK?-size (frameStep Lv c) (ofᵉ ts) hv) (s≤s z≤n))))
  , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
subscribeE-nest c d sl B W Lv g emptyᵉ κ id now sched st hsl hc hv hcl hn hw hd hsz hdw hpk hpl _ _ _ _ hceil =
  Lv , ceil-here c (pred d) d Lv _ _ hceil , ≤-refl , z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
subscribeE-nest {e = e} c d sl B W Lv g (mapᵉ f b) κ id now sched st hsl hc hv hcl hn hw hd hsz hdw hpk hpl stP stB stR stK hceil =
  jIH
  , jB
  , ≤-trans (n≤1+n Lv) IH≥
  , ≤-trans (proj₁ push)
  ...
  IH₀ = subscribeE-nest c d sl B W (suc Lv) g b (map-f f ↠ κ) id now sched st
         hsl (capsOK?-mono (frameStep Lv c) (frameStep (suc Lv) c) sched st step⊑ hc)
         (nestValOK?-widen _ b step⊑ (nestValOK?-map (frameStep Lv c) f b hv))
         (nestClosOK?-widen (frameStep Lv c) (frameStep (suc Lv) c) sl b step⊑
            (nestClosOK?-mono (frameStep Lv c) sl b (mapᵉ f b)
               (≤-trans (m≤n+m _ _) (n≤1+n _)) hcl))
         (≤-trans (m≤n+m (nestDᵉ b) (nestDᵗ f)) hn)
         (≤-trans (descW-map g f b κ id now sched st) hw)
         (≤-trans (m≤m⊔n _ _) hd)
         hsz′ hdw′ (proj₁ KEY) (proj₂ KEY)
         (∧-intro (proj₁ (∧-true _ _ stB)) stP) (proj₂ (∧-true _ _ stB))
         -- A MAP FRAME READS NOTHING AND OWNS NO CELL, so both readings
         -- cross it on their unit: its cell is zero and its store arm
         -- is silent
         (pathOrd?-push (Sched.nextNode sched) (map-f f) κ z≤n refl stR)
         (∧-intro refl stK) hceil′

  jIH = proj₁ IH₀
  jB  = proj₁ (proj₂ IH₀)
  IH≥ = proj₁ (proj₂ (proj₂ IH₀))
  IH  = proj₂ (proj₂ (proj₂ IH₀))
  S′  = Caps.cSize (frameStep jIH c)

  cap≤ : Caps.cSize c ≤ S′
  cap≤ = iterSize-infl (Caps.cSize c)
           (≤-trans (n≤1+n 1) (FaceOK.fSize faceHere))
           jIH (Caps.cSize c)

  capLv≤ : Caps.cSize (frameStep Lv c) ≤ S′
  capLv≤ = proj₁ (frameStep-mono-j c 2≤c (≤-trans (n≤1+n Lv) IH≥))
  ...
  hk = ≤-trans (≤-trans (s≤s (m≤m+n (syncSizeᵗ f) (syncSizeᵉ b)))
                        (nestValOK?-size (frameStep Lv c) (mapᵉ f b) hv))
               capLv≤
...
subscribeE-nest {e = e} c d sl B W Lv g (takeᵉ cnt b) κ id now sched st hsl hc hv hcl hn hw hd hsz hdw hpk hpl stP stB stR stK hceil
  with evalTm cnt in eqc
... | zero  = Lv , ceil-here c (pred d) d Lv _ _ hceil , ≤-refl , z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
... | suc k =
  jIH
  , jB
  , ≤-trans (n≤1+n Lv) IH≥
  ...
  inv₀ : capsOK? (frameStep Lv c) sched₀ st₀ ≡ true
  inv₀ = capsOK?-setNode (frameStep Lv c) nid (take-st (suc k)) sched₀ st refl refl refl
           (capsOK?-nextNode (frameStep Lv c) (suc (Sched.nextNode sched)) sched st
              (n≤1+n (Sched.nextNode sched)) hc)
           (λ i h → refl)
  ...
  read≤ : pathRead κ ≤ nid
  read≤ = pathOrd?-read (Sched.nextNode sched) κ stR
  ...
  IH₀ = subscribeE-nest c d sl B W (suc Lv) g b (take-f nid ↠ κ) id now sched₀ st₀
         hsl (capsOK?-mono (frameStep Lv c) (frameStep (suc Lv) c) sched₀ st₀ step⊑ inv₀)
         (nestValOK?-widen _ b step⊑ (nestValOK?-take (frameStep Lv c) cnt b hv))
         (nestClosOK?-widen (frameStep Lv c) (frameStep (suc Lv) c) sl b step⊑
            (nestClosOK?-mono (frameStep Lv c) sl b (takeᵉ cnt b)
               (≤-trans (m≤n+m _ _) (n≤1+n _)) hcl)) hn
         (≤-trans (descW-take g cnt b κ id now sched st k eqc) hw)
         (≤-trans (m≤m⊔n _ _) hd)
         hsz′ hdw′ (proj₁ KEY) (proj₂ KEY)
         (∧-intro refl stP) (proj₂ (∧-true _ _ stB))
         -- AND THE MINT'S TWO, BUILT HERE: the frame sits at the cell
         -- the counter has only now handed out, and the cell a take
         -- installs holds no payload for the store arm to read
...
subscribeE-nest {e = e} {u = u} c d sl B W Lv g (mergeAllᵉ lim b) κ id now sched st
  hsl hc hv hcl hn hw hd hsz hdw hpk hpl stP stB stR stK hceil =
  J
  , Jb
  , J≥Lv
  , proj₁ PUSH
  ...
  inv₀ : capsOK? (frameStep Lv c) sched₀ st₀ ≡ true
  inv₀ = capsOK?-setNode (frameStep Lv c) nid (mergeAll-st {t = u} lim 0 [] false) sched₀ st refl refl refl
           (capsOK?-nextNode (frameStep Lv c) (suc (Sched.nextNode sched)) sched st
              (n≤1+n (Sched.nextNode sched)) hc)
           (λ i h → refl)
  ...
  -- THE MINT'S TWO READINGS, BUILT RATHER THAN CARRIED.  The frame the
  -- child descends under names the cell the counter has only now handed
  -- out, so the order reading is the parent's pushed at a cell nothing
  -- below can already read, and the park reading is the parent's beside
  -- a freshly installed cell that carries no payload at all.
  ordPUSH : pathOrd? (Sched.nextNode sched₀) (thru-outer mergeAllᵒ nid ↠ κ) ≡ true
  ordPUSH = pathOrd?-push (suc nid) (thru-outer mergeAllᵒ nid) κ ≤-refl
              (T⇒≡true _ (≤⇒≤ᵇ read≤))
              (pathOrd?-mono nid (suc nid) κ (n≤1+n nid) stR)
  ...
  IH₀ = subscribeE-nest c d sl B W (suc Lv) g b (thru-outer mergeAllᵒ nid ↠ κ) id now sched₀ st₀
         hsl (capsOK?-mono (frameStep Lv c) (frameStep (suc Lv) c) sched₀ st₀ step⊑ inv₀)
         (nestValOK?-widen _ b step⊑ (nestValOK?-merge (frameStep Lv c) lim b hv))
         (nestClosOK?-widen (frameStep Lv c) (frameStep (suc Lv) c) sl b step⊑
            (nestClosOK?-mono (frameStep Lv c) sl b (mergeAllᵉ lim b) (n≤1+n _) hcl))
         (≤-trans (n≤1+n (nestDᵉ b)) hn)
         (≤-trans (descW-merge g lim b κ id now sched st) hw)
         (≤-trans (m≤m⊔n _ _) hd)
         hsz′ hdw′ (proj₁ KEY) (proj₂ KEY) stP stB ordPUSH parkPUSH hceil′

  BC  = subscribeE-burst-capsAt c d sl (suc Lv) g b (thru-outer mergeAllᵒ nid ↠ κ) id now sched₀ st₀
         hsl (capsOK?-mono (frameStep Lv c) (frameStep (suc Lv) c) sched₀ st₀ step⊑ inv₀)
         hsz′ hdw′ (proj₁ KEY) (proj₂ KEY) stP stB ordPUSH parkPUSH
         (≤-trans (m≤m⊔n _ _) hd) hceil′

  BN  = subscribeE-burst-nestAt c d sl (suc Lv) W g b (thru-outer mergeAllᵒ nid ↠ κ) id now sched₀ st₀
         hsl (capsOK?-mono (frameStep Lv c) (frameStep (suc Lv) c) sched₀ st₀ step⊑ inv₀)
         (nestValOK?-widen (obs _) b step⊑ (nestValOK?-merge (frameStep Lv c) lim b hv))
         (nestClosOK?-widen (frameStep Lv c) (frameStep (suc Lv) c) sl b step⊑
            (nestClosOK?-mono (frameStep Lv c) sl b (mergeAllᵉ lim b) (n≤1+n _) hcl))
         hsz′ hdw′ (proj₁ KEY) (proj₂ KEY)
         (≤-trans (descW-merge g lim b κ id now sched st) hw)
  ...
  J≥IH : jIH ≤ J
  J≥IH = ≤-trans (≤-trans (≤-trans (m≤m⊔n jIH (Caps.cSize c)) (m≤n⊔m (proj₁ BC) (jIH ⊔ Caps.cSize c))) (m≤n⊔m Lv _)) J₀≤J

  J≥S : Caps.cSize c ≤ J
  J≥S = ≤-trans (≤-trans (≤-trans (m≤n⊔m jIH (Caps.cSize c)) (m≤n⊔m (proj₁ BC) (jIH ⊔ Caps.cSize c))) (m≤n⊔m Lv _)) J₀≤J

  -- AND THE WALK'S OWN LEVEL IS UNDER THE COUNT, which is what lets
  -- the entry keys be read at the level this arm reports.  The
  -- ceiling is exactly that implication at an unspent remainder
  J≥Lv : Lv ≤ J
  J≥Lv = ≤-trans (m≤m⊔n Lv _) J₀≤J

  Lv⊑J = frameStep-mono-j c 2≤c J≥Lv
  hszJ = ≤-trans hsz (proj₁ Lv⊑J)
  hpkJ = pathSz?-⊑ κ Lv⊑J hpk
  hplJ = ≤-trans hpl (proj₁ Lv⊑J)
  ...
  1≤S : 1 ≤ Caps.cSize c
  1≤S = ≤-trans (n≤1+n 1) (FaceOK.fSize faceHere)

  Jb : J ≤ sizeCount c d ⊔ Caps.cSize c
  Jb = ⊔-lub (proj₁ (proj₂ BN))
         (⊔-lub Lv≤cnt (⊔-lub (proj₁ (proj₂ BC)) (⊔-lub jB (m≤n⊔m (sizeCount c d) (Caps.cSize c)))))
  ...
  HBC = burstCaps?-widen sl (proj₁ res)
          (frameStep-mono-j c (FaceOK.fSize faceHere)
             (≤-trans (≤-trans (m≤m⊔n (proj₁ BC) (jIH ⊔ Caps.cSize c)) (m≤n⊔m Lv _)) J₀≤J))
          (proj₂ (proj₂ BC))

  HBN = burstNest?-widen sl (proj₁ res)
          (frameStep-mono-j c 2≤c (m≤m⊔n (proj₁ BN) J₀))
          (proj₂ (proj₂ BN))

  FIT = thruFit-merge c J Lv sl B W g lim b κ id now sched st J≥S J≥Lv hsl hc hv hcl hn hw hszJ hpkJ hplJ HBC HBN (proj₁ IH) stP stB stR stK
...
subscribeE-nest {e = e} {u = u} c d sl B W Lv g (switchAllᵉ b) κ id now sched st
  ...
  inv₀ : capsOK? (frameStep Lv c) sched₀ st₀ ≡ true
  inv₀ = capsOK?-setNode (frameStep Lv c) nid (switch-st nothing false) sched₀ st refl refl refl
           (capsOK?-nextNode (frameStep Lv c) (suc (Sched.nextNode sched)) sched st
              (n≤1+n (Sched.nextNode sched)) hc)
           (λ i h → refl)
  ...
  -- below can already read, and the park reading is the parent's beside
  -- a freshly installed cell that carries no payload at all.
  ordPUSH : pathOrd? (Sched.nextNode sched₀) (thru-outer switchᵒ nid ↠ κ) ≡ true
  ordPUSH = pathOrd?-push (suc nid) (thru-outer switchᵒ nid) κ ≤-refl
              (T⇒≡true _ (≤⇒≤ᵇ read≤))
              (pathOrd?-mono nid (suc nid) κ (n≤1+n nid) stR)
  ...
  IH₀ = subscribeE-nest c d sl B W (suc Lv) g b (thru-outer switchᵒ nid ↠ κ) id now sched₀ st₀
         hsl (capsOK?-mono (frameStep Lv c) (frameStep (suc Lv) c) sched₀ st₀ step⊑ inv₀)
         (nestValOK?-widen _ b step⊑ (nestValOK?-switch (frameStep Lv c) b hv))
         (nestClosOK?-widen (frameStep Lv c) (frameStep (suc Lv) c) sl b step⊑
            (nestClosOK?-mono (frameStep Lv c) sl b (switchAllᵉ b) (n≤1+n _) hcl))
         (≤-trans (n≤1+n (nestDᵉ b)) hn)
         (≤-trans (descW-switch g b κ id now sched st) hw)
         (≤-trans (m≤m⊔n _ _) hd)
         hsz′ hdw′ (proj₁ KEY) (proj₂ KEY) stP stB ordPUSH parkPUSH hceil′
  ...
  BN  = subscribeE-burst-nestAt c d sl (suc Lv) W g b (thru-outer switchᵒ nid ↠ κ) id now sched₀ st₀
         hsl (capsOK?-mono (frameStep Lv c) (frameStep (suc Lv) c) sched₀ st₀ step⊑ inv₀)
         (nestValOK?-widen (obs _) b step⊑ (nestValOK?-switch (frameStep Lv c) b hv))
         (nestClosOK?-widen (frameStep Lv c) (frameStep (suc Lv) c) sl b step⊑
            (nestClosOK?-mono (frameStep Lv c) sl b (switchAllᵉ b) (n≤1+n _) hcl))
         hsz′ hdw′ (proj₁ KEY) (proj₂ KEY)
         (≤-trans (descW-switch g b κ id now sched st) hw)
  ...
  FIT = thruFit-switch c J Lv sl B W g b κ id now sched st J≥S J≥Lv hsl hc hv hcl hn hw hszJ hpkJ hplJ HBC HBN (proj₁ IH) stP stB stR stK
...
subscribeE-nest {e = e} {u = u} c d sl B W Lv g (exhaustAllᵉ b) κ id now sched st
  ...
  inv₀ : capsOK? (frameStep Lv c) sched₀ st₀ ≡ true
  inv₀ = capsOK?-setNode (frameStep Lv c) nid (exhaust-st false false) sched₀ st refl refl refl
           (capsOK?-nextNode (frameStep Lv c) (suc (Sched.nextNode sched)) sched st
              (n≤1+n (Sched.nextNode sched)) hc)
           (λ i h → refl)
  ...
  -- below can already read, and the park reading is the parent's beside
  -- a freshly installed cell that carries no payload at all.
  ordPUSH : pathOrd? (Sched.nextNode sched₀) (thru-outer exhaustᵒ nid ↠ κ) ≡ true
  ordPUSH = pathOrd?-push (suc nid) (thru-outer exhaustᵒ nid) κ ≤-refl
              (T⇒≡true _ (≤⇒≤ᵇ read≤))
              (pathOrd?-mono nid (suc nid) κ (n≤1+n nid) stR)
  ...
  IH₀ = subscribeE-nest c d sl B W (suc Lv) g b (thru-outer exhaustᵒ nid ↠ κ) id now sched₀ st₀
         hsl (capsOK?-mono (frameStep Lv c) (frameStep (suc Lv) c) sched₀ st₀ step⊑ inv₀)
         (nestValOK?-widen _ b step⊑ (nestValOK?-exhaust (frameStep Lv c) b hv))
         (nestClosOK?-widen (frameStep Lv c) (frameStep (suc Lv) c) sl b step⊑
            (nestClosOK?-mono (frameStep Lv c) sl b (exhaustAllᵉ b) (n≤1+n _) hcl))
         (≤-trans (n≤1+n (nestDᵉ b)) hn)
         (≤-trans (descW-exhaust g b κ id now sched st) hw)
         (≤-trans (m≤m⊔n _ _) hd)
         hsz′ hdw′ (proj₁ KEY) (proj₂ KEY) stP stB ordPUSH parkPUSH hceil′
  ...
  BN  = subscribeE-burst-nestAt c d sl (suc Lv) W g b (thru-outer exhaustᵒ nid ↠ κ) id now sched₀ st₀
         hsl (capsOK?-mono (frameStep Lv c) (frameStep (suc Lv) c) sched₀ st₀ step⊑ inv₀)
         (nestValOK?-widen (obs _) b step⊑ (nestValOK?-exhaust (frameStep Lv c) b hv))
         (nestClosOK?-widen (frameStep Lv c) (frameStep (suc Lv) c) sl b step⊑
            (nestClosOK?-mono (frameStep Lv c) sl b (exhaustAllᵉ b) (n≤1+n _) hcl))
         hsz′ hdw′ (proj₁ KEY) (proj₂ KEY)
         (≤-trans (descW-exhaust g b κ id now sched st) hw)
  ...
  FIT = thruFit-exhaust c J Lv sl B W g b κ id now sched st J≥S J≥Lv hsl hc hv hcl hn hw hszJ hpkJ hplJ HBC HBN (proj₁ IH) stP stB stR stK
  ...
  grow : nestB S′ W (nestUnit e sl) B (syncSizeᵉ b)
           ≤ nestB S′ W (nestUnit e sl) B (syncSizeᵉ (exhaustAllᵉ b))
  grow = nestB-mono S′ W (nestUnit e sl) B (n≤1+n (syncSizeᵉ b))
subscribeE-nest c d sl B W Lv g0 (μᵉ body) κ id now sched st hsl hc hv hcl hn hw hd hsz hdw hpk hpl _ _ _ _ hceil =
  Lv , ceil-here c (pred d) d Lv _ _ hceil , ≤-refl , z≤n , m≤m⊔n _ _ , (λ j → m≤m⊔n _ _)
subscribeE-nest {e = e} c d sl B W Lv (gs fuel) (μᵉ body) κ id now sched st hsl hc hv hcl hn hw hd hsz hdw hpk hpl stP stB stR stK hceil =
  jIH
  , jB
  , ≤-trans (m≤m+n Lv (proj₁ US)) IH≥
  , ≤-trans (proj₁ IH) grow
  , ≤-trans (proj₁ (proj₂ IH)) (⊔-mono-≤ ≤-refl grow)
  , (λ j → ≤-trans (proj₂ (proj₂ IH) j) (⊔-mono-≤ ≤-refl grow))
  where
  ...
  hceil′ = ceil-le c (pred d) d (Lv + proj₁ US) _
             (nest (unfoldμ body) sl (EvalSt.connectedShares st))
             _ (suc (sizeᵉ (unfoldμ body))) 2≤c
             (mu-step body sl (EvalSt.connectedShares st) _ ≤-refl)
             (s≤s (proj₁ (proj₂ US)))
             (ceil-mu c (pred d) d Lv _ _ (sizeᵉ (μᵉ body)) 2≤c hsz hceil)

  IH₀ = subscribeE-nest c d sl B W (Lv + proj₁ US) fuel (unfoldμ body) κ id now sched st hsl
         (capsOK?-mono (frameStep Lv c) (frameStep (Lv + proj₁ US) c) sched st up⊑ hc)
         (≤ᵇ-true (syncSizeᵉ (unfoldμ body)) (Caps.cSize (frameStep (Lv + proj₁ US) c))
           (≤-trans (≤-reflexive (syncSize-unfoldμ body))
             (≤-trans (≤-trans (n≤1+n (syncSizeᵉ body))
                               (nestValOK?-size (frameStep Lv c) (μᵉ body) hv))
                      (proj₁ up⊑))))
         (nestClosOK?-widen (frameStep Lv c) (frameStep (Lv + proj₁ US) c) sl (unfoldμ body) up⊑
            (nestClosOK?-mono (frameStep Lv c) sl (unfoldμ body) (μᵉ body)
         ...
         (pathSz?-⊑ κ up⊑ hpk) (≤-trans hpl (proj₁ up⊑)) stP
         (T⇒≡true (inputsBelowᵉ (pathFloor κ) (unfoldμ body))
            (ib-unfoldμ (pathFloor κ) body (T-to stB)))
         -- the unfolding subscribes the SAME chain at the SAME state,
         -- so both readings cross the substitution untouched
         stR stK hceil′
...
subscribeE-nest c d sl B W Lv g (deferᵉ body) κ id now sched st hsl hc hv hcl hn hw hd hsz hdw hpk hpl _ _ _ _ hceil =
  Lv , ceil-here c (pred d) d Lv _ _ hceil , ≤-refl , z≤n
  , ≤-trans (setNode-nodes _ _ (EvalSt.nodes st)) (⊔-lub z≤n (m≤m⊔n _ _))
  , (λ j → ≤-trans (nodeNestAt-set j _ _ st) (⊔-lub z≤n (m≤m⊔n _ _)))
...
subscribeInner-nest-tight : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {s}
  ...
  pathStrat? κ ≡ true →
  inputsBelowᵉ (pathFloor κ) o ≡ true →
  -- AND THE THREE THE DESCENT UNDER THE OUTER'S OWN FRAME SPENDS.  The
  -- inner subscribes below a `from-inner` whose sibling is the outer's
  -- cell, so the order reading is asked for at the chain the outer
  -- ALREADY extends and the park reading splits into that cell's own
  -- and the chain's below it -- which is exactly how the proven caps
  -- face beside this walk states them.
...
subscribeInner-nest-tight {e = e} c d sl B W Lv (gs fuel) op allNid κ id now o sched st
  ...
  R = subscribeE-nest c d sl B W (suc Lv) fuel o
    (from-inner op allNid (Sched.nextNode sched) ↠ κ) id now
    (record sched { nextNode = suc (Sched.nextNode sched) }) st hsl
    (capsOK?-mono (frameStep Lv c) (frameStep (suc Lv) c)
       (record sched { nextNode = suc (Sched.nextNode sched) }) st step⊑
       (capsOK?-nextNode (frameStep Lv c) (suc (Sched.nextNode sched)) sched st
              (n≤1+n (Sched.nextNode sched)) hc))
    (nestValOK?-widen _ o step⊑ hv)
    (nestClosOK?-widen (frameStep Lv c) (frameStep (suc Lv) c) sl o step⊑ hcl) hn
    (≤-trans (innerW-gs fuel op allNid κ id now o sched st) hw) hd
    (≤-trans hsz (proj₁ step⊑)) (≤-trans hdw (proj₁ (proj₂ step⊑)))
    (proj₁ KEY) (proj₂ KEY) (∧-intro refl stP) stB
    -- THE FLATTEN HOP, and both halves are already in hand: the order
    -- reading crosses from the outer's frame to the inner's by the one
...
subscribeInner-nest {e = e} c d sl B W Lv sf op allNid κ id now o sched st hsl hc hv hcl hn hw hd hsz hdw hpk hpl stP stB stR stK stQ stW hceil =
  ...
  tight = proj₂ (proj₂ (proj₂ tight₀))
  S′    = Caps.cSize (frameStep jT c)

  grow : nestB S′ W (nestUnit e sl) B (syncSizeᵉ o)
           ≤ nestB S′ W (nestUnit e sl) B S′
  grow = nestB-mono S′ W (nestUnit e sl) B
           (≤-trans (nestValOK?-size (frameStep Lv c) o hv)
             (proj₁ (frameStep-mono-j c (FaceOK.fSize faceHere) Lv≤jT)))
...
mergeAllDrain-nest : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {s}
  ...
  suc (pathLen κ) ≤ Caps.cSize (frameStep Lv c) →
  pathStrat? κ ≡ true →
  all (inputsBelowᵉ (pathFloor κ)) q ≡ true →
  -- AND THE THREE THE DRAIN'S OWN DESCENT SPENDS, carried across the
  -- queue: every entry is subscribed under the SAME outer cell, so the
  -- chain is fixed and what has to travel entry to entry is only the
  -- state under it -- which the subscribe's own freshness receipt gives.
...
... | true  =
  ...
  -- THE HEAD IS SUBSCRIBED ONE LEVEL UP, because that is the level its
  -- closure key is stated at and the key cannot be minted below it.
  -- Every other premise widens onto the higher frame by the step's own
  -- monotonicity, so the shift costs exactly one unit of the ceiling
  -- and nothing else.
  stepʰ = frameStep-mono-j c (FaceOK.fSize faceHere) (n≤1+n Lv)

  -- THE HEAD'S CEILING IS READ OFF THE FRAME'S SWEEP, one level up and
  -- one entry in, and the tail's is the same sweep with one unit spent
  -- on the climb the subscribe reports.  The successor the head is
  -- charged for is bought from the size cap's own strict climb, which
  -- is why the entry never had to store a roof for it.
  ceilHere : CeilD c (pred d) d (suc Lv) (nest o sl (EvalSt.connectedShares st))
               (suc (suc (sizeᵉ o)))
  ceilHere = ceilS-head c (pred d) d Lv B̂ (length q)
               (nest o sl (EvalSt.connectedShares st)) (suc (sizeᵉ o))
               (FaceOK.fSize faceHere) hB̂
               (≤-trans (s≤s (proj₁ (proj₂ (proj₂ (proj₂ (proj₂ hcdA))))))
                        (sizeAt-strict (Caps.cSize c) Lv
                           (≤-trans (s≤s z≤n) (FaceOK.fSize faceHere))))
               ceilQ

  ceilTail : CeilS c (pred d) d Lv″ (suc B̂) (length q)
  ceilTail = ceilS-sweep-step c (pred d) d Lv (suc B̂) (length q) r₀
               (FaceOK.fSize faceHere) (proj₁ (proj₂ tailΣ)) ceilQ

  SUB₀ = subscribeInner-nest c d sl B W (suc Lv) sf mergeAllᵒ allNid κ id now o sched st
          (proj₁ hcdA)
  ...
  IH₀ = mergeAllDrain-nest c d sl B W Lv″ sf allNid κ id now lim
         ...
         (B̂ , ceilTail , proj₂ (proj₂ tailΣ))
         (≤-trans (m≤n⊔m (nestDᵉ o) (queueNest q)) hq)
         splitW′
         (≤-trans (m≤n⊔m _ _) hd)
         (pathSz?-⊑ κ step″ hpk) (≤-trans hpl (proj₁ step″))
         stP (proj₂ (∧-true _ _ stQ))
         -- ONE ENTRY'S SUBSCRIBE ONLY RAISES THE COUNTER AND WRITES
         -- CELLS IT MINTED, so all three readings ride it on the
         -- subscribe's own freshness and no arithmetic enters here
         (subscribeInner-ord sf mergeAllᵒ allNid κ
            (thru-outer mergeAllᵒ allNid ↠ κ) id now o sched st stR)
         (subscribeInner-pathPark sf mergeAllᵒ allNid κ κ id now o sched st
  ...
  SUBd = ≤-trans (proj₁ (proj₂ (proj₂ (proj₂ SUB₀)))) upS
  SUBn = ≤-trans (proj₁ (proj₂ (proj₂ (proj₂ (proj₂ SUB₀))))) (⊔-mono-≤ (≤-refl {nodesMax st}) upS)
...
innerFinish-nest : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {s}
  ...
  -- AND THE TWO THE DRAIN UNDER THIS FRAME SPENDS.  The finish descends
  -- beneath the outer's own frame, so the order reading is asked for at
  -- the chain the outer ALREADY extends; the cell's own park reading is
...
... | just (take-st _)       | _ = 0 , z≤n , raiseN (Caps.cSize c) W (nodesMax st ⊔ nestDᵛˢ vals) (nestU (Caps.cSize c) (nestUnit e sl)) , ≤-trans (m≤n⊔m _ (nestDᵛˢ vals)) (raiseN (Caps.cSize c) W _ (nestU (Caps.cSize c) (nestUnit e sl)))
... | just (switch-st _ _)   | _ = 0 , z≤n , raiseN (Caps.cSize c) W (nodesMax st ⊔ nestDᵛˢ vals) (nestU (Caps.cSize c) (nestUnit e sl)) , ≤-trans (m≤n⊔m _ (nestDᵛˢ vals)) (raiseN (Caps.cSize c) W _ (nestU (Caps.cSize c) (nestUnit e sl)))
... | just (exhaust-st _ _)  | _ = 0 , z≤n , raiseN (Caps.cSize c) W (nodesMax st ⊔ nestDᵛˢ vals) (nestU (Caps.cSize c) (nestUnit e sl)) , ≤-trans (m≤n⊔m _ (nestDᵛˢ vals)) (raiseN (Caps.cSize c) W _ (nestU (Caps.cSize c) (nestUnit e sl)))
... | just (mergeAll-st {w} lim act q od) | stQ′ with w ≟ᵗ s
...
abstract
  stepFrame-nodes : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {s u}
    ...
    ⦃ _ : FaceOK c sl ⦄ →
    Sched.slots sched ≡ sl →
    1 ≤ W → length vals ≤ W → capsOK? (frameStep Lv c) sched st ≡ true →
    all (valCaps? (frameStep Lv c) sl s) vals ≡ true →
    slotsSize sl ≤ Caps.cSize (frameStep Lv c) →
    ...
    pathStrat? (f ↠ p) ≡ true →
    valsStrat? (pathFloor p) vals ≡ true →
    -- AND THE TWO REGISTRY READINGS AT THE CHAIN THIS FRAME ALREADY
    -- SITS ON, which only the inner descent spends: it re-reads the
    -- outer's own frame beneath itself, and the order reading at a
    -- `from-inner` is exactly the reading at the `thru-outer` it
    -- flattened out of.
    ...
    × ((nodesMax (proj₂ (proj₂ (proj₂ (proj₂ r)))) ⊔ nestDᵛˢ (proj₁ r))
         ≤ nestFac S′ W
           * (frameNestF f ^ W * ((nodesMax st ⊔ nestDᵛˢ vals) + W * frameNestD f)
              + nestU S′ (nestUnit e sl)))
  stepFrame-nodes {e = e} c d W sl Lv sf id now (map-f fn) p vals fin sched st hsl 1≤W hlen hc hv hss hfc hfd hfw h1S hdp hpk hpl _ _ _ _ hlv hw =
    0 , z≤n ,
  ...
  stepFrame-nodes {e = e} c d W sl Lv sf id now (scan-f fn nid) p vals fin sched st hsl 1≤W hlen hc hv hss hfc hfd hfw h1S hdp hpk hpl _ _ _ _ hlv hw =
    0 , z≤n ,
    ≤-trans (stepFrame-nodes-scan W sf id now fn nid p vals fin sched st hlen)
            (raiseN (Caps.cSize c) W _ (nestU (Caps.cSize c) (nestUnit e sl)))
  stepFrame-nodes {e = e} c d W sl Lv sf id now (take-f nid) p vals fin sched st hsl 1≤W hlen hc hv hss hfc hfd hfw h1S hdp hpk hpl _ _ _ _ hlv hw =
    0 , z≤n ,
...
--
-- AND THE PATH'S OWN PRICING IS A CONJUNCT HERE, AT THE WALK'S LEVEL,
-- because the frame lemma underneath wants it there and nothing can
-- lift it to there.  A frame's nodes bound is stated wholly in the
-- levelled cap -- its caps receipt, its values, its closure key -- and
-- the path receipt beside them used to be the ENTRY cap's, raised by a
-- level lemma at the call.  That raise is what tied the whole nodes
-- face to an entry-cap reading of the registry, since the paths a sink
-- walks come OUT of the registry: a registration priced at the level
-- it was made at could never be walked from zero.  Carried here the
-- receipt arrives already at the level the frame asks for, and the
-- caller supplies it from the same bundle it supplies the caps receipt
-- from.
--
-- WHICH FREES THE DELIVERY MEASURES TO BE READ AT THE EXIT CAP, and
-- that is where they were already stated.  `deliverLen`, `fanLen` and
-- the fan squares in this face's conclusions all name `ac`; only their
-- path premises named the entry cap, so weakening those to `ac` costs
...
mutual
  ...
  capsWalkOK {u = u} c ac sl d Lv sf gas id now (f ↠ p) vals fin sched st =
    ...
    -- AND THE TWO REGISTRY READINGS AT THE CHAIN THIS WALK STANDS ON.
    -- `capsOK?` holds both of the REGISTRY, and the walk's chain is a
    -- registry chain descended through frames, so the link is made once
    -- where the walk starts rather than re-derived at every frame --
    -- which is refuted, since a caps-legal state need not be a BUILT
    -- one.  They sit here and not in the caps receipt for the reason
    -- `frameStrat?` does: the recursion below carries them to the
    -- stepped state, which is the only place the descent can read them.
    × (pathOrd? (Sched.nextNode sched) (f ↠ p) ≡ true)
    × (pathPark? (f ↠ p) st ≡ true)
    × (Σ ℕ λ L′ →
  ...
  dispatchCapsOK c ac sl d Lv sf (suc gas) id now i vals fin sched st =
    (admSz? (Caps.cSize ac) (shareAdmit i (EvalSt.registry st)) ≡ true)
...
-- THE PATH READING IS ALREADY IN THE WALK'S HYPOTHESES, ONE FRAME AT A
-- TIME, so no consumer of the walk has to be handed it separately.
-- Each `↠` clause records its frame's reading at the floor its TAIL is
-- read at, and both leaves are `true` outright -- which is the
-- recursion `pathStrat?` is itself defined by, so the extraction is the
-- identity on structure and mints no premise.  What makes it worth a
-- name is where the reading is SPENT: the mint's side condition sits
-- under the *All arms of the subscribe descent, many frames below the
-- walk that holds this, and a premise threaded down would be a second
-- copy of a fact the predicate already carries.
capsWalkOK-strat : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (c ac : Caps) (sl : Slots Γ) (d Lv : ℕ) (sf : Gas) (gas : ℕ) (id : Id) (now : Tick)
  (p : Path Γ u t) (vals : List (Val Γ u)) (fin : Bool)
  (sched : Sched Γ) (st : EvalSt e) →
  capsWalkOK c ac sl d Lv sf gas id now p vals fin sched st →
  pathStrat? p ≡ true
capsWalkOK-strat c ac sl d Lv sf gas id now root           vals fin sched st _ = refl
capsWalkOK-strat c ac sl d Lv sf gas id now (share-sink i) vals fin sched st _ = refl
capsWalkOK-strat c ac sl d Lv sf gas id now (f ↠ p)        vals fin sched st h =
  ∧-intro (proj₁ tl)
    (capsWalkOK-strat c ac sl d _ sf gas id now p _ _ _ _
      (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ tl))))))
  where
  tl = proj₂ (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ h))))))

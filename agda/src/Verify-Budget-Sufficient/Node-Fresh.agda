...
-- (`mergeBump`, `thruConsume`, `thruWrap`, `innerFinish`, `takeDispatch`),
...
frozen-mergeBump : ∀ {n} {Γ : Ctx n} (w nid : ℕ) (d : Bool)
  (ns : List (NodeId × NodeState Γ)) → w ≤ nid →
  ∀ k → suc k ≤ w → lookupNode k (mergeBump nid d ns) ≡ lookupNode k ns
frozen-mergeBump w nid d ns hw k hk with lookupNode nid ns
... | just (merge-st m od)   =
      frozen-setNode w nid (merge-st (if d then m else suc m) od) ns hw k hk
... | just (scan-st _)       = refl
... | just (take-st _)       = refl
...
concatDrain-fresh : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {s}
...
takeDispatch-fresh w nid vals fin sched st (just (merge-st _ _))    hn = fresh-refl
takeDispatch-fresh w nid vals fin sched st (just (concat-st _ _ _)) hn = fresh-refl
...
thruWrap-fresh w mergeᵒ nid true vs bs sched st hn
  with lookupNode nid (EvalSt.nodes st)
... | just (merge-st k _)      = fresh ≤-refl (frozen-setNode w nid
                                    (merge-st k true) (EvalSt.nodes st) hn)
... | just (scan-st _)         = fresh-refl
... | just (take-st _)         = fresh-refl
... | just (concat-st _ _ _)   = fresh-refl
... | just (switch-st _ _)     = fresh-refl
... | just (exhaust-st _ _)    = fresh-refl
... | nothing                  = fresh-refl
thruWrap-fresh w concatᵒ nid true vs bs sched st hn
  with lookupNode nid (EvalSt.nodes st)
... | just (concat-st q act _) = fresh ≤-refl (frozen-setNode w nid
                                    (concat-st q act true) (EvalSt.nodes st) hn)
... | just (scan-st _)         = fresh-refl
... | just (take-st _)         = fresh-refl
... | just (merge-st _ _)      = fresh-refl
... | just (switch-st _ _)     = fresh-refl
... | just (exhaust-st _ _)    = fresh-refl
... | nothing                  = fresh-refl
...
thruConsume-fresh w g mergeᵒ nid κ id now o sched st hw hn =
  fresh-trans (subscribeInner-fresh w g mergeᵒ nid κ id now o sched st hw)
              (fresh ≤-refl (frozen-mergeBump w nid
                 (proj₁ (proj₂ (proj₂ (proj₂ SI))))
                 (EvalSt.nodes (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ SI)))))) hn))
  where SI = subscribeInner g mergeᵒ nid κ id now o sched st
thruConsume-fresh {u = u} w g concatᵒ nid κ id now o sched st hw hn
  with lookupNode nid (EvalSt.nodes st)
... | just (concat-st {v} q true od) with v ≟ᵗ u
...   | yes refl = fresh ≤-refl (frozen-setNode w nid
                      (concat-st (q ++ o ∷ []) true od) (EvalSt.nodes st) hn)
...   | no _     = fresh-refl
thruConsume-fresh {u = u} w g concatᵒ nid κ id now o sched st hw hn
    | just (concat-st q false od) =
      fresh-trans (subscribeInner-fresh w g concatᵒ nid κ id now o sched st hw)
                  (fresh ≤-refl (frozen-setNode w nid
                     (concat-st {t = u} [] (not (proj₁ (proj₂ (proj₂ (proj₂ SI))))) od)
                     (EvalSt.nodes (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ SI)))))) hn))
      where SI = subscribeInner g concatᵒ nid κ id now o sched st
thruConsume-fresh w g concatᵒ nid κ id now o sched st hw hn | nothing = fresh-refl
thruConsume-fresh w g concatᵒ nid κ id now o sched st hw hn | just (scan-st _) = fresh-refl
thruConsume-fresh w g concatᵒ nid κ id now o sched st hw hn | just (take-st _) = fresh-refl
thruConsume-fresh w g concatᵒ nid κ id now o sched st hw hn | just (merge-st _ _) = fresh-refl
thruConsume-fresh w g concatᵒ nid κ id now o sched st hw hn | just (switch-st _ _) = fresh-refl
thruConsume-fresh w g concatᵒ nid κ id now o sched st hw hn | just (exhaust-st _ _) = fresh-refl
...
concatDrain-fresh w g allNid κ id now [] sched st hw = fresh-refl
concatDrain-fresh w g allNid κ id now (o ∷ q) sched st hw
  with proj₁ (proj₂ (proj₂ (proj₂ (subscribeInner g concatᵒ allNid κ id now o sched st))))
... | true  =
      fresh-trans SI
        (concatDrain-fresh w g allNid κ id now q
          (proj₁ (proj₂ (proj₂ (proj₂ (proj₂
            (subscribeInner g concatᵒ allNid κ id now o sched st))))))
          (proj₂ (proj₂ (proj₂ (proj₂ (proj₂
            (subscribeInner g concatᵒ allNid κ id now o sched st))))))
          (≤-trans hw (nxMono SI)))
      where SI = subscribeInner-fresh w g concatᵒ allNid κ id now o sched st hw
... | false = subscribeInner-fresh w g concatᵒ allNid κ id now o sched st hw

innerFinish-fresh w g mergeᵒ allNid inst κ id now vals sched st
                  (just (merge-st k od)) hw ha =
  fresh ≤-refl (frozen-setNode w allNid (merge-st (pred k) od) (EvalSt.nodes st) ha)
innerFinish-fresh w g mergeᵒ allNid inst κ id now vals sched st nothing hw ha = fresh-refl
innerFinish-fresh w g mergeᵒ allNid inst κ id now vals sched st (just (scan-st _)) hw ha = fresh-refl
innerFinish-fresh w g mergeᵒ allNid inst κ id now vals sched st (just (take-st _)) hw ha = fresh-refl
innerFinish-fresh w g mergeᵒ allNid inst κ id now vals sched st (just (concat-st _ _ _)) hw ha = fresh-refl
innerFinish-fresh w g mergeᵒ allNid inst κ id now vals sched st (just (switch-st _ _)) hw ha = fresh-refl
innerFinish-fresh w g mergeᵒ allNid inst κ id now vals sched st (just (exhaust-st _ _)) hw ha = fresh-refl
innerFinish-fresh {s = s} w g concatᵒ allNid inst κ id now vals sched st
                  (just (concat-st {v} q act od)) hw ha with v ≟ᵗ s
... | yes refl =
      fresh-trans CD
      ...
      where
      CD = concatDrain-fresh w g allNid κ id now q sched st hw
      DR = concatDrain g allNid κ id now q sched st
... | no _ = fresh-refl
innerFinish-fresh w g concatᵒ allNid inst κ id now vals sched st nothing hw ha = fresh-refl
innerFinish-fresh w g concatᵒ allNid inst κ id now vals sched st (just (scan-st _)) hw ha = fresh-refl
innerFinish-fresh w g concatᵒ allNid inst κ id now vals sched st (just (take-st _)) hw ha = fresh-refl
innerFinish-fresh w g concatᵒ allNid inst κ id now vals sched st (just (merge-st _ _)) hw ha = fresh-refl
innerFinish-fresh w g concatᵒ allNid inst κ id now vals sched st (just (switch-st _ _)) hw ha = fresh-refl
innerFinish-fresh w g concatᵒ allNid inst κ id now vals sched st (just (exhaust-st _ _)) hw ha = fresh-refl
...
innerFinish-fresh w g switchᵒ allNid inst κ id now vals sched st (just (merge-st _ _)) hw ha = fresh-refl
innerFinish-fresh w g switchᵒ allNid inst κ id now vals sched st (just (concat-st _ _ _)) hw ha = fresh-refl
...
innerFinish-fresh w g exhaustᵒ allNid inst κ id now vals sched st (just (merge-st _ _)) hw ha = fresh-refl
innerFinish-fresh w g exhaustᵒ allNid inst κ id now vals sched st (just (concat-st _ _ _)) hw ha = fresh-refl
...
stepFrame-fresh w g id now (scan-f fn nid) κ vals fin sched st hw hf | just (merge-st _ _) = fresh-refl
stepFrame-fresh w g id now (scan-f fn nid) κ vals fin sched st hw hf | just (concat-st _ _ _) = fresh-refl
...
subscribeE-fresh w g (scanᵉ f z b) κ id now sched st hw =
  ...
  SEf = subscribeE-fresh w g b (scan-f f (Sched.nextNode sched) ↠ κ) id now
          ...
          (installNode (Sched.nextNode sched) (scan-st (evalTm z)) st)
          (≤-trans hw (n≤1+n _))
subscribeE-fresh w g (mergeAllᵉ b) κ id now sched st hw =
  subscribeAll-fresh w g mergeᵒ (merge-st 0 false) b κ id now sched st hw
subscribeE-fresh {u = u} w g (concatAllᵉ b) κ id now sched st hw =
  subscribeAll-fresh w g concatᵒ (concat-st {t = u} [] false false) b κ id now sched st hw
...
subscribeE-fresh w g (deferᵉ body) κ id now sched st hw =
  fresh (n≤1+n _) (frozen-setNode w (Sched.nextNode sched)
                     (merge-st 0 false) (EvalSt.nodes st) hw)

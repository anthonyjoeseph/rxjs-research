...
open import Rx.Exp using
  (_≟ᵗ_; Ctx; Exp; Tm; sizeᵉ; sizeᵗ; sizeᵗˢ; input; ofᵉ; emptyᵉ; mapᵉ; takeᵉ; scanᵉ; mergeAllᵉ;
...
-- so the admitted list inherits whatever `all`-shaped pricing the
-- registry carries and inherits it at whatever bound the registry was
-- priced at.  Which bound that is is a separate question and a live
-- one; these two are the part of the sink's opening pair that does not

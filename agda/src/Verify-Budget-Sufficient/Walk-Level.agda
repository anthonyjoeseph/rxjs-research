...
postulate
  ...
  --
  --      ⚠ THE INVENTORY ABOVE WAS INCOMPLETE, AND THE THREE IT MISSED ARE
  --      THE ONES THE WALK FACE DOES NOT SHARE WITH ITS TWIN.  Found by
  --      putting `wl`'s hypothesis list beside this statement's, which is
  --      the check the inventory itself was standing in for.  `wl` runs at
  --      `d`, the slot's DEF, while every size/width hypothesis here is at
  --      `b = inputᶜ i` — and no amount of arithmetic gets from the input's
  --      size to the def's, since the def can be arbitrarily larger.  The
  --      twin does not help either: `sharedConnect-caps` TAKES both of the
  --      first two as hypotheses (`szd`, `wdd`), so its caller supplies
  --      them and its clause shows nothing.  All three have a source here
  --      anyway, and it is a hypothesis this statement already holds:
  --
  --        sizeᵉ d ≤ cSize    slotsCaps?-lookup (.Caps-Face/Part1) at i,
  --                           refined by `slotEq : sl i ≡ shared d` — the
  --                           `shared` clause of slotCaps? IS
  --                           `(sizeᵉ d ≤ᵇ B) ∧ (pWᵉ ≤ᵇ W) ∧ (innWᵉ ≤ᵇ W)`.
  --                           Stated at `c`, so frameStep-mono-j widens it
  --                           to `frameStep (suc j) c`.
  --        dWᵉ n sl d ≤ cWid  the same lookup's SECOND conjunct is `pWᵉ`,
  --                           and `pWᵉ = outWᵉ ⊔ dWᵉ`, so the ⊔-bound
  --                           gives dWᵉ.  Do NOT look for a dWᵉ lemma;
  --                           the slot predicate is stated at pWᵉ.
  --        suc (sizeᵉ d)      NOT inherited — `ops` is CHOSEN at the inner
  --          ≤ ops            call.  The twin picks `ops := Caps.cSize
  --                           (frameStep (suc j) c)` and discharges the
  --                           bound with `frameStep-size-strict-suc`; the
  --                           conclusion's own `ops` is then reached
  --                           through `connect-step`, which is the twin's
  --                           bridge and is PROVEN.
  --
  --      So job 1 is mechanical after all, and the correction is that the
  --      inventory has to be taken against `wl`'s ACTUAL telescope rather
  --      than against the twin's — the two differ in exactly these three
  --      places, and reading the twin's clause is what hid them.
  --
...
------------------------------------------------------------------
-- ONE PAYLOAD — thruConsume-caps' twenty-five clauses ⊗ the wet
-- content.  merge subscribes then bumps; concat parks or subscribes;
-- switch cuts then subscribes; exhaust drops while busy.  Every node
-- write goes through INV?-setNode/-mergeBump; switch's registry cut
...
thruConsume-walk c Ψ F Ŝ R̂ G ℓ L̂ U r̂ ŝ dep bud j g mergeᵒ nid κ bid now o
  ...
  j′   = proj₁ SI
  S1   = proj₁ (proj₂ SI)
  R    = subscribeInner g mergeᵒ nid κ bid now o sched st
...
... | just (exhaust-st _ _)  | _ | _ =
  ...
  ZW = subst (λ x → INV? Ψ (Caps.cSize (frameStep x c)) sched st ≡ true)
             (sym (+-identityʳ j)) invW
... | just (concat-st {w} q false od) | (bn , wn) | _ =
  j′ , capsOK?-setNode (frameStep (j + j′) c) nid
         (concat-st {t = u} [] (not done) od) sd₁ st₁ refl refl S1
     , proj₁ (proj₂ (proj₂ SI))
     , proj₁ (proj₂ (proj₂ (proj₂ SI)))
     , proj₁ (proj₂ (proj₂ (proj₂ (proj₂ SI))))
     , INV?-setNode Ψ (Caps.cSize (frameStep (j + j′) c)) nid
         (concat-st {t = u} [] (not done) od) sd₁ st₁ refl refl
         (proj₁ (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ SI))))))
     , proj₁ (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ SI))))))
     , proj₁ (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ SI)))))))
     , proj₁ (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ (proj₂ SI))))))))
  ...
  j′   = proj₁ SI
  S1   = proj₁ (proj₂ SI)
  R    = subscribeInner g concatᵒ nid κ bid now o sched st
  done = proj₁ (proj₂ (proj₂ (proj₂ R)))
  sd₁  = proj₁ (proj₂ (proj₂ (proj₂ (proj₂ R))))
  st₁  = proj₂ (proj₂ (proj₂ (proj₂ (proj₂ R))))
... | just (concat-st {w} q true od) | (bn , wn) | (bnW , fnW) with w ≟ᵗ u
...   | no _ =
  0 , subst (λ x → capsOK? (frameStep x c) sched st ≡ true)
            (sym (+-identityʳ j)) inv
    ...
    , subst (λ x → INV? Ψ (Caps.cSize (frameStep x c)) sched st ≡ true)
            (sym (+-identityʳ j)) invW
    , refl , refl , refl , rgs
-- THE PARK — the one write that grows a concat queue.  The caps side
-- is the caps clause verbatim; the wet side's queue bound comes from
-- the state's own INV? through the lookup (fnW), extended with the
...
...   | yes refl =
  1 , subst (λ x → capsOK? (frameStep x c) sched st₊ ≡ true) (sym lvl1) capsPark
    ...
    , refl , refl , refl , rgs
  where
  st₊ = record st { nodes = setNode nid (concat-st (q ++ o ∷ []) true od)
                              (EvalSt.nodes st) }
  lvl1 : j + 1 ≡ suc j
  lvl1 = +-comm j 1
  ...
  FN : fnCapNode Ψ (concat-st (q ++ o ∷ []) true od) ≡ true
  ...
  invPark = INV?-setNode Ψ (Caps.cSize (frameStep (suc j) c))
              nid (concat-st (q ++ o ∷ []) true od) sched st BN FN
              (INV?-widen sched st

...
--
-- ⚠ READ `depthCap`'S HEADER FIRST: REFUTED 2026-08-22
-- (`Refuted.Depth-Comp`).  THE CAP THIS MODULE'S EXPORT IS PROVED
-- AGAINST IS FALSE — not a leaf of it, the exported conclusion, at a
-- closed program with no slots and an empty path, where `depthE` reads
-- 4 against a cap of 3 and 8 against the same 3 when the map's source
-- widens.  Everything below is the decomposition of a false statement:
-- its bodies are sound reductions to leaves, two of the leaves are
-- separately refuted (`Refuted.Emit-Map`, `Refuted.Emit-Scan`), and
-- the restatement moves the CAP rather than any of them.
--
...
-- ⚠ REFUTED 2026-08-22 — `Refuted.Depth-Comp`.  THIS CAP IS FALSE, AND
-- THE ARGUMENT THAT SET IT IS THE REFUTED ONE.  The reasoning was: the
-- depth is generated by the spending arc alone, so `nestDᵉ` is the
-- currency and `sizeᵉ` is slack (measured slack — at the two μ probe
-- programs the old cap read 7 and 12 where the depth is 1 and 2, and
-- `nestDᵉ` reads 1 and 2 on the nose); and dropping the size term is
-- what makes the `*All` burst arm statable, because an emitted inner
...
-- payload precisely to cover this."
--
-- IT CHARGES NOTHING, BECAUSE IT CHARGES AT THE WRONG TERM.  The
-- product is `outWᵉ src * nestDᵗ f`, and `outWᵉ` is read at the
-- UNSUBSTITUTED source, where a bare payload variable is 0.  Put the
-- payload variable in a scan's source, re-wrap the accumulator in the
-- step, and stack two `*All` layers so the walk descends into what the
-- scan emits: `depthE` reads FOUR against this cap's THREE.
--
-- AND THE GAP IS THE PAYLOAD COUNT, so no constant repair reaches it —
-- widening the map's source from three literals to seven takes the
-- depth 4 → 8 with the cap at 3 in both.  The cap is read off the
-- syntax; the depth is the count the RUN delivers; the count is a
-- WIDTH, and no syntactic term bounds it.  Restoring `sizeᵉ` does not
-- repair it either: the burst arm reads the cap at the PAYLOAD, whose
-- size and nesting both exceed the emitter's.  The exported widening
-- `cap-≤-store` re-admits `sizeᵉ` and still covers these two
...
postulate
  ...
  --
  -- DEAD ROUTE 2026-08-22: CONSUME THE CAPS FACE.  `subscribeE-caps`
  -- already concludes a `burstCaps?` on exactly this burst, so reading the
  -- payload's bound off it is the first thing anyone tries.  It is
  -- CIRCULAR and not merely awkward: that face takes `depthE g b κ … ≤
  -- dep` as a premise, because its own `*All` recursion needs a level
  -- count, and `depth-compositional` is what supplies such a bound.  The
  -- same kills conditioning this face on `capsOK?` at the state: a
  -- recursion into an emitted payload runs at the REACHED state, so the
  -- state predicate would have to be re-established there, and
  -- re-establishing it is that face's conclusion.
  --
  -- THE ROUTE THAT AVOIDS IT IS SUBJECT-SIDE ONLY, and it is available
  -- because this cap already stopped reading the node store
  -- (`depthCapN`'s header).  Carry caps hypotheses about the SUBJECT and
  -- the SLOTS — `sizeᵉ b`, `dWᵉ b`, and the slot caps — and no state
  -- predicate at all; the slots are preserved unconditionally
  -- (`subscribeE-slots`), and the payload's own subject-side measures are
  -- what the restated leaf reports, at a `frameStep`-inflated level the
  -- way `wid-lift` reads one.  Then nothing needs re-establishing at a
  -- state and the circularity does not arise.
  --
  -- AND ITS ONE MISSING INGREDIENT IS NAMEABLE: `nest-subΘ`, the nesting
  -- mirror of `wid-subΘ`, bounding `nestDᵉ sl (subΘExp Θloc σ e)` at an
  -- iterated count in `sizeᵉ e` seeded by the environment's nesting AND
  -- its width.  The width seed is the whole point — the count factor the
  -- third witness moves is `outWᵉ` of the substituted source, and
  -- `wid-subΘ` bounds exactly that, PROVEN.  The map and scan clauses of
  -- the leaf then close on `applyFn-iterSize` and `applyFn-iterFold`,
  -- both proven, plus that one new induction.
...
-- the collapse to 1.
--
-- ⚠ AND THE ROWS' SHAPES ARE THE LIMIT OF WHAT THEY SAY.  "Equals
-- `depthE` on the nose" holds where the payload count the product term
-- reads is the count the RUN delivers — which is every program the
-- rows walk, because none of them substitutes a payload into a scan's
-- SOURCE.  Do that and the two counts part company: the count is read
-- at the unsubstituted source, where a bare variable is 0, and the
-- measure under-reads by the substituted value's WIDTH
-- (`Refuted.Depth-Comp`).  The equality is a property of the probed
-- shapes, not of the measure.
--

...
----------------------------------------------------------------------
-- SERIES Ψ / Ψ2 / Ψ3 WERE HERE AND ARE DELETED (2026-08-15).
--
-- They probed whether evaluation can inflate fnCap past Ψ — the fnCap
-- analogue of the hop refutation.  All three came back green, and all
-- three were STRUCTURALLY INCAPABLE of coming back anything else: in
-- every one the emitted value is a SUBTERM of the def it flows from, and
-- fnCapᵉ is a ⊔-fold over subterms, so the bound holds by construction
-- rather than by measurement.  That is why their numbers came out EQUAL
-- rather than merely ordered.
--
-- The question they were built for is settled, and by proof rather than
-- probe: `caseW-subΘ` (caseWᵗ is substitution-invariant, because reify
-- images carry no caseᵗ) and `fnCap-subΘᵉ` (EnvFnCap Ψ σ → fnCapᵉ e ≤ Ψ →
-- fnCapᵉ (subΘExp …) ≤ Ψ), both in .Measures, both already proven when
-- these series were written.  Nobody grepped for them first.
--
--   RECOVERY: git show 7c01c28 restores all three.
----------------------------------------------------------------------

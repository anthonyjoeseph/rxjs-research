...
----------------------------------------------------------------------
-- SERIES V (2026-08-14) — the four unrun conjuncts of input-wet at
-- the series W program: INV?, burstB?, hasDry, regsLen?.
--
-- INSTANTIATION.
--   r    := subscribeE (gs g0) prog-W root 0 0 schedᵂ stᵂ
--   c    := caps 5 0 0  (cSize = slotsSize insᵂ = 5, cWid = 0, cReg = 0)
--   j    := 0;  j′ := 0  →  frameStep (0+0) (caps 5 0 0) = caps 5 0 0
--   Ψ    := 0   (= slotsFnCap insᵂ = fnCapᵉ defᵂ = 0)
--   ℓ    := 0   (any works; exit registry is empty)
--   B    := Caps.cSize (frameStep 0 (caps 5 0 0)) = 5
--
-- HOW B = 5 IS CHOSEN.  slotsSize insᵂ = sizeᵉ defᵂ = 5.  INV?'s last-
-- but-one conjunct is `slotsSize (Sched.slots exit_sched) ≤ᵇ B`, and
-- the exit sched carries insᵂ unchanged (ofᵉ does not alter slots).
-- B = 4 fails that conjunct; B = 5 is the tight minimum (pinned below).
--
-- WHY Ψ = 0.  fnCapᵉ defᵂ = fnCapᵗˢ [strmᵗ Yᵂ] = (caseWᵗ(strmᵗ Yᵂ) ⊔
-- fnCapᵗ(strmᵗ Yᵂ)) ⊔ 0 = (0 ⊔ fnCapᵉ Yᵂ) ⊔ 0 = fnCapᵉ(mergeAllᵉ emptyᵉ)
-- = 0.  slotsFnCap insᵂ = 0.  So Ψ = 0 is already exact; the Ψ axis of
-- both INV? and burstB? is degenerate at this program.
--
-- EXIT STATE STRUCTURE.  subscribeSharedSlot routes to sharedConnect
-- because connectedShares is empty.  sharedConnect calls subscribeE g0
-- (ofᵉ [strmᵗ Yᵂ]), which fires oneShotBurst, producing a burst that
-- contains complete → burstCompleted = true.  The then-branch drops the
-- one registry entry (register 0 root) via dropSource 0.  Exit:
--   · registry = []         (dropped; conjunct 5 is degenerate here)
--   · nodes    = []         (ofᵉ creates none)
--   · live     = []         (oneShotBurst does not add live sources)
-- stBounded? and fnCapBounded? are vacuously true over empty lists.
--
-- CONJUNCT 4 (hasDry) already pinned in series W (line 1037):
--   hasDry (proj₁ (subscribeE (gasPad 1 g0) prog-W root 0 0 schedᵂ stᵂ))
--          ≡ false.
-- Not repeated; cited as the conjunct-4 receipt.
--
-- REGION REACHED.
--   · conjunct 1 (INV?): the slotsSize conjunct (5 ≤ᵇ B) is load-bearing
--     — B = 4 fails (pinned below).  All other INV? conjuncts are vacuous
--     over empty lists; the Ψ axis is degenerate (slotsFnCap insᵂ = 0).
--   · conjunct 2 (burstB?): the emitted value Yᵂ has sizeᵉ = 2; B = 1
--     fails the size conjunct (pinned below).  The Ψ axis is degenerate
--     (fnCapᵉ Yᵂ = 0).
--   · conjunct 4 (hasDry): series W receipt, gs g0 suffices.
--   · conjunct 5 (regsLen?): DEGENERATE — exit registry is empty at this
--     program.  Non-vacuous coverage exists in the P-series (P-a through
--     P-COMP2) but NOT co-instantiated with INV?/burstB?.
--
-- NOT REACHED: non-trivial exit registry requires deferᵉ inside defᵂ's
-- body to survive the connect burst; non-zero Ψ requires a function-
-- valued slot def (e.g. scanᵉ in the def).  Those shapes would each
-- need a new series analogous to P-S1 but with INV?/burstB? as the
-- measured conjuncts.
----------------------------------------------------------------------

-- CALIBRATION PINS — the two numerals the instantiation hangs on.
-- LOAD-BEARING: each fails if sizeᵉ or slotsSize formula changes.
_ : slotsSize insᵂ ≡ 5
_ = refl
...
-- CONJUNCT 1: INV? at Ψ = 0, B = 5 (tight on the slotsSize conjunct)
-- LOAD-BEARING: B = 4 gives false (pinned immediately below).
_ : let r = subscribeE (gs g0) prog-W root 0 0 schedᵂ stᵂ
    in INV? 0 5 (proj₁ (proj₂ r)) (proj₂ (proj₂ r)) ≡ true
_ = refl

-- contrast: B = 4 fails — slotsSize insᵂ = 5 > 4.
-- LOAD-BEARING (confirms the row above is not degenerate on B).
_ : let r = subscribeE (gs g0) prog-W root 0 0 schedᵂ stᵂ
    in INV? 0 4 (proj₁ (proj₂ r)) (proj₂ (proj₂ r)) ≡ false
_ = refl

-- CONJUNCT 2: burstB? at B = 5, Ψ = 0
-- LOAD-BEARING on B (B = 1 fails; pinned below). DEGENERATE on Ψ.
_ : let r = subscribeE (gs g0) prog-W root 0 0 schedᵂ stᵂ
    in burstB? 5 0 (proj₁ r) ≡ true
_ = refl

-- contrast: B = 1 fails — sizeᵉ Yᵂ = 2 > 1.
-- LOAD-BEARING (confirms the burst carries a value that imposes a size bound).
_ : let r = subscribeE (gs g0) prog-W root 0 0 schedᵂ stᵂ
    in burstB? 1 0 (proj₁ r) ≡ false
_ = refl

-- CONJUNCT 4 (hasDry): pinned HERE, at series V's own gas term.
--
-- The series W receipt sits at `gasPad 1 g0`, and this series runs at
-- `gs g0`.  Those are not interchangeable by inspection: the row just
-- under that receipt pins `hasDry (subscribeE g0 …) ≡ true`, i.e. the
-- connect DRIES one gas step down, so gas is load-bearing for exactly
-- this conjunct on exactly this program.  Citing the W receipt in a
-- comment would have asserted a fact about a term it was never run at
-- — one line settles it instead.
-- LOAD-BEARING: fails if `gs g0` sits at or below the dry boundary,
-- which would make series V's whole instantiation inconsistent (the
-- other conjuncts are pinned at this same gas term).
_ : hasDry (proj₁ (subscribeE (gs g0) prog-W root 0 0 schedᵂ stᵂ))
    ≡ false
_ = refl

-- CONJUNCT 5: regsLen? at ℓ = 0
-- DEGENERATE: exit registry is empty; regsLen? [] = true for any ℓ.
-- The burstCompleted=true path (sharedConnect's then-branch) calls
-- dropSource 0, removing the one registration inserted by register.
_ : regsLen? 0 (EvalSt.registry
      (proj₂ (proj₂ (subscribeE (gs g0) prog-W root 0 0 schedᵂ stᵂ)))) ≡ true
_ = refl

----------------------------------------------------------------------
-- SERIES Ψ (2026-08-15) — INV? and burstB? at a POSITIVE slotsFnCap.
--
-- MOTIVATION.  Series V held Ψ = 0 at every row: fnCapᵉ (input i) = 0
-- and fnCapᵉ defᵂ = 0 (defᵂ = ofᵉ [strmᵗ (mergeAllᵉ emptyᵉ)], no
-- caseᵗ anywhere).  The only path to positive Ψ is a slot def whose
-- mapᵉ function uses caseᵗ (caseWᵗ (caseᵗ s l r) = 2 + ...; all other
-- formers give 0).  This series builds exactly that.
--
-- PROGRAM.  Reuse Γᵂ / prog-W = input zero.  Replace the slot def:
--   defΨ = ofᵉ [strmᵗ (mapᵉ fΨ eΨ)]
--   fΨ   = caseᵗ (varᵗ (here refl)) (varᵗ (here refl)) (varᵗ (here refl))
--          : Tm Γᵂ [] [] ((natᵗ +ᵗ natᵗ) ∷ []) natᵗ   — the case function
--   eΨ   = ofᵉ [inlᵗ (nat̂ 0)]
--          : Exp Γᵂ [] [] [] (natᵗ +ᵗ natᵗ)
--
-- KEY COMPUTATIONS.
--   fnCapᵉ defΨ = fnCapᵗˢ [strmᵗ mapFΨ]
--               = (caseWᵗ(strmᵗ mapFΨ) ⊔ fnCapᵗ(strmᵗ mapFΨ)) ⊔ 0
--               = (0 ⊔ fnCapᵉ mapFΨ) ⊔ 0
--               = fnCapᵉ mapFΨ
--               = (caseWᵗ fΨ ⊔ fnCapᵗ fΨ) ⊔ fnCapᵉ eΨ
--               = (2 ⊔ 0) ⊔ 0 = 2.
--   slotsFnCap insΨ = fnCapᵉ defΨ = 2.
--   ΨAt prog-W insΨ = fnCapᵉ prog-W + slotsFnCap insΨ = 0 + 2 = 2.
--
--   sizeᵉ defΨ = suc (sizeᵗ(strmᵗ mapFΨ) + sizeᵗˢ [])
--              = suc (suc(sizeᵉ mapFΨ) + 1)
--              = suc (suc(suc(sizeᵗ fΨ + sizeᵉ eΨ)) + 1)
--              = suc (suc(suc(4 + 4)) + 1) = suc (suc 9 + 1)
--              = suc (10 + 1) = 12.
--   slotsSize insΨ = sizeᵉ defΨ = 12.
--   (Note: sizeᵗˢ [] = 1, not 0 — the base case of sizeᵗˢ.)
--
--   The burst from defΨ's oneShotBurst contains [init, value mapFΨ,
--   close, complete].  The value emitted is:
--     sizeᵛ (obs natᵗ) mapFΨ = sizeᵉ mapFΨ = 9.
--     fnCapᵛ (obs natᵗ) mapFΨ = fnCapᵉ mapFΨ = 2.
--   So valB? B Ψ (obs natᵗ) mapFΨ = (9 ≤ᵇ B) ∧ (2 ≤ᵇ Ψ).
--
-- MINIMUM REQUIREMENTS.
--   INV?:    slotsSize conjunct needs B ≥ 12; slotsFnCap conjunct Ψ ≥ 2.
--   burstB?: size conjunct needs B ≥ 9; fnCap conjunct needs Ψ ≥ 2.
--   Combined: B = 12, Ψ = 2 (tight minimum for both).
--
-- EXIT STATE (identical path to series W/V — same program + ofᵉ def):
--   registry = [] (dropSource 0 removes the one entry)
--   nodes = [], live = [] (ofᵉ creates none)
-- stBounded?/fnCapBounded?/regsB? are vacuously true over empty lists.
--
-- NEGATIVE CONTROLS.
--   INV? B-axis: B=11, Ψ=2 → slotsSize conjunct false (12 > 11).
--   Ψ-axis: B=12, Ψ=1 → slotsFnCap conjunct false (2 > 1) for INV?
--            AND fnCap conjunct false (2 > 1) for burstB?.
--   burstB? B-axis: B=8, Ψ=2 → size conjunct false (9 > 8).
--
-- REGION REACHED.
--   · INV? Ψ-axis: slotsFnCap insΨ = 2 > 0 is the gap series V missed.
--     Negative control at Ψ=1 is LOAD-BEARING: confirms the conjunct
--     does distinguish Ψ=1 from Ψ=2.
--   · burstB? Ψ-axis: fnCap of the emitted observable = 2. Negative
--     control at Ψ=1 confirms the fnCap conjunct fires.
--   · burstB? B-axis: size of the emitted observable = 9. Negative at
--     B=8 confirms the size conjunct fires.
--   · INV? B-axis: slotsSize = 12 is the slot-size constraint. Negative
--     at B=11 confirms it.
--
-- NOT REACHED.
--   · Non-empty exit registry (regsLen? non-vacuous): same limitation as
--     series V — only deferᵉ in the def would keep a registry entry.
--   · fnCapBounded? / stBounded? non-vacuous: empty live/nodes throughout.
--   · The refutation target: fnCapᵛ of the emitted value (2) does NOT
--     exceed slotsFnCap insΨ (2); they are equal, and burstB? holds.
--     So this is GREEN at the real Ψ = ΨAt prog-W insΨ = 2.
--
-- ⚠ AND THAT ROW COULD NOT HAVE REFUTED — read the boundary before
-- reading the green (design-session review, 2026-08-15).  defΨ contains
-- NO `input`: it is mapᵉ/ofᵉ over literals on a ONE-slot telescope.  So
-- the emitted value is a SUBTERM of the def, and fnCapᵉ is a ⊔-fold over
-- subterms — a subterm's fnCap is bounded by the whole's as a matter of
-- structure, which is why the two numbers came out EQUAL rather than
-- merely ordered.  Equality here is the signature of a test that had no
-- room to fail, not of a tight bound discovered.
--
-- THE DANGEROUS CONFIGURATION IS THE CROSS-SLOT ONE, and it is untested.
-- Let slot 1's def READ `input 0` where slot 0's def has positive fnCap.
-- Then values flowing out of slot 1 are NOT subterms of slot 1's def,
-- the ⊔-fold argument does not reach them, and `fnCapᵉ (input i) = 0`
-- means slot 1's own summand accounts for exactly none of it.  What is
-- supposed to cover it is that `slotsFnCap` SUMS, so slot 0's fnCap is a
-- separate summand of ΨAt — the same division of labour the hop
-- refutation broke, since hop's environment had no such summand and its
-- `input` clause had to be parameterised by η instead.
--
-- This is the exact shape of the series W → series T gap: W's fixpoint
-- rows sat at slot 0, where the staging is vacuous, and T had to reach
-- k ≥ 1 before the probe meant anything.  Series Ψ is at "slot 0" for
-- fnCap.  A series reaching the cross-slot case is what would settle
-- input-wet's Ψ axis; until then this receipt covers the within-def half.
----------------------------------------------------------------------

-- The case function: case-match on natᵗ +ᵗ natᵗ, project the bound nat.
-- caseWᵗ fΨ = 2 (the minimum non-zero; both branches are variables).
fΨ : Tm Γᵂ [] [] ((natᵗ +ᵗ natᵗ) ∷ []) natᵗ
fΨ = caseᵗ (varᵗ (here refl)) (varᵗ (here refl)) (varᵗ (here refl))

-- Inner source: emits one sum value.
eΨ : Closed Γᵂ (natᵗ +ᵗ natᵗ)
eΨ = ofᵉ (inlᵗ (nat̂ 0) ∷ [])

-- The mapped expression: fnCapᵉ = 2.
mapFΨ : Closed Γᵂ natᵗ
mapFΨ = mapᵉ fΨ eΨ

-- Slot def: emits the observable mapFΨ as a stream value.
-- fnCapᵉ defΨ = 2; sizeᵉ defΨ = 12 (see header for derivations).
defΨ : Closed Γᵂ (obs natᵗ)
defΨ = ofᵉ (strmᵗ mapFΨ ∷ [])

-- Slots: same Γᵂ, fnCap-heavy def.
insΨ : Slots Γᵂ
insΨ = λ { zero → shared defΨ }

schedΨ : Sched Γᵂ
schedΨ = sched-init prog-W insΨ

stΨ : EvalSt prog-W
stΨ = st-init prog-W

-- CALIBRATION PINS — fail if the fnCap or size formula changes.
-- LOAD-BEARING: each pins a numeral the probe rows depend on.
_ : slotsFnCap insΨ ≡ 2
_ = refl
...
-- ΨAt row: confirms the real Ψ seed for prog-W + insΨ.
-- LOAD-BEARING: fails if fnCapᵉ (input i) becomes nonzero.
_ : ΨAt prog-W insΨ ≡ 2
_ = refl

-- CONJUNCT 1 (INV?): B = 12, Ψ = ΨAt prog-W insΨ = 2.
-- LOAD-BEARING on both axes (negative controls below).
_ : let r = subscribeE (gs g0) prog-W root 0 0 schedΨ stΨ
    in INV? 2 12 (proj₁ (proj₂ r)) (proj₂ (proj₂ r)) ≡ true
_ = refl

-- INV? B-axis negative control: B = 11 fails the slotsSize conjunct
-- (slotsSize insΨ = 12 > 11).
-- LOAD-BEARING: confirms row above is not degenerate on B.
_ : let r = subscribeE (gs g0) prog-W root 0 0 schedΨ stΨ
    in INV? 2 11 (proj₁ (proj₂ r)) (proj₂ (proj₂ r)) ≡ false
_ = refl

-- INV? Ψ-axis negative control: Ψ = 1 fails the slotsFnCap conjunct
-- (slotsFnCap insΨ = 2 > 1).  THIS IS THE Ψ-AXIS GAP series V missed.
-- LOAD-BEARING: confirms the slotsFnCap conjunct is live at this program.
_ : let r = subscribeE (gs g0) prog-W root 0 0 schedΨ stΨ
    in INV? 1 12 (proj₁ (proj₂ r)) (proj₂ (proj₂ r)) ≡ false
_ = refl

-- CONJUNCT 2 (burstB?): B = 12, Ψ = 2.
-- LOAD-BEARING on both axes (negative controls below).
-- The value emitted is mapFΨ : obs natᵗ with fnCap = 2, size = 9.
_ : let r = subscribeE (gs g0) prog-W root 0 0 schedΨ stΨ
    in burstB? 12 2 (proj₁ r) ≡ true
_ = refl

-- burstB? Ψ-axis negative control: Ψ = 1 fails the fnCap conjunct
-- (fnCapᵛ (obs natᵗ) mapFΨ = 2 > 1).
-- LOAD-BEARING: confirms burstB? sees the emitted observable's fnCap.
_ : let r = subscribeE (gs g0) prog-W root 0 0 schedΨ stΨ
    in burstB? 12 1 (proj₁ r) ≡ false
_ = refl

-- burstB? B-axis negative control: B = 8 fails the size conjunct
-- (sizeᵉ mapFΨ = 9 > 8).
-- LOAD-BEARING: confirms burstB? sees the emitted observable's size.
_ : let r = subscribeE (gs g0) prog-W root 0 0 schedΨ stΨ
    in burstB? 8 2 (proj₁ r) ≡ false
_ = refl

----------------------------------------------------------------------
-- SERIES Ψ2 (2026-08-15) — THE CROSS-SLOT CASE: slot 0 has positive
-- fnCap (defΨ body), slot 1 reads input 0.  Program subscribes
-- through slot 1.  Uses context Γᵁ and reuses dᵁ1.
--
-- UNVERIFIED — no agda-dev run (machine owns a live make agda).
--
-- PURPOSE: test whether the slotsFnCap SUM mechanism covers a value
-- emitted through slot 1 when slot 0 (not slot 1) holds the fnCap-
-- bearing def.  slotsFnCap SUMS all slots, so slot 0's fnCap 2 is
-- its own summand of ΨAt even when the program routes through slot 1.
-- These rows are the evidence for that reading.
--
-- KEY NUMBERS (hand-verified, no agda-dev):
--   fnCapᵉ d0Ψ2 = 2  (defΨ body: caseWᵗ fΨ2 = 2; all others 0)
--   fnCapᵉ dᵁ1  = 0  (mergeAllᵉ (input i): no caseᵗ)
--   slotsFnCap insΨ2 = 2 + 0 = 2
--   sizeᵉ d0Ψ2  = 12 (same derivation as defΨ; Γᵁ vs Γᵂ only types)
--   sizeᵉ dᵁ1   = 5  (suc(sizeᵗˢ [strmᵗ (mergeAllᵉ (input 0))]) =
--                      suc(3 + 1) = 5)
--   slotsSize insΨ2  = 12 + 5 = 17
--   fnCapᵉ progΨ2    = fnCapᵉ (input (F.suc zero)) = 0
--   ΨAt progΨ2 insΨ2 = 0 + 2 = 2
--
-- EMITTED VALUE through slot 1: mergeAllᵉ (input zero).
--   IS a SUBTERM of dᵁ1 = ofᵉ (strmᵗ (mergeAllᵉ (input zero)) ∷ []).
--   fnCapᵛ = fnCapᵉ (mergeAllᵉ (input zero)) = 0.
--   sizeᵛ  = sizeᵉ (mergeAllᵉ (input zero)) = 2.
-- Consequence: burstB? Ψ-axis is DEGENERATE (0 ≤ᵇ Ψ always).
-- INV? Ψ-axis IS LOAD-BEARING: slotsFnCap = 2 fails at Ψ=1.
-- NOTE: the Ψ-axis INV? gate tests slot 0's contribution to the sum,
-- NOT the emitted value's own fnCap — that is zero and degenerate.
-- FINDING: GREEN.  The cross-slot fnCap is covered via the sum.
----------------------------------------------------------------------

-- Slot-0 function: same body as fΨ, typed at the two-slot context Γᵁ.
-- caseWᵗ fΨ2 = 2; fnCapᵗ fΨ2 = 0 (varᵗ branches contribute nothing).
fΨ2 : Tm Γᵁ [] [] ((natᵗ +ᵗ natᵗ) ∷ []) natᵗ
fΨ2 = caseᵗ (varᵗ (here refl)) (varᵗ (here refl)) (varᵗ (here refl))

eΨ2 : Closed Γᵁ (natᵗ +ᵗ natᵗ)
eΨ2 = ofᵉ (inlᵗ (nat̂ 0) ∷ [])

mapFΨ2 : Closed Γᵁ natᵗ
mapFΨ2 = mapᵉ fΨ2 eΨ2

-- Slot-0 def: fnCapᵉ d0Ψ2 = 2; sizeᵉ d0Ψ2 = 12.
-- (Same term structure as defΨ; broader context Γᵁ only affects typing.)
d0Ψ2 : Closed Γᵁ (obs natᵗ)
d0Ψ2 = ofᵉ (strmᵗ mapFΨ2 ∷ [])

-- Slot-1 def: dᵁ1 (defined in series T, reused here directly).
-- fnCapᵉ dᵁ1 = 0; sizeᵉ dᵁ1 = 5.

insΨ2 : Slots Γᵁ
insΨ2 = λ { zero → shared d0Ψ2 ; (F.suc zero) → shared dᵁ1 }

progΨ2 : Closed Γᵁ (obs natᵗ)
progΨ2 = input (F.suc zero)

schedΨ2 : Sched Γᵁ
schedΨ2 = sched-init progΨ2 insΨ2

stΨ2 : EvalSt progΨ2
stΨ2 = st-init progΨ2

-- CALIBRATION PINS — fail if fnCap, size, or ΨAt formula changes.
-- LOAD-BEARING: each pins a numeral the probe rows depend on.
_ : slotsFnCap insΨ2 ≡ 2
_ = refl

_ : slotsSize insΨ2 ≡ 17
_ = refl

_ : ΨAt progΨ2 insΨ2 ≡ 2
_ = refl

-- CONJUNCT 1 (INV?): B = 17, Ψ = ΨAt progΨ2 insΨ2 = 2.
-- LOAD-BEARING on both axes (negative controls below confirm).
_ : let r = subscribeE (gs g0) progΨ2 root 0 0 schedΨ2 stΨ2
    in INV? 2 17 (proj₁ (proj₂ r)) (proj₂ (proj₂ r)) ≡ true
_ = refl

-- INV? B-axis negative: B=16 fails slotsSize conjunct (17 > 16).
-- LOAD-BEARING: confirms B is not vacuously unconstrained.
_ : let r = subscribeE (gs g0) progΨ2 root 0 0 schedΨ2 stΨ2
    in INV? 2 16 (proj₁ (proj₂ r)) (proj₂ (proj₂ r)) ≡ false
_ = refl

-- INV? Ψ-axis negative: Ψ=1 fails slotsFnCap conjunct (2 > 1).
-- LOAD-BEARING: confirms cross-slot slotsFnCap sum is live at Ψ.
_ : let r = subscribeE (gs g0) progΨ2 root 0 0 schedΨ2 stΨ2
    in INV? 1 17 (proj₁ (proj₂ r)) (proj₂ (proj₂ r)) ≡ false
_ = refl

-- CONJUNCT 2 (burstB?): B = 17, Ψ = 2.
-- Emitted value: mergeAllᵉ (input zero), a SUBTERM of dᵁ1.
--   fnCapᵛ = 0 — Ψ-axis is DEGENERATE (0 ≤ᵇ Ψ always true).
--   sizeᵛ  = 2.
-- LOAD-BEARING on B-axis only.
_ : let r = subscribeE (gs g0) progΨ2 root 0 0 schedΨ2 stΨ2
    in burstB? 17 2 (proj₁ r) ≡ true
_ = refl

-- burstB? B-axis negative: B=1 fails size conjunct (sizeᵛ=2 > 1).
-- LOAD-BEARING: confirms the emitted observable's size is measured.
_ : let r = subscribeE (gs g0) progΨ2 root 0 0 schedΨ2 stΨ2
    in burstB? 1 2 (proj₁ r) ≡ false
_ = refl

----------------------------------------------------------------------
-- SERIES Ψ3 (2026-08-15) — burstB? Ψ-AXIS NON-DEGENERATE, CROSS-SLOT.
-- Two slots (Γᵁ): slot 0 keeps d0Ψ2 (fnCap 2); slot 1 is new def
-- d1Ψ3 whose emitted value contains caseᵗ → fnCapᵛ = 2 > 0.
--
-- UNVERIFIED — no agda-dev run (machine owns a live make agda).
--
-- PURPOSE: confirm burstB? Ψ-axis is non-degenerate once the emitted
-- value's fnCap is positive.  Series Ψ/Ψ2 had fnCapᵛ = 0 (degenerate).
-- Here fnCapᵛ = 2, giving a load-bearing negative at Ψ=1.
--
-- KEY NUMBERS (hand-verified):
--   f3Ψ = caseᵗ (inlᵗ {t=natᵗ} (nat̂ 0)) (nat̂ 0) (nat̂ 0)
--   caseWᵗ f3Ψ = 2; fnCapᵗ f3Ψ = 0
--   mapF3Ψ = mapᵉ f3Ψ (input zero) : Closed Γᵁ natᵗ
--   fnCapᵉ mapF3Ψ = (2⊔0)⊔0 = 2          (the emitted value)
--   sizeᵉ mapF3Ψ = suc(5+1) = 7           (sizeᵗ f3Ψ = suc(2+1+1) = 5)
--   fnCapᵉ d1Ψ3  = (0⊔fnCapᵉ mapF3Ψ) = 2
--   sizeᵉ d1Ψ3   = suc(sizeᵗˢ [strmᵗ mapF3Ψ]) = suc(8+1) = 10
--   slotsFnCap insΨ3 = fnCapᵉ d0Ψ2 + fnCapᵉ d1Ψ3 = 2 + 2 = 4
--   slotsSize insΨ3  = sizeᵉ d0Ψ2 + sizeᵉ d1Ψ3 = 12 + 10 = 22
--   fnCapᵉ (input (F.suc zero)) = 0
--   ΨAt progΨ3 insΨ3 = 0 + 4 = 4
--
-- EMITTED VALUE from slot 1: mapF3Ψ.
--   IS a SUBTERM of d1Ψ3 = ofᵉ [strmᵗ mapF3Ψ].
--   fnCapᵛ (obs natᵗ) mapF3Ψ = fnCapᵉ mapF3Ψ = 2.
--   sizeᵛ (obs natᵗ) mapF3Ψ  = sizeᵉ mapF3Ψ  = 7.
-- The ⊔-fold bound holds: fnCapᵛ = 2 ≤ fnCapᵉ d1Ψ3 = 2 ≤ slotsFnCap = 4.
-- So burstB? cannot fail at Ψ = ΨAt = 4.  No refutation.
-- burstB? Ψ-axis IS LOAD-BEARING: negative at Ψ=1 (fnCapᵛ=2 > 1).
-- INV?   Ψ-axis IS LOAD-BEARING: negative at Ψ=3 (slotsFnCap=4 > 3).
----------------------------------------------------------------------

-- Slot-1 function: caseᵗ on an internally-constructed inlᵗ value;
-- ignores the (obs natᵗ) argument from slot 0.  caseWᵗ f3Ψ = 2.
f3Ψ : Tm Γᵁ [] [] ((obs natᵗ) ∷ []) natᵗ
f3Ψ = caseᵗ (inlᵗ {t = natᵗ} (nat̂ 0)) (nat̂ 0) (nat̂ 0)

-- The expression that WILL BE the emitted value (type natᵗ, fnCap 2).
mapF3Ψ : Closed Γᵁ natᵗ
mapF3Ψ = mapᵉ f3Ψ (input zero)

-- Slot-1 def: emits mapF3Ψ as the obs natᵗ stream value.
-- fnCapᵉ d1Ψ3 = 2; sizeᵉ d1Ψ3 = 10.
d1Ψ3 : Closed Γᵁ (obs natᵗ)
d1Ψ3 = ofᵉ (strmᵗ mapF3Ψ ∷ [])

insΨ3 : Slots Γᵁ
insΨ3 = λ { zero → shared d0Ψ2 ; (F.suc zero) → shared d1Ψ3 }

progΨ3 : Closed Γᵁ (obs natᵗ)
progΨ3 = input (F.suc zero)

schedΨ3 : Sched Γᵁ
schedΨ3 = sched-init progΨ3 insΨ3

stΨ3 : EvalSt progΨ3
stΨ3 = st-init progΨ3

-- CALIBRATION PINS — fail if fnCap/size formula changes.
-- fnCapᵉ mapF3Ψ = 2: THE KEY PIN (non-degenerate Ψ-axis depends on this).
-- LOAD-BEARING: fails if caseWᵗ of f3Ψ changes.
_ : fnCapᵉ mapF3Ψ ≡ 2
_ = refl

_ : slotsFnCap insΨ3 ≡ 4
_ = refl

_ : slotsSize insΨ3 ≡ 22
_ = refl

_ : ΨAt progΨ3 insΨ3 ≡ 4
_ = refl

-- CONJUNCT 1 (INV?): B = 22, Ψ = ΨAt progΨ3 insΨ3 = 4.
-- LOAD-BEARING on both axes (negative controls below confirm).
_ : let r = subscribeE (gs g0) progΨ3 root 0 0 schedΨ3 stΨ3
    in INV? 4 22 (proj₁ (proj₂ r)) (proj₂ (proj₂ r)) ≡ true
_ = refl

-- INV? B-axis negative: B=21 fails slotsSize conjunct (22 > 21).
-- LOAD-BEARING: confirms B is live.
_ : let r = subscribeE (gs g0) progΨ3 root 0 0 schedΨ3 stΨ3
    in INV? 4 21 (proj₁ (proj₂ r)) (proj₂ (proj₂ r)) ≡ false
_ = refl

-- INV? Ψ-axis negative: Ψ=3 fails slotsFnCap conjunct (4 > 3).
-- LOAD-BEARING: confirms the slotsFnCap sum (both slots) is live at Ψ.
_ : let r = subscribeE (gs g0) progΨ3 root 0 0 schedΨ3 stΨ3
    in INV? 3 22 (proj₁ (proj₂ r)) (proj₂ (proj₂ r)) ≡ false
_ = refl

-- CONJUNCT 2 (burstB?): B = 22, Ψ = 4.
-- Emitted value: mapF3Ψ, IS a subterm of d1Ψ3.
--   fnCapᵛ = 2 (POSITIVE — this is the non-degenerate Ψ-axis).
--   sizeᵛ  = 7.
-- LOAD-BEARING on BOTH axes.
_ : let r = subscribeE (gs g0) progΨ3 root 0 0 schedΨ3 stΨ3
    in burstB? 22 4 (proj₁ r) ≡ true
_ = refl

-- burstB? Ψ-axis negative: Ψ=1 fails fnCap conjunct (fnCapᵛ=2 > 1).
-- LOAD-BEARING: THE NON-DEGENERATE ROW — confirms the emitted value's
-- own caseᵗ is measured by burstB?.
_ : let r = subscribeE (gs g0) progΨ3 root 0 0 schedΨ3 stΨ3
    in burstB? 22 1 (proj₁ r) ≡ false
_ = refl

-- burstB? B-axis negative: B=6 fails size conjunct (sizeᵛ=7 > 6).
-- LOAD-BEARING: confirms the emitted value's size is measured.
_ : let r = subscribeE (gs g0) progΨ3 root 0 0 schedΨ3 stΨ3
    in burstB? 6 4 (proj₁ r) ≡ false
_ = refl

----------------------------------------------------------------------
-- SERIES REG (2026-08-15) — regsLen? NON-DEGENERATE, SHARED SLOT.
-- Slot 0 def is deferᵉ at the TOP LEVEL: subscribeE is called on
-- deferᵉ directly, which calls register src → registry non-empty.
-- This is the ONLY conjunct of input-wet never run non-vacuously.
--
-- UNVERIFIED — no agda-dev run (machine owns a live make agda).
--
-- CONTEXT: Γᵂ (one obs natᵗ slot), program prog-W = input zero.
--
-- KEY MECHANISM.  defREG = deferᵉ emptyᵉ : Closed Γᵂ (obs natᵗ).
-- subscribeE fuel (deferᵉ emptyᵉ) κ id now sched st:
--   1. Mints src; schedules body emptyᵉ for tick (suc now)
--   2. Calls register src → adds entry to registry
--   3. Body is NOT delivered (deferred to tick 1; probe runs at tick 0)
-- Result: registry has one entry with pathLen = 1 + pathLen κ ≥ 1.
-- dropSource 0 removes the CONNECT entry, not the deferred entry.
-- → registry non-empty after subscribeE returns.
--
-- CRITICAL STRUCTURAL POINT: deferᵉ must be TOP-LEVEL in the slot def.
-- A deferᵉ inside strmᵗ (e.g. ofᵉ [strmᵗ (deferᵉ ...)]) is an EMITTED
-- VALUE, never subscribed → no registry entry.  Top-level → subscribed.
--
-- KEY NUMBERS (hand-verified):
--   fnCapᵉ (deferᵉ emptyᵉ) = fnCapᵉ emptyᵉ = 0
--   sizeᵉ (deferᵉ emptyᵉ)  = suc (sizeᵉ emptyᵉ) = suc 1 = 2
--   slotsFnCap insREG = 0; slotsSize insREG = 2; ΨAt prog-W insREG = 0
--
-- REGISTRY PIN (the load-bearing calibration):
--   regsLen? 0 ... ≡ false → all (pathLen ≤ᵇ 0) is false
--   → at least one entry has pathLen ≥ 1 (guaranteed: thru-outer cons ↠ κ)
--   (If registry is empty, all (pathLen ≤ᵇ 0) [] = true — so false PROVES non-empty.)
--
-- EXACT pathLen: 1 + pathLen κ where κ is the slot's continuation.
-- pathLen κ ≥ 1 (thru-share or similar), so pathLen ≥ 2.
-- Upper bound ℓ=5 is conservative; tighten once agda is available.
----------------------------------------------------------------------

-- Slot def: deferᵉ at top level → subscribeE called on deferᵉ directly.
-- fnCapᵉ defREG = 0; sizeᵉ defREG = 2.
defREG : Closed Γᵂ (obs natᵗ)
defREG = deferᵉ emptyᵉ

insREG : Slots Γᵂ
insREG = λ { zero → shared defREG }

-- Program: same as series V/W — subscribes through slot 0.
schedREG : Sched Γᵂ
schedREG = sched-init prog-W insREG

stREG : EvalSt prog-W
stREG = st-init prog-W

-- CALIBRATION PINS — hand-verified.
_ : slotsSize insREG ≡ 2
_ = refl

_ : slotsFnCap insREG ≡ 0
_ = refl

_ : ΨAt prog-W insREG ≡ 0
_ = refl

-- REGISTRY NON-EMPTY PIN — THE KEY CALIBRATION ROW.
-- regsLen? 0 registry ≡ false ↔ registry has ≥1 entry with pathLen > 0.
-- If this refl fails, the registry is empty and the series is NOT ACHIEVED.
-- LOAD-BEARING: fails if deferᵉ inside defREG does not create a registry entry.
_ : let r = subscribeE (gs g0) prog-W root 0 0 schedREG stREG
    in regsLen? 0 (EvalSt.registry (proj₂ (proj₂ r))) ≡ false
_ = refl

-- regsLen? UPPER BOUND: ℓ=5 is conservative; tighten once agda can run.
-- LOAD-BEARING: fails if pathLen > 5 (would indicate unexpected nesting).
_ : let r = subscribeE (gs g0) prog-W root 0 0 schedREG stREG

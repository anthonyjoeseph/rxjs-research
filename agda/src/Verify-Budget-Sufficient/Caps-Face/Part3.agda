...
open import Data.Nat.Properties using (≤ᵇ⇒≤; ≤-trans; ≤-refl; ≤-reflexive; +-suc; +-comm; +-assoc; +-monoˡ-≤; *-monoˡ-≤; m≤m+n;
  m≤n+m; n≤1+n; +-mono-≤; ^-monoˡ-≤; *-mono-≤; m≤m⊔n; m≤n⊔m; ⊔-lub; *-identityˡ)
...
open import Rx.Prim      using (Source; InstEmit; _at_from_as_; InstEvent; init; value; close; handoff; complete; exhausted;
  after_,_)
...
open import Verify-Budget-Sufficient.Measures using
  (all-++-intro; all-impl; len-subΘTms;
                                                      sizeᵉ-pos; sizeᵗ-pos; ∧-true)
open import Verify-Budget-Sufficient.Caps using
  (_⊑ᶜ_; Caps; foldStep-infl; frameStep; frameStep-mono-j; frameStep-size-suc;
   iterFold-infl; iterFold-mono-count; iterFold-suc; iterSize-infl; s≤2s)
...
open import Verify-Budget-Sufficient.Caps-Face.Part2 using
  (EnvW; envW-lookup; envW-mono; plug-dW; plug-iW; plug-pI; plug-pO; pWᵛ-fst;
   pWᵛ-pair; pWᵛ-snd; slotsCaps?-slotWid; SlotWid-mono; wid-iterFold)
open import Verify-Budget-Sufficient.Caps-Face.Part1 using
  (+-shuffle; 4≤iterFold;
                                                             burstCaps?; capsOK?; eventCaps?;
                                                             frameSz?; iterFold-+;
                                                             iterFold-lift; iterFold-mono-w;
                                                             k≤iterFold; len≤sizeᵗˢ;
                                                             max3-suc; node1; node2;
                                                             obsCaps?; one-fits; pathSz?;
                                                             pathSz?-widen; powʳ1;
                                                             module Red; slotsCaps?; SlotWid;
                                                             suc≤foldStep; thr; thr3;
                                                             two-folds; T≤TT; valCaps?; Wᴱ;
                                                             Wᴱ-mono; Wᴸ; Wᴸ-mono; Wᵀ;
                                                             Wᵀ-mono)
open import Decide using (ite≤; ∧-intro; ≤ᵇ-widen)

...
open import Verify-Budget-Sufficient.Caps-Face.Part1 using
  ...
  pathFloor; pathOrd?; pathOrd?-cell; pathOrd?-mono; pathOrd?-read; pathPark?; pathRead;
  pathStrat?; regOrd?;
  regPark?; regPark?-nodes; regStrat?; regOwn?; pathOwn?;
  frameParked?; frameOwned?; frameParked?-nodes; any-all; all-any; regOwn?-fresh)
open import Verify-Budget-Sufficient.Keeps-Ring using (switchKill-nodes)
open import Verify-Budget-Sufficient.Delivery-Walk using
  (chainsGo-chQ; shareAdmit-chQ)
open import Verify-Budget-Sufficient.Caps-Face.Part4 using (valsStrat?; capsOK?-regOrd)
...
open import Verify-Budget-Sufficient.Measures using
  (∧-true; all-impl; cutThrough-all; all-++-intro; takeVals-all)
...
-- WHAT THE TAKE HEAD LEAVES BELOW THE FLOOR, and it owes the store
-- NOTHING -- which is why it needs no park premise where its two
-- subscribing siblings do.  The head emits a PREFIX of what it was
-- handed, at both sides of the cut: the budgeted branch and the
-- severing branch differ in the registry, the completion flag and the
-- cell they write back, and agree exactly on the list.  So the reading
-- survives by the LIST and not by the state, and every cell shape that
-- is not a take emits nothing at all.
...
take-strat-step {s = s} k sf nid now nd κ vals fin sched st hib
  with lookupNode nd (EvalSt.nodes st)
... | nothing                    = refl
... | just (scan-st _)           = refl
... | just (mergeAll-st _ _ _ _) = refl
... | just (switch-st _ _)       = refl
... | just (exhaust-st _ _)      = refl
... | just (take-st m) with proj₂ (proj₂ (takeVals m vals))
...
-- the op does not own re-emit precisely what arrived.  The merge arm
-- is the only one that APPENDS, and its addend is the drain's, whose
-- premise is the park conjunct read at the node the frame names.  So
-- the head owes exactly one fact about the drain and nothing about
-- itself.
inner-strat-step : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {s}
  ...
  all (inputsBelowᵛ k s)
    (proj₁ (stepFrame sf nid now (from-inner op allNid inst) κ vals fin sched st))
      ≡ true
inner-strat-step k sf nid now op allNid inst κ vals false sched st hpk hib = hib
inner-strat-step {s = s} k sf nid now op allNid inst κ vals true sched st hpk hib
  with any (aliveThroughᶠ inst st) (EvalSt.registry st)
... | true  = hib
... | false = innerFinish-strat k sf op allNid inst κ nid now vals sched st
                (lookupNode allNid (EvalSt.nodes st)) (∧-trueˡ hpk) hib
  where
  innerFinish-strat : ∀ {n′} {Γ′ : Ctx n′} {t′} {e′ : Closed Γ′ t′} {s′}
    (k′ : ℕ) (g : Gas) (o : AllOp) (aN i′ : NodeId) (κ′ : Path Γ′ s′ t′)
    (id′ : Id) (nw : Tick) (vs : List (Val Γ′ s′))
    (sc : Sched Γ′) (s″ : EvalSt e′) (ns : Maybe (NodeState Γ′)) →
    parkStrat? k′ ns ≡ true →
    all (inputsBelowᵛ k′ s′) vs ≡ true →
    all (inputsBelowᵛ k′ s′)
      (proj₁ (innerFinish g o aN i′ κ′ id′ nw vs sc s″ ns)) ≡ true
  innerFinish-strat k′ g mergeAllᵒ aN i′ κ′ id′ nw vs sc s″ nothing _ h = h
  innerFinish-strat k′ g switchᵒ   aN i′ κ′ id′ nw vs sc s″ nothing _ h = h
  innerFinish-strat k′ g exhaustᵒ  aN i′ κ′ id′ nw vs sc s″ nothing _ h = h
  innerFinish-strat k′ g mergeAllᵒ aN i′ κ′ id′ nw vs sc s″ (just (scan-st _)) _ h = h
  innerFinish-strat k′ g switchᵒ   aN i′ κ′ id′ nw vs sc s″ (just (scan-st _)) _ h = h
  innerFinish-strat k′ g exhaustᵒ  aN i′ κ′ id′ nw vs sc s″ (just (scan-st _)) _ h = h
  innerFinish-strat k′ g mergeAllᵒ aN i′ κ′ id′ nw vs sc s″ (just (take-st _)) _ h = h
  innerFinish-strat k′ g switchᵒ   aN i′ κ′ id′ nw vs sc s″ (just (take-st _)) _ h = h
  innerFinish-strat k′ g exhaustᵒ  aN i′ κ′ id′ nw vs sc s″ (just (take-st _)) _ h = h
  innerFinish-strat k′ g switchᵒ   aN i′ κ′ id′ nw vs sc s″ (just (mergeAll-st _ _ _ _)) _ h = h
  innerFinish-strat k′ g exhaustᵒ  aN i′ κ′ id′ nw vs sc s″ (just (mergeAll-st _ _ _ _)) _ h = h
  innerFinish-strat k′ g mergeAllᵒ aN i′ κ′ id′ nw vs sc s″ (just (switch-st _ _)) _ h = h
  innerFinish-strat k′ g exhaustᵒ  aN i′ κ′ id′ nw vs sc s″ (just (switch-st _ _)) _ h = h
  innerFinish-strat k′ g mergeAllᵒ aN i′ κ′ id′ nw vs sc s″ (just (exhaust-st _ _)) _ h = h
  innerFinish-strat k′ g switchᵒ   aN i′ κ′ id′ nw vs sc s″ (just (exhaust-st _ _)) _ h = h
  innerFinish-strat k′ g exhaustᵒ  aN i′ κ′ id′ nw vs sc s″ (just (exhaust-st _ _)) _ h = h
  innerFinish-strat k′ g switchᵒ   aN i′ κ′ id′ nw vs sc s″ (just (switch-st nothing _)) _ h = h
  innerFinish-strat k′ g switchᵒ   aN i′ κ′ id′ nw vs sc s″ (just (switch-st (just c) _)) _ h
    with c ≡ᵇ i′
  ...  | true  = h
  ...  | false = h
  innerFinish-strat {s′ = s′} k′ g mergeAllᵒ aN i′ κ′ id′ nw vs sc s″
                    (just (mergeAll-st {w} lim act q od)) hq h with w ≟ᵗ s′
  ...  | no _     = h
  ...  | yes refl =
         all-++-intro (inputsBelowᵛ k′ s′) vs _ h
           (mergeAllDrain-strat k′ g aN κ′ id′ nw lim (pred act) q sc s″ hq)

-- WHAT THE OUTER HEAD LEAVES BELOW THE FLOOR, and the head's own
-- contribution is nothing: the completion wrap reads a cell to decide
-- whether the frame is done and writes the decision back, and hands
-- the payload on untouched at every arm and every op.  So the reading
-- is entirely the walk's, and the enumeration below is the price of
-- saying so -- a wrap that dispatches on the op and on a cell cannot
-- reduce at a variable of either.
...
thru-strat-step {Γ = Γ} {t = t} {e = e} {u = u} c k sf nid now op nd κ vals fin sched st hck hib =
  ...
  wrap : (o : AllOp) (nd′ : NodeId) (b : Bool)
    ...
    all P (proj₁ r) ≡ true → all P (proj₁ (thruWrap o nd′ b r)) ≡ true
  wrap o         nd′ false (vs , bs , sc , s″) h = h
  wrap mergeAllᵒ nd′ true  (vs , bs , sc , s″) h
    with lookupNode nd′ (EvalSt.nodes s″)
  ...  | nothing                    = h
  ...  | just (scan-st _)           = h
  ...  | just (take-st _)           = h
  ...
  wrap switchᵒ   nd′ true  (vs , bs , sc , s″) h
    with lookupNode nd′ (EvalSt.nodes s″)
  ...  | nothing                    = h
  ...  | just (scan-st _)           = h
  ...  | just (take-st _)           = h
  ...
  ...  | just (switch-st _ _)       = h
  ...  | just (exhaust-st _ _)      = h
  wrap exhaustᵒ  nd′ true  (vs , bs , sc , s″) h
    with lookupNode nd′ (EvalSt.nodes s″)
  ...  | nothing                    = h
  ...  | just (scan-st _)           = h
  ...  | just (take-st _)           = h
...
-- ledger puts every registered chain's cells strictly below it -- so
-- the reading holds at EVERY floor and the ledger is the whole of the
-- argument.  Stated over the scheduler rather than over a bare node so
-- that the `≤` is `refl` at the site and the caps receipt is the only
-- thing a caller has to name.
mint-regOwn : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t}
  (c : Caps) (k : ℕ) (sched : Sched Γ) (st : EvalSt e) →
  capsOK? c sched st ≡ true →
  regOwn? (Sched.nextNode sched) k (EvalSt.registry st) ≡ true
mint-regOwn c k sched st inv =
  regOwn?-fresh (Sched.nextNode sched) (Sched.nextNode sched) k
    (EvalSt.registry st) ≤-refl (capsOK?-regOrd c sched st inv)
...
installNode-scanPark k fn nid ac st how hac =
  ∧-intro (trans (cong (parkStrat? k)
...
installNode-thruPark {u = u} k op nid st how =
  ∧-intro (cong (parkStrat? k)
...
installNode-cellPark k op nid ns st how h =
  ∧-intro (trans (cong (parkStrat? k)
                    (lookupNode-setNode nid ns (EvalSt.nodes st))) h)
          (∧-intro how refl)
...
-- AND THE OWNER HALF TRAVELS BY ITS OWN PREMISE, since a freeze is a
-- statement about the STORE and ownership reads the registry.  The two
-- are separated rather than bundled because their sites differ: a
-- record update that touches neither field spends `refl` for the
-- registry half, while a subscribe grows it and owes a real argument.
frozen-framePark : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {s u}
  ...
  (∀ nd → any (_≡ᵇ nd) (frameNodes f) ≡ true →
     regOwn? nd k (EvalSt.registry st) ≡ true →
     regOwn? nd k (EvalSt.registry st′) ≡ true) →
  parkBelow w f →
  framePark? k f st ≡ true →
  framePark? k f st′ ≡ true
frozen-framePark {Γ = Γ} k w f st st′ hfz how hb hp =
  ∧-intro (parked f hb (proj₁ hs))
          (any-all (λ nd → regOwn? nd k (EvalSt.registry st′)) (frameNodes f)
             (λ nd hm → how nd hm
                (all-any (λ m → regOwn? m k (EvalSt.registry st)) nd
                   (frameNodes f) (proj₂ hs) hm)))
  where
  hs = ∧-true (frameParked? k f st) (frameOwned? k f (EvalSt.registry st)) hp
  parked : ∀ {s′ u′} (f′ : Frame Γ s′ u′) → parkBelow w f′ →
    frameParked? k f′ st ≡ true → frameParked? k f′ st′ ≡ true
  parked (map-f _)          hb′ h = h
  parked (take-f _)         hb′ h = h
  parked (thru-outer _ nid) hb′ h = trans (cong (parkStrat? k) (hfz nid hb′)) h
  parked (scan-f _ nd)      hb′ h = trans (cong (parkStrat? k) (hfz nd hb′)) h
  parked (from-inner _ a _) hb′ h = trans (cong (parkStrat? k) (hfz a hb′)) h

-- AND WHAT A KILL DOES TO IT, WHICH IS NOTHING, AND THAT IS PROVEN
...
switchKill-regOwn : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t}
  ...
  regOwn? nd k (EvalSt.registry st) ≡ true →
  regOwn? nd k (EvalSt.registry (proj₂ (proj₂ (switchKill cur sched st)))) ≡ true
switchKill-regOwn nd k nothing  sched st h = h
switchKill-regOwn nd k (just v) sched st h =
  cutThrough-all (λ en → pathOwn? nd k (proj₂ (proj₂ (proj₂ en)))) v
    (EvalSt.delivered st) (EvalSt.regWatermark st) (EvalSt.dying st)
    (EvalSt.registry st) h

-- AND THE WHOLE CHAIN READING ACROSS THE SAME KILL, which is no longer
-- a node-table transport: folding ownership into the frame reading put
-- the REGISTRY on the side the kill writes, so the two halves travel by
-- different arguments -- the node table is untouched, and the owner
-- half rides the sublist above.
switchKill-framePark : ∀ {n} {Γ : Ctx n} {s u t} {e : Closed Γ t}
  (k : ℕ) (f : Frame Γ s u) (cur : Maybe NodeId) (sched : Sched Γ) (st : EvalSt e) →
  framePark? k f st ≡ true →
  framePark? k f (proj₂ (proj₂ (switchKill cur sched st))) ≡ true
switchKill-framePark k f cur sched st h =
  ∧-intro
    (trans (frameParked?-nodes k f (proj₂ (proj₂ (switchKill cur sched st))) st
              (switchKill-nodes cur sched st))
           (∧-trueˡ {a = frameParked? k f st} h))
    (all-impl (λ nd → regOwn? nd k (EvalSt.registry st))
              (λ nd → regOwn? nd k
                        (EvalSt.registry (proj₂ (proj₂ (switchKill cur sched st)))))
              (λ nd → switchKill-regOwn nd k cur sched st)
              (frameNodes f) (∧-trueʳ {a = frameParked? k f st} h))

switchKill-pathPark : ∀ {n} {Γ : Ctx n} {u t} {e : Closed Γ t}
  (κ : Path Γ u t) (cur : Maybe NodeId) (sched : Sched Γ) (st : EvalSt e) →
  pathPark? κ st ≡ true →
  pathPark? κ (proj₂ (proj₂ (switchKill cur sched st))) ≡ true
switchKill-pathPark root           cur sched st h = refl
switchKill-pathPark (share-sink _) cur sched st h = refl
switchKill-pathPark (f ↠ p)        cur sched st h =
  ∧-intro (switchKill-framePark (pathFloor p) f cur sched st
             (∧-trueˡ {a = framePark? (pathFloor p) f st} h))
          (switchKill-pathPark p cur sched st
             (∧-trueʳ {a = framePark? (pathFloor p) f st} h))

-- AND WHAT A STEP DOES TO THE SAME LEDGER, which is a CASE SPLIT and
-- two writers rather than one statement.  Three of the five frames
-- register nothing and each says so for its own reason: a map hands
-- back the state it was given, a scan rewrites the node table on every
-- branch of its accumulator's type test, and a take either rewrites
-- the table or CUTS -- and a cut keeps a sublist, which is the one
-- write a universal over the ledger survives for free.  So the gap is
-- exactly the two arms that REGISTER, and each is stated over its own
-- writer rather than over the frame that dispatches to it.
--
-- AND THE WRAP IS NOT ONE OF THEM, which is what makes the outer arm a
-- leaf and not a pair.  Every branch of `thruWrap` rewrites `nodes`
-- and nothing else, so the registry the outer arm reports is the walk's
-- own -- the equality below, and the arm is a transport along it.
thruWrap-regs : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (op : AllOp) (nid : NodeId) (b : Bool)
  (r : List (Val Γ u) × List (InstEvent (Val Γ t)) × Sched Γ × EvalSt e) →
  EvalSt.registry (proj₂ (proj₂ (proj₂ (proj₂ (thruWrap op nid b r)))))
    ≡ EvalSt.registry (proj₂ (proj₂ (proj₂ r)))
thruWrap-regs op        nid false r = refl
thruWrap-regs mergeAllᵒ nid true  r@(_ , _ , _ , st′)
...
... | just (exhaust-st _ _)      = refl
... | nothing                    = refl
thruWrap-regs switchᵒ   nid true  r@(_ , _ , _ , st′)
  with lookupNode nid (EvalSt.nodes st′)
... | just (switch-st _ _)       = refl
... | just (scan-st _)           = refl
... | just (take-st _)           = refl
...
... | just (exhaust-st _ _)      = refl
... | nothing                    = refl
thruWrap-regs exhaustᵒ  nid true  r@(_ , _ , _ , st′)
  with lookupNode nid (EvalSt.nodes st′)
... | just (exhaust-st _ _)      = refl
... | just (scan-st _)           = refl
... | just (take-st _)           = refl
...
-- per operator registers at all -- so the reading rides the SUBSCRIBE
-- above and the ledger writes around it are node-table rewrites the
-- registry does not see.  Two branches are worth naming because they
-- read as writers and are not.  The ENQUEUE arm -- a mergeAll with no
-- room, the writer whose own cell the reading is about -- parks the
-- value in the node's queue and touches the ledger nowhere, so it is
-- paid by the hypothesis unchanged.  And the switch arm KILLS before
-- it subscribes, which the proven sublist reading above carries, so
-- the subscribe is entered at a state the thread already holds.
thruConsume-regOwn : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  regOwn? nd (pathFloor κ)
    ...
    ≡ true
thruConsume-regOwn {u = u} g mergeAllᵒ nid κ id now o sched st nd h
  with lookupNode nid (EvalSt.nodes st)
... | nothing                = h
... | just (scan-st _)       = h
... | just (take-st _)       = h
...
...     | false = h
...     | true  = subscribeInner-regOwn g mergeAllᵒ nid κ id now o sched st nd h
thruConsume-regOwn g switchᵒ nid κ id now o sched st nd h
  with lookupNode nid (EvalSt.nodes st)
... | nothing                    = h
... | just (scan-st _)           = h
... | just (take-st _)           = h
...
... | just (switch-st cur od)    =
      subscribeInner-regOwn g switchᵒ nid κ id now o
        (proj₁ (proj₂ (switchKill cur sched st)))
        (proj₂ (proj₂ (switchKill cur sched st))) nd
        (switchKill-regOwn nd (pathFloor κ) cur sched st h)
thruConsume-regOwn g exhaustᵒ nid κ id now o sched st nd h
  with lookupNode nid (EvalSt.nodes st)
... | nothing                    = h
... | just (scan-st _)           = h
... | just (take-st _)           = h
... | just (mergeAll-st _ _ _ _) = h
... | just (switch-st _ _)       = h
... | just (exhaust-st true  od) = h
... | just (exhaust-st false od) =
      subscribeInner-regOwn g exhaustᵒ nid κ id now o sched st nd h

-- AND THE WALK IS THE FOLD OVER IT, which is the whole of the outer
-- arm's remaining content: a drain consumes its emits one at a time
-- through the same node at the same floor, so the reading is carried
-- emit by emit and the induction is on the emit list alone.
thruWalk-regOwn : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (g : Gas) (op : AllOp) (nid : NodeId) (κ : Path Γ u t)
  (id : Id) (now : Tick) (vals : List (Val Γ (obs u)))
  (sched : Sched Γ) (st : EvalSt e) (nd : NodeId) →
  regOwn? nd (pathFloor κ) (EvalSt.registry st) ≡ true →
  regOwn? nd (pathFloor κ)
    ...
    ≡ true
thruWalk-regOwn g op nid κ id now []       sched st nd h = h
thruWalk-regOwn g op nid κ id now (o ∷ os) sched st nd h =
  thruWalk-regOwn g op nid κ id now os
    (proj₁ (proj₂ (proj₂ (thruConsume g op nid κ id now o sched st))))
    (proj₂ (proj₂ (proj₂ (thruConsume g op nid κ id now o sched st)))) nd
    (thruConsume-regOwn g op nid κ id now o sched st nd h)

stepFrame-regOwn : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {s u}
  (g : Gas) (id : Id) (now : Tick)
  (f : Frame Γ s u) (κ : Path Γ u t)
  (vals : List (Val Γ s)) (fin : Bool)
  (sched : Sched Γ) (st : EvalSt e) (nd : NodeId) →
  ...
  regOwn? nd (pathFloor κ)
    (EvalSt.registry
      (proj₂ (proj₂ (proj₂ (proj₂ (stepFrame g id now f κ vals fin sched st))))))
    ≡ true
stepFrame-regOwn g id now (map-f _) κ vals fin sched st nd h = h
stepFrame-regOwn {u = u} g id now (scan-f fn nid) κ vals fin sched st nd h
  with lookupNode nid (EvalSt.nodes st)
... | nothing                    = h
... | just (take-st _)           = h
... | just (mergeAll-st _ _ _ _) = h
...
...   | no  _    = h
...   | yes refl = h
stepFrame-regOwn g id now (take-f nid) κ vals fin sched st nd h
  with lookupNode nid (EvalSt.nodes st)
... | nothing                    = h
... | just (scan-st _)           = h
... | just (mergeAll-st _ _ _ _) = h
...
...   | false = h
...   | true  =
        cutThrough-all (λ en → pathOwn? nd (pathFloor κ) (proj₂ (proj₂ (proj₂ en))))
          nid (EvalSt.delivered st) (EvalSt.regWatermark st)
          (EvalSt.dying st) (EvalSt.registry st) h
stepFrame-regOwn g id now (from-inner op allNid inst) κ vals fin sched st nd h =
  innerReact-regOwn g op allNid inst κ id now vals fin sched st nd h
stepFrame-regOwn g id now (thru-outer op nid) κ vals fin sched st nd h =
  subst (λ rs → regOwn? nd (pathFloor κ) rs ≡ true)
...
subscribeE-framePark k g b κ bid now f sched st hb refl hp =
  frozen-framePark (pathFloor κ) (Sched.nextNode sched) f st _
    (subscribeE-nodes-below g b κ bid now sched st)
    (λ nd _ hj → subscribeE-regOwn g b κ bid now sched st nd hj)
    hb hp
...
frozen-pathPark : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  pathPark? κ st ≡ true →
  pathPark? κ st′ ≡ true
frozen-pathPark w root           st st′ hfz how hb hp = refl
frozen-pathPark w (share-sink _) st st′ hfz how hb hp = refl
frozen-pathPark w (f ↠ p) st st′ hfz how (hbf , hbp) hp =
  ∧-intro (frozen-framePark (pathFloor p) w f st st′ hfz
             (λ nd hm → how nd (cong (_∨ pathHasNode nd p) hm)) hbf
             (∧-trueˡ hp))
          (frozen-pathPark w p st st′ hfz
             (λ nd hh → how nd (trans (cong (any (_≡ᵇ nd) (frameNodes f) ∨_) hh)
                                      (∨-zeroʳ (any (_≡ᵇ nd) (frameNodes f)))))
             hbp (∧-trueʳ hp))
...
fresh-pathPark : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  ...
  pathPark? κ st ≡ true →
  pathPark? κ st′ ≡ true
fresh-pathPark κ nx nx′ st st′ fr how hp =
  frozen-pathPark (pathRead κ) κ st st′ (FreshC.frozen fr) how
...
subscribeE-pathPark : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u w}
  ...
  pathFloor κ ≡ pathFloor β →
  pathPark? κ st ≡ true →
  pathPark? κ (proj₂ (proj₂ (subscribeE g b β bid now sched st))) ≡ true
subscribeE-pathPark g b β κ bid now sched st hb hf hp =
  frozen-pathPark (Sched.nextNode sched) κ st _
    (subscribeE-nodes-below g b β bid now sched st)
    (λ nd _ hj →
       subst (λ m → regOwn? nd m
                (EvalSt.registry
                  (proj₂ (proj₂ (subscribeE g b β bid now sched st)))) ≡ true)
         (sym hf)
         (subscribeE-regOwn g b β bid now sched st nd
            (subst (λ m → regOwn? nd m (EvalSt.registry st) ≡ true) hf hj)))
...
subscribeInner-pathPark g0 op allNid β κ id now o sched st hb hf hp = hp
subscribeInner-pathPark (gs fuel) op allNid β κ id now o sched st hb hf hp =
  subscribeE-pathPark fuel o (from-inner op allNid (Sched.nextNode sched) ↠ β) κ
    ...
    (≤-trans hb (n≤1+n (Sched.nextNode sched))) hf hp
...
subscribeInner-cellPark k (gs fuel) op allNid β κ id now o sched st hb hp =
  trans (cong (parkStrat? k)
           (subscribeE-nodes-below fuel o
              (from-inner op allNid (Sched.nextNode sched) ↠ β) id now
              (record sched { nextNode = suc (Sched.nextNode sched) }) st
...
-- THE ONE CELL A WALK CANNOT FREEZE, and it is the outer's own.  The
-- flatten's consume writes the very cell the reading is about, so no
-- watermark reaches it; but only ONE of the four writes is opaque.  A
-- consume with no room enqueues the payload, and the extended queue's
-- reading is the entry queue's and the payload's, which are two of the
-- premises.  A switch and an exhaust reinstall a cell the floor check
-- reads as `true` whatever it holds.  What is left is a subscribe
-- entered at this node, and the bump that follows it is transparent to
-- the reading.
--
-- AND THE ORDERING CONJUNCT IS WHAT PAYS FOR THAT ONE ARM, WITHOUT
-- BEING A HYPOTHESIS THE CALLER HAPPENS TO HOLD.  The already-proven
-- form of the subscribe's own reading buys it from the consumed node
-- being an OLD one -- `suc nid` under the scheduler's counter -- which
-- no cell-level premise can supply.  It is not a fact that had to be
-- sourced: the node the consume is entered at is the CELL OF THE HEAD
-- of the very chain the walk is walking, and every walk statement over
-- this chain already carries the counter bound over the whole of it.
-- So the premise here is the walk's own conjunct verbatim, projected
-- at the head rather than threaded, and a second caller could not
-- appear that does not hold it -- the chain is what makes `nid`
-- nameable at all.
thruConsume-cellPark : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t} {u}
  (g : Gas) (op : AllOp) (nid : NodeId) (κ : Path Γ u t)
  (id : Id) (now : Tick) (o : Val Γ (obs u))
  (sched : Sched Γ) (st : EvalSt e) →
  pathOrd? (Sched.nextNode sched) (thru-outer {u = u} op nid ↠ κ) ≡ true →
...
thruConsume-cellPark {u = u} g mergeAllᵒ nid κ id now o sched st ho hb hp
...
...     | true  =
          mergeAllBump-cellPark (pathFloor κ) nid
            ...
            (subscribeInner-cellPark (pathFloor κ) g mergeAllᵒ nid κ κ id now o
               sched st
               (≤-trans (m≤m⊔n (suc nid) (pathCell κ))
                        (pathOrd?-cell (Sched.nextNode sched)
                           (thru-outer {u = u} mergeAllᵒ nid ↠ κ) ho))
               (parkStrat?-cell eq hp))
          where
...
...     | false =
          trans (cong (parkStrat? (pathFloor κ))
                ...
                (all-++-intro (inputsBelowᵉ (pathFloor κ)) q (o ∷ [])
                   hp (∧-intro hb refl))
thruConsume-cellPark g switchᵒ nid κ id now o sched st _ hb hp
  with lookupNode nid (EvalSt.nodes st) in eq
... | nothing                    = parkStrat?-cell eq hp
... | just (scan-st _)           = parkStrat?-cell eq hp
... | just (take-st _)           = parkStrat?-cell eq hp
...
... | just (switch-st cur od)    =
      ...
      where
        r = subscribeInner g switchᵒ nid κ id now o
              (proj₁ (proj₂ (switchKill cur sched st)))
              (proj₂ (proj₂ (switchKill cur sched st)))
thruConsume-cellPark g exhaustᵒ nid κ id now o sched st _ hb hp
  with lookupNode nid (EvalSt.nodes st) in eq
... | nothing                    = parkStrat?-cell eq hp
... | just (scan-st _)           = parkStrat?-cell eq hp
... | just (take-st _)           = parkStrat?-cell eq hp
...
pathPark-step g id now f κ vals fin sched st ho hp =
  frozen-pathPark (pathRead κ) κ st _
    ...
    (λ nd _ hj → stepFrame-regOwn g id now f κ vals fin sched st nd hj)
    (pathRead-below (pathRead κ) κ ≤-refl) hp
  where
...
cascadeLatch-registry : ∀ {n} {Γ : Ctx n} {t} {e : Closed Γ t}
  (a : Arrival Γ) (st : EvalSt e) →
  EvalSt.registry st ≡ EvalSt.registry (cascadeLatch a st)
cascadeLatch-registry a st with Arrival.isLast a
... | true  = refl
... | false = refl
...
shareAdmit-park i true  st h =
  shareAdmit-chQ (λ {u} _ κ → pathPark? κ (shareLatch i true st)) i
    (EvalSt.registry st)
    (regPark?-nodes (EvalSt.registry st) st (shareLatch i true st) refl refl h)
...
cascade-admit-park a st h =
  chainsGo-chQ (λ {u} _ κ → pathPark? κ (cascadeLatch a st)) a
    ...
    (regPark?-nodes (EvalSt.registry st) st (cascadeLatch a st)
       (cascadeLatch-nodes a st) (cascadeLatch-registry a st) h)
...
subscribeE-readings g b κ id now sched st ho hp =
  ...
  , fresh-pathPark κ (Sched.nextNode sched) (Sched.nextNode (proj₁ (proj₂ r))) st
      (proj₂ (proj₂ r)) FR
      (λ nd _ hj → subscribeE-regOwn g b κ id now sched st nd hj) hp
  where

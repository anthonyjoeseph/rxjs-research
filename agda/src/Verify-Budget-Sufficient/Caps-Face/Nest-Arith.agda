...
open import Data.Nat.Properties using (<⇒≤; *-identityˡ; ^-distribˡ-+-*; ≤ᵇ⇒≤; ^-monoʳ-≤; ^-monoˡ-≤; *-monoˡ-≤; ≤-trans; ≤-refl; m∸n+n≡m;
...
open import Verify-Budget-Sufficient.Caps using
  (1≤capsAt-reg; 1≤pow≤; 2≤capsAt-size; 8≤capsAt-size; 129≤capsAt-size; B2-cReg≤cSize; Caps;
  capsAt; capsAt-base-size; capsAt-size-mono; capsH; capsAt-exp-gain; size≤sizeCount;
...
abstract
  ...
  -- THE CAP'S CHARGE CARRIES THE PATH FACTOR, WHICH IS WHAT LETS A
  -- DEPTH PREMISE BE DENOMINATED IN THE CAP AT ALL.  A frame arm
  -- spends its nest premise multiplied by the factor the path can
  -- still apply, and that factor is capped at two to `(S + S) *
  -- (suc S * S)` -- so a charge whose exponent is smaller than that
  -- can only be spent against a multiplicand the size cap covers,
  -- which is the UNIT and not the cap.  The exponent here is that
  -- factor's cap -- the two cubes and two squares it expands to --
  -- and TWO beside it, since the arm doubles its own charge before
  -- splitting it and each half is doubled again by the frame's own
  -- factor.  So the cap-denominated half of the potential is the half
  -- a cap-denominated premise is spent against.
...
-- ONE FIFTH POWER'S EXPANSION, the same ring content one degree up: a
-- successor adds a FOURTH POWER's worth of terms to a fifth.
quintStep : ∀ (t : ℕ) →
  suc t * (suc t * (suc t * (suc t * suc t)))
    ≡ t * (t * (t * (t * t)))
      + (5 * (t * (t * (t * t)))
         + (10 * (t * (t * t)) + (10 * (t * t) + (5 * t + 1))))
quintStep =
  solve 1 (λ a → (con 1 :+ a)
                   :* ((con 1 :+ a)
                       :* ((con 1 :+ a) :* ((con 1 :+ a) :* (con 1 :+ a))))
                 := a :* (a :* (a :* (a :* a)))
                    :+ (con 5 :* (a :* (a :* (a :* a)))
                        :+ (con 10 :* (a :* (a :* a))
                            :+ (con 10 :* (a :* a) :+ (con 5 :* a :+ con 1)))))
          refl

-- AND WHAT IT ADDS IS UNDER THE FIFTH POWER ITSELF from thirty-one on,
-- so a successor at most DOUBLES it -- one more factor of two, which
-- is the whole rung.  Thirty-one because the added terms collapse to
-- `31` fourth powers before the size is spent, and the size is what
-- pays for them.
quintGap : ∀ (t : ℕ) → 31 ≤ t →
  5 * (t * (t * (t * t)))
    + (10 * (t * (t * t)) + (10 * (t * t) + (5 * t + 1)))
    ≤ t * (t * (t * (t * t)))
quintGap t 31≤t =
  ≤-trans (+-monoʳ-≤ (5 * P4) (≤-trans (+-monoʳ-≤ (10 * P3) stepA) stepB))
          (≤-trans (≤-reflexive eq31) (*-monoˡ-≤ P4 31≤t))
  where
  1≤t : 1 ≤ t
  1≤t = ≤-trans (s≤s z≤n) 31≤t
  P2 = t * t
  P3 = t * (t * t)
  P4 = t * (t * (t * t))
  t≤P2 : t ≤ P2
  t≤P2 = ≤-trans (≤-reflexive (sym (*-identityʳ t))) (*-monoʳ-≤ t 1≤t)
  P2≤P3 : P2 ≤ P3
  P2≤P3 = *-monoʳ-≤ t t≤P2
  P3≤P4 : P3 ≤ P4
  P3≤P4 = *-monoʳ-≤ t P2≤P3
  eq6 : 5 * t + t ≡ 6 * t
  eq6 = solve 1 (λ a → con 5 :* a :+ a := con 6 :* a) refl t
  lin : 5 * t + 1 ≤ 6 * P2
  lin = ≤-trans (+-monoʳ-≤ (5 * t) 1≤t)
                (≤-trans (≤-reflexive eq6) (*-monoʳ-≤ 6 t≤P2))
  eq16 : 10 * P2 + 6 * P2 ≡ 16 * P2
  eq16 = solve 1 (λ a → con 10 :* a :+ con 6 :* a := con 16 :* a) refl P2
  stepA : 10 * P2 + (5 * t + 1) ≤ 16 * P3
  stepA = ≤-trans (+-monoʳ-≤ (10 * P2) lin)
                  (≤-trans (≤-reflexive eq16) (*-monoʳ-≤ 16 P2≤P3))
  eq26 : 10 * P3 + 16 * P3 ≡ 26 * P3
  eq26 = solve 1 (λ a → con 10 :* a :+ con 16 :* a := con 26 :* a) refl P3
  stepB : 10 * P3 + 16 * P3 ≤ 26 * P4
  stepB = ≤-trans (≤-reflexive eq26) (*-monoʳ-≤ 26 P3≤P4)
  eq31 : 5 * P4 + 26 * P4 ≡ 31 * P4
  eq31 = solve 1 (λ a → con 5 :* a :+ con 26 :* a := con 31 :* a) refl P4

-- FOUR FIFTH POWERS UNDER THE EXPONENTIAL, and THIRTY-ONE is where the
-- gap above first closes -- the statement itself first holds at
-- twenty-six, so this base is where the INDUCTION starts rather than
-- where the inequality does, and the caps floor is read well past both.
--
-- AND THE OFFSET SITS ON THE RIGHT OF THE PLUS, WHICH IS THE ONLY
-- REASON THIS CHECKS AT ALL.  `_+_` matches on its LEFT argument, so
-- `j + 31` is STUCK at a variable `j` and every power of it is stuck
-- too; the induction step then costs one `suc` peel and nothing else.
-- Write the threshold on the left -- as `31 + j`, or as a nested
-- tower of `suc` -- and the subject becomes thirty-one successors,
-- which `_*_` (matching on its own left argument) expands into
-- thirty-two copies of its right at EVERY one of the five nestings.
-- DEAD ROUTE: the nested-`suc` threshold, ported up from a degree-3
--   subject, where it is affordable.  It
--   is not merely slower: the expansion is the nesting depth's power of
--   the threshold, so a degree-3 subject at fifteen successors is tens
--   of thousands of nodes while a degree-5 one at thirty-one is
--   millions, and the module stopped checking inside the dev loop's
--   budget entirely.  The neutral form below is the same proof.
quint-exp : ∀ (j : ℕ) →
  4 * ((j + 31) * ((j + 31) * ((j + 31) * ((j + 31) * (j + 31)))))
    ≤ 2 ^ (j + 31)
quint-exp zero    = ≤ᵇ⇒≤ 114516604 2147483648 tt
quint-exp (suc j) =
  ≤-trans (≤-reflexive step)
          (≤-trans (+-mono-≤ (quint-exp j)
                             (≤-trans (*-monoʳ-≤ 4 (quintGap t (m≤n+m 31 j)))
                                      (quint-exp j)))
                   (≤-reflexive (sym (2X≡X+X (2 ^ t)))))
  where
  t = j + 31
  step : 4 * (suc t * (suc t * (suc t * (suc t * suc t))))
           ≡ 4 * (t * (t * (t * (t * t))))
             + 4 * (5 * (t * (t * (t * t)))
                    + (10 * (t * (t * t)) + (10 * (t * t) + (5 * t + 1))))
  step = trans (cong (4 *_) (quintStep t))
               (*-distribˡ-+ 4 (t * (t * (t * (t * t))))
                               (5 * (t * (t * (t * t)))
                                + (10 * (t * (t * t))
                                   + (10 * (t * t) + (5 * t + 1)))))

-- AND THE PREMISE IS SPENT BY SUBTRACTION rather than by a wall of
-- `s≤s`, since the induction is now indexed by the OFFSET and the
-- offset is exactly what a truncated subtraction recovers.
quint4≤2^ : ∀ (S : ℕ) → 31 ≤ S → 4 * (S * (S * (S * (S * S)))) ≤ 2 ^ S
quint4≤2^ S 31≤S =
  subst (λ z → 4 * (z * (z * (z * (z * z)))) ≤ 2 ^ z)
        (m∸n+n≡m 31≤S)
        (quint-exp (S ∸ 31))

-- THE PATH FACTOR'S EXPONENT, EXPANDED.  The factor's own ceiling is
-- stated in the form the path's recursion produces, and both readings
-- of it below need the polynomial: the cap's charge is denominated at
-- exactly this exponent, and the walk's is widened from it.
pathExp≡ : ∀ (S : ℕ) →
  (S + S) * (suc S * S) ≡ S * (S * S) + S * (S * S) + (S * S + S * S)
pathExp≡ S = solve 1 (λ s → (s :+ s) :* ((con 1 :+ s) :* s)
                              := s :* (s :* s) :+ s :* (s :* s)
                                 :+ (s :* s :+ s :* s))
                   refl S

-- THE EXPONENT THE PATH FACTOR IS CAPPED AT, READ INTO THE WALK'S OWN.
-- A path's factor is capped at two to `(S + S) * (suc S * S)`, which
-- expands to two cubes and two squares, while the walk's exponent is
-- three fifth powers and a square -- so the reading is a WIDENING and
-- not an equation, and the two squares are the only summand shared.
walkExp-widen : ∀ (S : ℕ) → 1 ≤ S →
  (S + S) * (suc S * S)
    ≤ S * (S * (S * (S * S))) + S * (S * (S * (S * S)))
      + S * (S * (S * (S * S))) + (S * S + S * S)
walkExp-widen S 1≤S =
  ≤-trans (≤-reflexive (pathExp≡ S))
          (+-monoˡ-≤ (S * S + S * S)
            (≤-trans (+-mono-≤ C3≤C5 C3≤C5) (m≤m+n (C5 + C5) C5)))
  where
  C3 = S * (S * S)
  C5 = S * (S * (S * (S * S)))
  ≤mul : ∀ (x : ℕ) → x ≤ S * x
...
nestΦ-frame-charge e sl id =
  ...
  split : 4 * (Z * (C + Sq)) + 2 * (Z * (S * W))
            ≡ 4 * (Z * C) + 2 * (Z * (Sq + Sq + S * W))
  split = solve 4 (λ z c q v →
                     con 4 :* (z :* (c :+ q)) :+ con 2 :* (z :* v)
                       := con 4 :* (z :* c)
                          :+ con 2 :* (z :* (q :+ q :+ v)))
                refl Z C Sq (S * W)
  coeff : 4 * Z ≡ 2 ^ suc (suc E)
  coeff = trans (cong (λ z → 4 * 2 ^ z) (pathExp≡ S)) (*-assoc 2 2 (2 ^ E))
  capHalf : 4 * (Z * C) ≡ capΦAt e sl id
  capHalf = trans (sym (*-assoc 4 Z C))
                  (trans (cong (_* C) coeff) (sym (capΦAt-def e sl id)))
  xwFit : Sq + Sq + S * W ≤ U + (Sq + Sq) + S + S * W
  xwFit = +-monoˡ-≤ (S * W)
            (≤-trans (m≤n+m (Sq + Sq) U) (m≤m+n (U + (Sq + Sq)) S))
  walkHalf : 2 * (Z * (Sq + Sq + S * W)) ≤ nestWalkAt e sl id
  walkHalf =
...
-- FACTOR's own cap -- two cubes and two squares -- so the room premise
-- gains all four beside the size it already had.  Two cubes is what
-- puts the sum past a cube of its own: it lands inside a FIFTH power
-- with four to spare, which the size cap's reachable floor buys at the
-- same threshold the walk's charge is already fitted at.
capΦ-room-ℕ : ∀ (S′ p : ℕ) → 31 ≤ S′ → p ≤ S′ →
  2 * (S′ + suc (suc (S′ * (S′ * S′) + S′ * (S′ * S′)
                      + (S′ * S′ + S′ * S′))) + 3 + p)
    ≤ 2 ^ S′
capΦ-room-ℕ S′ p 31≤S′ hp =
  ≤-trans (*-monoʳ-≤ 2 (+-monoʳ-≤ (S′ + suc (suc (Cu + Cu + (Sq + Sq))) + 3) hp))
  ...
  4≤Sq : 4 ≤ Sq
  4≤Sq = ≤-trans 4≤S′ S′≤Sq
  expand : 2 * (S′ + suc (suc (Cu + Cu + (Sq + Sq))) + 3 + S′)
             ≡ 4 * (Cu + Sq + S′) + 10
  expand = solve 3 (λ s c q →
                      con 2 :* (s :+ (con 2 :+ (c :+ c :+ (q :+ q)))
                                :+ con 3 :+ s)
                        := con 4 :* (c :+ q :+ s) :+ con 10)
                 refl S′ Cu Sq
  wideEq : 4 * (Cu + Sq + S′ + 3) ≡ 4 * (Cu + Sq + S′) + 12
  wideEq = solve 3 (λ s c q → con 4 :* (c :+ q :+ s :+ con 3)
                                := con 4 :* (c :+ q :+ s) :+ con 12)
                 refl S′ Cu Sq
  wide : 4 * (Cu + Sq + S′) + 10 ≤ 4 * (Cu + Sq + S′ + 3)
  wide = ≤-trans (+-monoʳ-≤ (4 * (Cu + Sq + S′)) (≤ᵇ⇒≤ 10 12 tt))
                 (≤-reflexive (sym wideEq))
  fourCu : Cu + Cu + Cu + Cu ≡ 4 * Cu
  fourCu = solve 1 (λ c → c :+ c :+ c :+ c := con 4 :* c) refl Cu
  quEq : Sq * Cu ≡ Qu
  ...
  rest : Cu + Sq + S′ + 3 ≤ Qu
  rest =
    ≤-trans (+-mono-≤ (+-mono-≤ (+-monoʳ-≤ Cu Sq≤Cu) S′≤Cu) 3≤Cu)
    (≤-trans (≤-reflexive fourCu)
...
-- them under the fifth power the floor affords.
capΦ-base-ℕ : ∀ (S u : ℕ) → 31 ≤ S → u ≤ S →
  S * (4 * (2 ^ suc (suc (S * (S * S) + S * (S * S) + (S * S + S * S))) * u))
    ≤ 2 ^ (2 ^ S)
capΦ-base-ℕ S u 31≤S hu =
  ...
  S≤Cu = ≤-trans S≤Sq Sq≤Cu
  Cu≤Qu : Cu ≤ Qu
  Cu≤Qu = *-monoʳ-≤ S (*-monoʳ-≤ S S≤Cu)
  twoSq : Sq + Sq ≡ 2 * Sq
  twoSq = solve 1 (λ q → q :+ q := con 2 :* q) refl Sq
  twoS : S + S ≡ 2 * S
  twoS = solve 1 (λ s → s :+ s := con 2 :* s) refl S
  SqSq≤Cu : Sq + Sq ≤ Cu
  SqSq≤Cu = ≤-trans (≤-reflexive twoSq) (*-monoˡ-≤ Sq 2≤S)
  S2≤Cu : S + 2 ≤ Cu
  S2≤Cu =
    ≤-trans (+-monoʳ-≤ S 2≤S)
    (≤-trans (≤-reflexive twoS) (≤-trans (*-monoˡ-≤ S 2≤S) Sq≤Cu))
  fshape : S + Ê ≡ Cu + Cu + (Sq + Sq) + (S + 2)
  fshape = solve 3 (λ s c q → s :+ (con 2 :+ (c :+ c :+ (q :+ q)))
                                := c :+ c :+ (q :+ q) :+ (s :+ con 2))
                 refl S Cu Sq
  fourEq : Cu + Cu + Cu + Cu ≡ 4 * Cu
  fourEq = solve 1 (λ c → c :+ c :+ c :+ c := con 4 :* c) refl Cu
  fit : S + Ê ≤ 2 ^ S
  fit =
    ≤-trans (≤-reflexive fshape)
    (≤-trans (+-mono-≤ (+-monoʳ-≤ (Cu + Cu) SqSq≤Cu) S2≤Cu)
    (≤-trans (≤-reflexive fourEq)
    (≤-trans (*-monoʳ-≤ 4 Cu≤Qu) (quint4≤2^ S 31≤S))))
...
nestCap≤exp-suc e sl id ih =
  ...
  hA : 2 * (S′ + Ê′ + 3 + 2 ^ S) ≤ 2 ^ S′
  hA = capΦ-room-ℕ S′ (2 ^ S)
         (≤-trans (≤ᵇ⇒≤ 31 129 tt) (129≤capsAt-size e sl (suc id)))
...
nestCap≤exp e sl zero =
  subst (_≤ 2 ^ (2 ^ S)) (sym (cong (λ z → S * (4 * z)) (capΦAt-def e sl 0)))
    (capΦ-base-ℕ S (nestCapAt e sl 0)
       (≤-trans (≤ᵇ⇒≤ 31 129 tt) (129≤capsAt-size e sl 0)) C≤S)
  where
...
walk-sight≤exp e sl id =
  ...
  where
  S = Caps.cSize (capsAt e sl id)
  -- the cube and the fifth power the walk's charge now carries, named
  -- once so the collapse equations and the fit below read the same
  -- expressions
  C = S * (S * S)
  Q = S * (S * (S * (S * S)))
  E = suc (Q + Q + Q + (S * S + S * S))
  ...
  31≤S : 31 ≤ S
  31≤S = ≤-trans (≤ᵇ⇒≤ 31 129 tt) (129≤capsAt-size e sl id)
  1≤S : 1 ≤ S
  1≤S = ≤-trans (s≤s z≤n) 8≤S
  2≤S : 2 ≤ S
  ...
  -- THE EXPONENT SUM IS THREE FIFTH POWERS PLUS TWO SQUARES PLUS A
  -- LINE, and the tail past the three is paid by a fourth, which is
  -- what the four on offer covers.
  shapeL : S + (S + (E + (1 + 3 * S)))
             ≡ Q + (Q + (Q + (S * S + S * S + (5 * S + 2))))
                               ...
                               :+ (a :* (a :* (a :* (a :* a)))
                                   :+ (a :* (a :* (a :* (a :* a)))
                                       :+ (a :* a :+ a :* a
                                           :+ (con 5 :* a :+ con 2)))))
                 refl S
  -- THE SQUARES AND THE LINE ARE PAID BY ONE FIFTH POWER BETWEEN THEM,
  -- which is what the fourth on offer is for: three squares sit under a
  -- cube once the size is past three, and a cube sits under a fifth
  -- power once the size is past one, so the whole tail lands inside the
  -- four this ceiling affords.
  6≤S : 6 ≤ S
  ...
  twoSq+lin≤Q =
    ...
    (≤-trans (≤-reflexive (solve 1 (λ a → a :* a :+ a :* a :+ a :* a
                                            := con 3 :* (a :* a)) refl S))
    (≤-trans (*-monoˡ-≤ (S * S) 3≤S) C≤Q))
  fourQ : Q + (Q + (Q + Q)) ≡ 4 * Q
  fourQ = solve 1 (λ q → q :+ (q :+ (q :+ q)) := con 4 :* q) refl Q
  expfit : S + (S + (E + (1 + 3 * S))) ≤ 2 ^ S
  expfit =
    ≤-trans (≤-reflexive shapeL)
    (≤-trans (+-monoʳ-≤ Q (+-monoʳ-≤ Q (+-monoʳ-≤ Q twoSq+lin≤Q)))
    (≤-trans (≤-reflexive fourQ) (quint4≤2^ S 31≤S)))

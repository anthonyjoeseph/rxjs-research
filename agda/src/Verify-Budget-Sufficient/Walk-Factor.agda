...
open import Data.Nat using (ℕ; suc; _+_; _*_; _^_; _≤_; z≤n; _≤ᵇ_)
open import Data.Nat.Properties using
  (≤-refl; ≤-trans; ≤-reflexive; ≤ᵇ⇒≤; +-mono-≤; *-monoˡ-≤; *-identityˡ;
   ^-distribˡ-+-*; ^-monoʳ-≤)
...
-- AND THE WALK NEEDS A LARGER ONE AT EXACTLY ONE FRAME.  `frameNestF`
...
-- premise already carries.  Nothing else moves: at every other frame
-- this reads exactly as `frameNestF` does.
--
...
frameΦF B (scan-f f _)       = 2 ^ sizeᵗ f
...
frameΦSz≤ : ∀ {n} {Γ : Ctx n} {s u} (B : ℕ) (f : Frame Γ s u) →
  frameSz? B f ≡ true → frameΦSz B f ≤ B
frameΦSz≤ B (map-f fn)         h = ≤ᵇ⇒≤ (sizeᵗ fn) B (T-to h)
frameΦSz≤ B (scan-f fn _)      h = ≤ᵇ⇒≤ (sizeᵗ fn) B (T-to h)
frameΦSz≤ B (take-f _)         h = z≤n
frameΦSz≤ B (from-inner _ _ _) h = z≤n
frameΦSz≤ B (thru-outer _ _)   h = ≤-refl

pathΦSz-len : ∀ {n} {Γ : Ctx n} {s t} (B : ℕ) (p : Path Γ s t) →
  pathSz? B p ≡ true → pathΦSz B p ≤ pathLen p * B

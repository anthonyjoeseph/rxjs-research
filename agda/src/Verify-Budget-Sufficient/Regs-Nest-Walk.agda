...
postulate
  ...
  --   telescope is walked.
  -- DEAD ROUTE: instantiating this conclusion at all.  The four
  --   receipts below were taken against the statement this leaf
  --   replaces, whose conclusion read a SUBSCRIPTION; this one reads
  --   the fold that subscription is entered on, and the fold does not
  --   normalise.  Measured on a warm cone at the narrowest fixture
  --   either half owns -- an eight-deep product, one arriving emit, a
  --   four-cell table -- the subscription producing that burst costs
  --   twelve seconds and the same burst pushed through the frame does
  --   not finish in ten minutes; at the thirteen-deep fixture it takes
  --   the machine's memory instead.  So the arrival count and the
  --   product width are both innocent, and what is out of reach is the
  --   frame re-entry itself.  The receipts below are therefore NOT
  --   evidence for this statement and must be re-earned or dropped:
  --   what is owed first is a point at which the fold reduces -- a dry
  --   entry, or a burst supplied directly rather than through a
  --   subscribe -- and if none exists the four probes expire here.

...
| `agda-dev Verify-Batch-Simultaneous/The-Proof.agda` | **7.0 s** | 7.4 s | 5 |
| `make agda (full gate, 2 modules)` | **7.0 s** | 7.0 s | 1 |
| `agda-dev Verify-Well-Formed/Root-Probe.agda` | **6.5 s** | 7.7 s | 3 |
| `agda-dev Verify-Budget-Sufficient/Caps-Bridge.agda` | **6.4 s** | 6.4 s | 10 |
...
| `agda-dev Verify-Well-Formed/Part13.agda` | **5.3 s** | 5.3 s | 5 |
...
| `agda-dev Verify-Budget-Sufficient/Nest-Store.agda` | **3.3 s** | 3.4 s | 3 |
...
Two consequences worth carrying. **The light gate cannot clear a module in this
state at all**, even for a comment-only edit, because a changed file over budget is
red by design while a cone member over budget is merely skipped — so an edit to
`Caps-Bridge` forces the heavy gate whatever it touches. And `Caps-Bridge` is a
**split candidate on the second axis**: `--list` reports 31 blocks and *no* multi-member
one, so every member is in no cycle, nothing is stubbed, and each focused check
re-proves the whole file. Splitting is therefore safe as well as available — there is
no mutuality to restructure — but it moves the tree's claim door and is not to be
done in passing.

...
Cloud container: 4 cores, 15 GB RAM, non-macOS (Linux). No full 66-module cold scan has
been run here yet — the AUTO block below reflects whatever a real green `agda-dev`/
`gate-heavy` run has actually recorded, which is a smaller, growing sample rather than
the laptop's exhaustive one. Treat its rows as real but partial until one lands.

...
> Agda development in [agda/](agda/) states the spec and is proving the
> implementation meets it; what remains is tracked in
> [PROOF-STATE.md](PROOF-STATE.md). The TypeScript implementation is validated
> against the Agda one by a property-based oracle. When the edge cases here
> and the Agda spec disagree, **this section wins.**
>
> **Every example takes one form:** the inputs, labelled, up front; the
> program, in primitive operators; `.subscribe()` at the end; and what is
> logged, in comments underneath.
...
- **After the frame, it counts.** Every emission carries the provenance of
  ...
  when the count drains. No clock, no scheduler tricks.
...
**Batching never reorders.** The batches, joined back together, are exactly
...
```sh
make gate          # the merge gate: cheap textual checks, then the Agda tower
make gate-cheap    # the textual checks alone, in seconds
make qc-fast       # the all-Agda QuickCheck: every top-line statement, on random programs
make bug-cache     # the cached counterexample corpus
make postulates    # what remains to prove, by name
```
...
npm install
npm run typecheck
npm run oracle     # the property oracle: random programs through rxjs and the Agda evaluator
```
...
| [CLAUDE.md](CLAUDE.md), [docs/](docs/) | How the work is done, and the tooling that enforces it |
